...
export async function getOrCreateBusinessTermsAcceptanceInTransaction(
  ...
  if (snapshot.exists) {
    ...
    // Malformed existing document at the deterministic id — fail closed
    // rather than silently overwriting corrupted evidence.
    throw new Error(`Malformed BusinessTermsAcceptance document at "${id}".`);
  }
...
export async function readBusinessTermsAcceptanceInTransaction(
  ...
  return fromBusinessTermsAcceptanceDocument(id, snapshot.data());
}
