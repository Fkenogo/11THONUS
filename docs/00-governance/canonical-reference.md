...
> **Note (`DEC-LOY-016` / `FD-REWARD-QUALIFYING-ITEM-001`, 2026-09-18):** "governed launch taxonomy" (above) and "Reward Programs with fixed 10-unit threshold" together no longer imply that a Reward Program's qualifying item(s) must themselves be governed-taxonomy (Commerce Knowledge `standard_product`/`standard_service`) entries. A Business defines its own qualifying item(s) directly; the governed launch taxonomy remains authoritative for Industry/Business Category/Business Type and for optional product/service classification, but is not a Phase 1 qualification prerequisite. See [Decision Register `DEC-LOY-016`](decisions/decision-register.md) and its [evidence file](decisions/evidence/FD-REWARD-QUALIFYING-ITEM-001-founder-decision-2026-09-18.md).
