...
> **Last controlled update:** 2026-09-07 (`AUTH-MFA-003D-FD-001` — added `DEC-SEC-005` recording Founder disposition `FD-MFA-R` (R1–R10) on the Platform Administrator MFA Recovery Policy: **R1** normal recovery initiated on the target's behalf by an independent authorised Platform Administrator presenting server-verified MFA evidence (no target-initiated pre-MFA recovery-proof mechanism at MVP); **R2** self-approval prohibited without exception; **R3** one independent active MFA-satisfied approver at MVP (verified via `sign_in_second_factor` → `verifiedMfaSatisfied === true`; active status alone insufficient); **R4** break-glass requires explicit Founder authorization + backend/service-account execution, fully attributable, no public endpoint, no standing service-account authority; **R5** session revocation mandatory and fail-closed before factor removal (`approved → revoke → succeeds → verify factor binding → remove factor`; no natural-expiry downgrade); **R6** administrator lifecycle unchanged (recovery state separate from `invited → active → suspended → removed`; no activation/reactivation/role/status change); **R7** persistent recovery-request record bound to the exact original factor enrollment (immutable provider identifier equivalent to `targetFactorEnrollmentId`; no TOTP secrets/codes/recovery credentials stored; retries never remove a replacement factor); **R8** approval expires one hour after approval if execution has not begun (no revival, no in-place extension); **R9** completion requires `reset → fresh TOTP enrollment → end enrollment session → fresh primary sign-in → genuine TOTP challenge → server-verified sign_in_second_factor` before privileged access is restored (no shortcut); **R10** audit vocabulary extended with `mfa_recovery_requested`, `mfa_recovery_approved`, `mfa_recovery_executed`, `mfa_recovery_denied`, `mfa_recovery_expired` (no `mfa_recovery_failed` unless separately justified/approved). Resolves the recovery approver/initiation model left open by `DEC-SEC-004` without superseding its TOTP-only factor policy or dev-only Identity Platform bootstrap; recovery remains TOTP-only; no new Platform Administrator role activated; **this entry records policy only and does not authorize implementation**. The AUTH-MFA-003D design assessment was finalized to the approved design state and closed. Sourced from `AUTH-MFA-003D-DESIGN-001`/`-CORR-001` (PR #232) and the Founder's disposition; no other Decision Register entry modified except the `DEC-SEC-004` note referencing this entry.)
> **Prior update:** 2026-09-04 (`AUTH-MFA-002-CLOSE-001` — added `DEC-SEC-004` recording Founder disposition `FD-MFA-2` on the Platform Administrator MFA policy and Identity Platform upgrade, approved partially-scoped: **dev-only** Identity Platform upgrade authorized for `eleventh-on-us-dev` (`eleventh-on-us-staging` and production NOT authorized, each separately authorizable per environment); **TOTP-only** MFA factor policy for platform administrators (no SMS, no customer/Business MFA); **controlled, auditable, non-bypassable** reset/re-enrollment recovery required (must not bypass MFA, no permanent exemption, no client assertion, no self-service bypass — authority stays server-side Admin-SDK-only; specific approver model Engineering-Lead-determined). Builds strictly within `DEC-SEC-002` and `DEC-GOV-011`/`FD-KS-1`; does not apply to Business users or customers. The Identity Platform upgrade is a one-way, permanent Firebase operation (no downgrade). `PlatformAdministrator.mfaRequired` / enrollment state must NOT be used as proof of session MFA — only the verified `firebase.sign_in_second_factor` claim via the AUTH-MFA-001 trust chain is trusted. This decision does not itself authorize implementation; `AUTH-MFA-003A`/`A/A1`/`B`/`C`/`D`/`E` each require separate authorization per the assessment's §12 package sequence. Sourced from `AUTH-MFA-002` (PR #227) and the Founder's own disposition 2026-09-04; no other Decision Register entry modified.)
> **Prior update:** 2026-08-31 (`DEC-LEGAL-002-BT-DRAFT-004-CORR-001` — PR #206 review-finding corrections to Core Business Terms Part IV: Founder review of PR #206 (head `46f061c1564b4eb0899ba3f12d8b8a90cde52651`, CI SUCCESS) confirmed Part IV broadly sound but identified two P2 review findings requiring correction. **§16.2** — the original wording imported §15's urgent no-prior-notice suspension exception directly into termination, creating a broader immediate-termination power with no governed basis (LEG-FD-06's emergency treatment is scoped to suspension only; the reconciled External Legal Opinion §18 Reinstatement/Termination table states termination occurs "after fair process"); corrected so 11thONUS may terminate for sufficiently serious §15.1-type reasons, ordinary termination follows a fair and appropriate process with no fixed notice/cure period or formal appeals/hearing process invented, urgent circumstances trigger immediate suspension under §15 while termination is separately assessed, and that immediate-suspension authority does not by itself authorize immediate termination without a fair and appropriate process. **§16.4** — the original wording understated LEG-FD-07's mandatory-where-necessary run-off condition by stating an arrangement "may be appropriate"; corrected to state the arrangement "must be provided" where necessary, while preserving full flexibility over duration/mechanics and the existing no-universal-60-day-period/no-mandatory-cash-settlement/non-guarantor protections. Both corrections applied using only already-governed FD-4/LEG-FD-06/LEG-FD-07 authority and the reconciled External Legal Opinion §18 table — no new Founder or legal position required, no new Controlled Input created; CI-01/CI-05 remain the only open controlled inputs. §15.1–§15.7, §16.1, §16.3, §16.5–§16.8, and §17.1–§17.6 re-read against the same authorities and left byte-for-byte unedited — no further contradiction found. Both PR #206 review threads (run-off mandatory condition; emergency-suspension-into-termination scope) replied to with the correcting commit and resolved after verification. `DEC-LEGAL-002` = `OPEN_LEGAL`; Capability 3 = Open; Terms = `NOT CONFIGURED`; `DEC-ID-005`/`DEC-LOY-005`/`DEC-LOY-009`/`DEC-LOY-011`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change; docs-only. PR #206 not merged by this task; Part V not begun. See the [Core Business Terms Draft v4.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v4.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v4.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part IV Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-004-CORR-001-correction-report-2026-08-31.md).) Previously: 2026-08-31 (`DEC-LEGAL-002-BT-DRAFT-004` — Core Business Terms Part IV (Platform Action, Business Exit and Complaints, §§15–17: Suspension and Restriction, Business Exit and Termination; Outstanding Rewards, Complaints and Dispute Facilitation) drafted on branch `docs/dec-legal-002-bt-draft-004` from `origin/main` `b2f798d` (PR #205 merge commit, verified). Drafted entirely on existing authority — FD-2/FD-3/FD-4; `DEC-LOY-011` (CONFIRMED); `DEC-LOY-005` (CONFIRMED); LEG-FD-01/06/07/08/11/12; the governed Business lifecycle-status architecture (`functions/src/domains/business/models/businessStatus.ts`, `ENG-P2-002-DESIGN-001` §6 — `suspended`/`closed` states, no new state invented); `TRD17-subscription-and-billing.md` §17.20; Part I §§1.4/6 and Part III §§10.4/13/14 cross-referenced — no new Founder or legal position required, no `[CONTROLLED INPUT REQUIRED: ...]` marker needed. §15 states 11thONUS's non-exhaustive suspension/restriction-grounds authority (FD-4/LEG-FD-06), the ordinary/immediate distinction with a reasonable-notice-not-fixed-period standard, the commercial/subscription-suspension carve-out preserving `DEC-LOY-011` verbatim, and earned-reward survival during suspension by cross-reference to §13 — no fixed 7/14/24/48-day/hour period invented. §16 states voluntary exit and platform-initiated termination on the same grounds/notice architecture as §15, the FD-3/LEG-FD-07/LEG-FD-08 outstanding-reward core rule (exit does not extinguish earned rewards; no universal 60-day run-off; no mandatory cash settlement; 11thONUS not the funder), and preserves `DEC-LOY-005`'s no-automatic-expiry position against exit being used as an indirect expiry mechanism — no new Business lifecycle status invented. §17 states the LEG-FD-11/LEG-FD-12 three-tier dispute architecture (Customer↔Business primary responsibility; 11thONUS facilitation without becoming merchant/obligor/adjudicator/guarantor; Business↔11thONUS complaint mechanism acknowledged in principle only, final B2B arbitration mechanics left to undrafted §21; Customer↔11thONUS complaints reserved to the separate Customer Terms/Platform Terms instrument) and a platform-records-as-evidence clause with no evidence hierarchy invented. §15.7/§16.8 explicitly preserve the `DEC-ID-005` boundary (owner-initiated business self-suspension) — neither resolved nor implied; `DEC-ID-005` remains `OPEN_FOUNDER`, not resolved by this task, and no new controlled input is created for it. Parts I–III clause text verified byte-for-byte unchanged apart from administrative scope-label corrections (stale "not drafted in this task"/"Part IV heading only" cross-references at Part 0 §0.1/§0.2, Part I §1.4, Part II §10.4, and Part III §13.2/§13.3, corrected once Part IV existed, following the identical precedent established for Parts II/III). CI-01/CI-05 remain the only open controlled inputs; no new one created. `grep`-based prohibited-concept search (fixed 7/14/24/48-day/hour periods, universal 60-day run-off, universal cash settlement/refund, automatic reward extinction on suspension/exit, commercial suspension blocking earned rewards, 11thONUS as reward guarantor/funder, 11thONUS as dispute adjudicator, mandatory/customer-forced arbitration in §17, exhaustive suspension grounds, automatic permanent termination, new Business lifecycle states, subscription plan/pricing/grace-period decisions, `DEC-ID-005`/`DEC-LOY-009`/any `DEC-SUB-*` resolution, Customer Terms content) found every match to be a negation clause or explicit non-resolution/cross-reference statement, not an assertion of the concept. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`); Capability 3 unchanged (blocked on governed Terms-content configuration, `CDR-001` §5); Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged; `DEC-LOY-005`/`DEC-LOY-011` remain CONFIRMED, unaltered. No application/source/Firebase/dependency/config change made; docs-only. PR not yet opened/merged by this task; Part V not begun. See the [Core Business Terms Draft v4.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v4.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v4.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part IV Drafting Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-004-drafting-report-2026-08-31.md).) Previously: 2026-08-31 (`DEC-LEGAL-002-BT-DRAFT-003-CORR-002` — Founder review of PR #205 confirmed Part III substantively sound but identified three narrow authority defects in the previously-approved Part I baseline, explicitly authorizing their correction (not a broad Part I/II reopening). Corrected: **§4.2** — removed "at what threshold" from the list of decisions "the Business makes," which contradicted `TRD10-firestore-data-architecture.md` §10.9.2's platform-fixed MVP earning threshold ("fixed... by platform rule and... not business-configurable"); now states a governed earning threshold, where platform governance fixes one, is determined by that governance, using durable non-numeric wording (no clause hardcodes "10", since TRD10 treats the value as MVP-governed, changeable through formal product approval, not permanent). **§6.1** — removed the unsupported "owns and controls"/"take ownership of" language (no FD-1–7/LEG-FD-01–15 authority establishes an IP/legal-ownership conclusion; FD-5 establishes control/responsibility only); now states the Business "controls and is responsible for" its Reward Program, and the platform does not "take over" the customer relationship. **§6.2** — removed the affirmative "what qualifies as a rewardable purchase" allocation (no CONFIRMED authority establishes unrestricted Business authority over this; the Legal Counsel Handoff Pack §4 lists it among items "not yet Founder-positioned, still genuinely open"); now states this question is "separately governed and... not decided by these Terms" — reserved, not escalated into a new Controlled Input. **§11.1** (Part III) and the **§2 "Reward Program" definition** (Part I, a consequential fix required by this task's own validation checklist for zero surviving "owns" assertions) reconciled to match. A full review-thread reconciliation was performed: all 20 open PR #205 review threads (0 previously resolved via the GitHub API despite prior tasks fixing their underlying text) were individually re-verified against current file content — all 20 underlying issues are now corrected, including the two (§4.2 threshold; §6.2 qualifying-purchase allocation) that the prior task had explicitly flagged rather than fixed, for lack of Part I authorization this task now supplies. All 20 threads replied-to and resolved. No new Controlled Input created; the prior task's "Flagged pre-existing defect — Part I §4.2" item is resolved and reclassified, since governing authority already supplied the answer and this task's own brief explicitly authorized applying it — it was never genuinely a Controlled Input. CI-01/CI-05 remain the only open controlled inputs. `DEC-LEGAL-002` = `OPEN_LEGAL`; Capability 3 = Open; Terms = `NOT CONFIGURED`; `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. §12–§14 reconfirmed unchanged by direct diff, not rewritten. No application/source/Firebase/dependency/config change; docs-only. PR #205 not merged by this task; Part IV not begun. See the [Core Business Terms Draft v3.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v3.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [CORR-002 Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-003-CORR-002-correction-report-2026-08-31.md), which includes the full 20-row review-thread reconciliation table.) Previously: 2026-08-30 (`DEC-LEGAL-002-BT-DRAFT-003-CORR-001` — A seventh review round (head `c00be25`) found one genuine finding out of five reported (three stale), and it is the **only finding across all seven rounds not resolved outright**: already-approved Part I §4.2 (drafted in task `DEC-LEGAL-002-BT-DRAFT-001`, unedited since) states the earning threshold is "[a] decision the Business makes for its own Reward Program," directly contradicting `TRD10-firestore-data-architecture.md` §10.9.2's platform-fixed MVP threshold — the same authority Part III's own §11.1/§11.6 correctly rely on. This defect predates this task; per the governing instruction against substantively rewriting §§1–10, Part I §4.2 is **flagged, not edited** — recorded in the Controlled Inputs Register's new "Flagged pre-existing defect" section, classified as blocking whole-instrument Founder approval (not Part III's own approval, since Part III's own text is correct) pending a dedicated Founder-authorized Part I correction task. A sixth review round (head `168df94`) found two further genuine findings out of six reported (four stale): §11.6 still listed the earning threshold as Business-controlled, contradicting round 5's §11.1 correction — fixed; and round 5's own §11.1 fix had added a disclaimer that created an apparent contradiction with the already-approved Part I §6.2 (which allocates "what qualifies as a rewardable purchase" to the Business as existing baseline) — resolved by cross-referencing §6.2 directly rather than modifying it, per the governing instruction not to substantively rewrite §§1–10; §6.2 verified byte-for-byte unedited by direct diff throughout. Thirty Codex review comments across six rounds in total, sixteen substantively distinct corrections. Previously (five review rounds): Twenty-four Codex review comments (all P2) across five review rounds on PR #205 (2/4/6/6/6 per round: 2, 4, 3-of-6, 2-of-6, 2-of-6 genuine per round, 11 total confirmed stale re-flags of already-fixed earlier-round items) resolved into **thirteen genuine Codex findings plus one self-identified consistency fix, fourteen corrections in total**, per the Correction Report's own numbered findings 1–14. Round 5 (head `8f765b2`) found the two most substantive drafting gaps in this task, both in §11.1 and both missed by the original authority review: **(13)** §11.1 stated the Business "determines... the threshold(s) for earning a reward," but `TRD10-firestore-data-architecture.md` §10.9.2 fixes the MVP earning threshold (`requiredVerifiedUnits`) at 10 platform-wide, "not business-configurable" — corrected so the Business's authority operates within the platform's governed mechanics, "including any platform-fixed earning threshold, which this section does not authorize a Business to alter"; **(14)** §11.1 stated the Business "determines what qualifies as a rewardable purchase," but the Legal Counsel Handoff Pack §4 lists "qualifying-purchase definition" as genuinely open, not yet Founder-positioned — corrected so the Business "selects the purchasable categories from that catalogue that qualify under its own Reward Program" (the narrower, already-governed catalogue-selection authority) with an explicit statement that this section does not resolve the separate open qualifying-purchase-definition question, using the same explicit-non-resolution technique as §13.7/§14.4. These two, alongside the round-3 `DEC-LOY-005` fix (finding 7), are the three substantive drafting corrections in this task; every other correction is administrative or citation-only. Third round (requested via `@codex review`), reviewed head `9ae48745`: **(7) — the one substantive drafting fix in this pass** — §13.6 originally authorized a Business's Reward Program to provide for reward expiry now, but `DEC-LOY-005` ("No automatic reward expiry in MVP") is **CONFIRMED**: earned rewards do not auto-expire in the MVP, the `expired` reward state is architecturally supported but not enabled — a genuine authority-review gap the original drafting pass missed; corrected to preserve `DEC-LOY-005` exactly (no auto-expiry currently, clause does not itself authorize Business-level expiry, any future per-program policy requires separate governed authorization); (8) the Part 0 §0.1 architecture list still labelled Part II "heading only — not drafted," a label stale since Part II was drafted in `DEC-LEGAL-002-BT-DRAFT-002` and never corrected then — corrected; (9) the Drafting Report's file inventory omitted the correction report itself — corrected. First round, reviewed head `ef874961a7`: (1) §14.4 originally asserted a no-customer-reacceptance default for Reward Program changes, prematurely deciding a question the Legal Counsel Handoff Pack §4 records as genuinely open ("programme publication (does publication create a binding offer to customers?)") — corrected to an explicit non-resolution (neither requires nor forecloses a customer-facing reacceptance mechanism), using the same technique §13.7 uses for `DEC-LOY-009`; (2) the Part I heading note still read "Parts III–VIII remain headings/placeholders only" after Part III was drafted, an internally contradictory stale scope label the original five-correction inventory missed — corrected, count updated to six. Second round (requested via `@codex review`), reviewed head `e7a39d2b40`: (3) `docs/00-governance/documentation-changes-log.md` was never updated despite `README.md` Rule 4 requiring an entry for any governed-document change — two entries added (128 for the Part III draft, 129 for this correction pass), following the Part I precedent format; (4) the §11.1 Traceability Matrix row cited FD-5 for the clause's "owns" language, but FD-5 establishes only "responsible for and in control of," a product/commercial position, not a legal ownership conclusion — corrected to attribute "owns" to the Legal Counsel Handoff Pack §2 governing principle (the same source Part I §6.1's identical "owns and controls" wording already cites), with FD-5 now cited only for the control/responsibility proposition; clause text unchanged, since "owns" already appears in the Founder-approved Part I baseline; (5) the "How to read this document" narrative still said "three parts" after this task's Part III bullet made it four — corrected; (6) the Drafting Report's §7/§39/§44 still stated "five" administrative corrections, inconsistent with the corrected Traceability Matrix — corrected to "six." No new controlled input created by any of the six corrections; CI-01/CI-05 remain the only open controlled inputs. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`); Capability 3 unchanged; Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR #205 not merged by this task. See the [Core Business Terms Draft v3.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v3.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v3.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part III Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-003-CORR-001-correction-report-2026-08-30.md).) Previously: 2026-08-30 (`DEC-LEGAL-002-BT-DRAFT-003` — Core Business Terms Part III (Programme Operation, §§11–14: Reward Program Responsibility, Transaction Recording, Reward Obligations, Programme Changes) drafted on branch `docs/dec-legal-002-bt-draft-003` from `origin/main` `1bfcc5c8` (PR #204 merge commit; CI `Build, Lint, Test, Emulator Validation` SUCCESS). Drafted entirely on existing authority (FD-2/FD-3/FD-5/FD-6; `DEC-LOY-011` CONFIRMED; LEG-FD-01/04/05/07/08; `TRD10-firestore-data-architecture.md` §§10.9–10.12; `TRD17-subscription-and-billing.md` §§17.19–17.20; Part I §§4/6 and Part II §10 cross-referenced) — no new Founder or legal position required, no `[CONTROLLED INPUT REQUIRED: ...]` marker needed. §11 states Business Reward Program control (design/ownership/communication/honouring obligations) without inventing reward types/amounts/thresholds/cadence/tiers. §12 states the underlying-transaction/platform-record distinction (Business↔customer sale; 11thONUS not seller/processor/merchant of record) without inventing settlement/POS/tax/chargeback/refund/correction mechanics; Business Code omitted entirely per its governed internal/support-use-only status (`ENG-P2-002-DESIGN-001` §24). §13 states earned-reward survival through suspension (`DEC-LOY-011` default-redeemable-with-governed-exceptions, verbatim substance) and exit (FD-3, no universal run-off/cash-settlement per LEG-FD-07/08), no retrospective reduction (FD-5/LEG-FD-05), reward-value treatment preserving FD-6/LEG-FD-04 exactly (not cash/bank-deposit/e-money/stored-value/withdrawal-entitlement; may have economic value; not "no monetary value"), and reward expiry left to each Business's own Reward Program (no platform-wide expiry rule invented). §13.7 explicitly preserves the `DEC-LOY-009` hard boundary — reward quantity at creation and multiple-unredeemed-reward coexistence are neither drafted nor implied; `DEC-LOY-009` remains `OPEN_FOUNDER`, not resolved by this task, and no new controlled input is created for it. §14 states prospective-only programme changes (FD-5/LEG-FD-05) with a reasonable-notice standard, not a fixed period, and no invented reacceptance mechanism. Parts I–II clause text verified byte-for-byte unchanged apart from five administrative scope-label corrections (stale "not drafted in this task"/"remains Part III, not drafted" cross-references, corrected once Part III existed, following the identical precedent PR #204 review finding 4 established for Part II). CI-01/CI-05 remain the only open controlled inputs; no new one created. `grep`-based prohibited-concept search (shared/unified loyalty programme, 11thONUS as guarantor/funder/merchant/payment processor, blanket "no monetary value," universal cash settlement/60-day run-off/notice period, retrospective reduction, suspension extinguishing rewards, invented expiry/quantity/coexistence semantics, `DEC-LOY-009`/`DEC-ID-005`/`DEC-SUB-*` resolution, Business Code as commerce identifier, mandatory reward type/value/cadence, unsupported transaction-correction mechanics, Part IV/V mechanics, gift-card/stored-value architecture) found every match to be a negation clause expressly denying the concept, not an assertion of it. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`); Capability 3 unchanged (blocked on governed Terms-content configuration, `CDR-001` §5); Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR not yet opened/merged by this task; Part IV not begun. See the [Core Business Terms Draft v3.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v3.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v3.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part III Drafting Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-003-drafting-report-2026-08-30.md).) Previously: 2026-08-30 (`DEC-LEGAL-002-BT-DRAFT-002-CORR-002` — Two final Founder wording corrections applied to Core Business Terms Part II on PR #204, reviewed head `45031bfd`: (1) the §2 "Staff" definition's categorical "Staff are not... Authorized Representatives" was corrected — Staff platform status alone confers neither Business Owner nor Authorized Representative status, but an individual who is Staff may independently hold legal authority to bind the Business under the Business's own corporate/legal arrangements, entirely apart from platform Staff status; that independent authority is never inferred from Staff status alone; (2) §9.6's "to the same extent as if the Business had undertaken those acts itself" strict-liability formulation was replaced with proportionate, itemized account responsibility (deciding whom to authorize; permissions granted; in-scope Staff activity), with explicit carve-outs for unauthorized access, credential compromise, out-of-scope/malicious activity, and 11thONUS-attributable failures, each conditioned on the Business not having breached its own §9.5/§9.7/§9.8 obligations, and an explicit statement that this is account/access responsibility only, not a Part VI §19 liability regime; (3) §9.3's open-ended "except to the extent these Terms expressly provide otherwise" was removed and replaced with language aligned to Part I §7.5's own governance-required-for-delegation boundary — Part I §7.5 itself was read and left byte-for-byte unedited. All four CORR-001 fixes and their review-thread resolutions were re-verified intact before and after this pass; no new automated or human review finding appeared. CI-01/CI-05 remain the only open controlled inputs; no new one created. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`); Capability 3 unchanged; Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR #204 not merged by this task; Part III not begun. See the [Core Business Terms Draft v2.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v2.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v2.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part II Final Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-002-CORR-002-correction-report-2026-08-30.md).) Previously: 2026-08-30 (`DEC-LEGAL-002-BT-DRAFT-002-CORR-001` — Four PR #204 Codex review findings (all P2) corrected on Core Business Terms Part II, reviewed head `ab10dd5a17`: (1) removed the invented approve/decline/restrict verification-outcome architecture from §8.6–§8.8 — `ENG-P3-002-DESIGN-001`'s `pending_verification → trial` mechanism is explicitly ungoverned and "must not be invented"; the corrected clauses state only that verification/participation requirements are separately governed, with no new Controlled Input created (Part II remains fully draftable without resolving that mechanism); (2) added a new "Authorized Representative" definition and corrected "Business Owner"/§9.1/§9.3 so the single governed platform Owner (`ownerUserId`/owner membership, `ENG-P2-002-DESIGN-001` §7) is no longer conflated with every individual who has legal authority to bind the Business (§1.3) — Part I §7.5's already-approved acceptance principle was read and left unedited, since it already contemplates both as alternatives; (3) removed §8.3's self-defeating "onboarding requirements... expressly provide otherwise" exception, so the governed exclusions (Subscription Plan, Staff invitation, Reward Program publication, multi-location establishment) can be changed only through separate governance/authorization, not by future published requirements/UI/config; (4) corrected exactly three stale "Part I only"/"only Part drafted"/"Part II §9, not drafted" labels that became false once Part II was drafted, leaving every other accurate "not drafted in this task" reference (Parts III/IV/VII/VIII) untouched. Added new §10.5 clarifying §10 is a high-level contractual prohibition only, not a fraud-detection/classification/adjudication/enforcement policy — assessed and confirmed that a separate fraud/abuse operational-policy decision is required only before later enforcement design, not before Part II approval. Part I clause body text verified byte-for-byte unchanged by direct diff against `ab10dd5`. CI-01/CI-05 remain the only open controlled inputs; no new one created. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`); Capability 3 unchanged; Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR #204 not merged by this task; Part III not begun. See the [Core Business Terms Draft v2.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v2.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v2.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part II Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-002-CORR-001-correction-report-2026-08-30.md).) Previously: 2026-08-30 (`DEC-LEGAL-002-BT-DRAFT-002` — Core Business Terms Part II (Business Participation, §§8–10: Business Eligibility/Registration/Onboarding, Account Authority, Prohibited Conduct) drafted on branch `docs/dec-legal-002-bt-draft-002` from `origin/main` `cff1091`. Drafted entirely on existing authority (`ENG-P2-002`/`003`/`004`-DESIGN-001, `ENG-P3-002`-DESIGN-001, FD-4, LEG-FD-01, LEG-FD-06, and Part I §1.3/§5/§7.5 cross-referenced) — no new Founder or legal position required, no `[CONTROLLED INPUT REQUIRED: ...]` marker needed. Part I baseline (§§1–7) preserved unedited apart from two additive §2 definitions (Business Owner, Staff) and header/table metadata. Verification treated as discretionary (approve/decline/restrict/request information) with no SLA, automatic-approval rule, exhaustive criteria, or fee invented, consistent with `ENG-P3-002`'s confirmed-ungoverned verification-mechanism finding. Business Type, Team/Staff invitation, Subscription Plan selection, and multi-branch establishment preserved as optional/deferred/out-of-scope per governed onboarding architecture — none elevated to a mandatory eligibility condition. Account Authority limited to the governed Owner/Manager/Staff role model; no unsupported role invented; Terms-acceptance authority boundary (staff permissions never confer bind/accept authority) drawn by direct cross-reference to already-approved Part I §7.5, not restated as new law. Prohibited Conduct catalogue (fraud, fabricated activity, record manipulation, unauthorized access, identity misuse, platform interference, false information, unlawful use, verification-control defeat, customer-information misuse) drawn from FD-4/LEG-FD-06's already-reconciled platform-integrity grounds; cross-references suspension to undrafted §15 without inventing periods or automatic termination; preserves `DEC-LOY-011`/FD-2/FD-3/FD-4 unmodified. CI-01/CI-05 remain the only open controlled inputs, unchanged. `grep`-based prohibited-concept search (mandatory Business Type, Team/Subscription-Plan-as-onboarding, Business Code as public identifier, multi-branch, KYC/KYB, unsupported roles, staff Terms-acceptance authority, automatic verification, fixed numeric periods, `DEC-SUB-*`/`DEC-ID-005`/`DEC-LOY-009` resolution, reward-earning/fulfilment rules) returned zero matches. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`); Capability 3 unchanged (blocked on governed Terms-content configuration, `CDR-001` §5); Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR not merged by this task. See the [Core Business Terms Draft v2.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v2.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v2.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part II Drafting Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-002-drafting-report-2026-08-30.md).) Previously: 2026-08-29 (`DEC-LEGAL-002-FOUNDER-CLOSE-001` — Founder records `LEG-FD-14` (B2B dispute resolution: good-faith → mediation → binding KIAC arbitration, seat Kigali, English/French, Business↔Platform only) and `LEG-FD-15` (liability architecture: Business cap = 12-month fees paid, no invented zero-fee figure; customer standard = "maximum extent permitted by applicable law," no nominal cap adopted), resolving the two items `DEC-LEGAL-002-LEGAL-OPINION-RECON-001` had left open. Core Business Terms drafting readiness raised to 16/16 sections at the architecture/decision level. `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL` — readiness to draft is not itself a closure criterion); `EXT-LEG-002` unchanged (`EVIDENCE_RECEIVED`); Capability 3 unchanged (blocked on governed Terms-content configuration, `CDR-001` §5); Terms configuration unchanged (`NOT CONFIGURED`). Verified PR #202 structurally safe (base retargeted to `ab5e9053`, the PR #201 merge commit already on `origin/main`; PR #202 contains exactly one commit, no PR #201 history resubmitted) before continuing on the same branch/PR — no fresh branch needed. `DEC-LOY-011`/`DEC-ID-005`/`DEC-LOY-009`/`DEC-SUB-*` statuses all verified unchanged. No application/source/Firebase change. See the [Founder Legal Architecture Disposition Record v2.0](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-founder-legal-architecture-dispositions-2026-08-29.md).) Previously: 2026-08-29 (`DEC-LEGAL-002-LEGAL-OPINION-RECON-001` — external Legal Opinion reconciliation: counsel's Comprehensive Legal Opinion & Core Terms Framework response filed verbatim as evidence and reconciled section-by-section against Founder FD-1–FD-7/`DEC-LOY-011`/existing architecture; thirteen Founder legal-architecture positions `LEG-FD-01`–`LEG-FD-13` recorded, confirming most counsel conclusions while declining several specific universal-rule recommendations (blanket "no monetary/cash value" reward phrasing, mandatory 60-day exit run-off, mandatory cash-conversion-on-exit duty, universal 30-day programme-change notice, universal 7/14/24/48 suspension periods, "data as consideration," Kirundi as a general application language, forced scrolling as universal) — each fully reconciled, none left contradictory; live-authority conflict search found zero tracked-document hits for any declined position. `EXT-LEG-002` updated `PENDING` → `EVIDENCE_RECEIVED` (External Dependencies Register). `DEC-LEGAL-002` Status unchanged (`OPEN_LEGAL`) — see its Notes field for the full update; one Founder decision (dispute-resolution forum/seat/rules) remains before Core Business Terms drafting can proceed to completion. §5 Register Summary counts unchanged (no CONFIRMED/OPEN status transition in this update). See the [Reconciliation Matrix](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-reconciliation-matrix-2026-08-29.md). Docs-only; no `functions/`, `apps/web/`, Firestore Rules, or config change.) Previously: 2026-08-29 (`DEC-LOY-011` recorded CONFIRMED — Founder resolution per `DEC-LEGAL-002-FOUNDER-DISP-001`: Option (a), redeemable by default during business suspension, subject to governed exceptions (fraud/security/integrity/legal-regulatory/disputed-validity or another governed exception); commercial-relationship/subscription-status suspension alone does not block redemption; original `DEC-SUB-003` dependency removed (not replaced with `DEC-SUB-008`) as the approved position does not condition on any grace-period value; historical A–D options and question preserved for traceability, not superseded. `DEC-ID-005`'s dependency on `DEC-LOY-011` annotated as resolved; `DEC-ID-005` itself (a broader, separate question) remains `OPEN_FOUNDER`, unchanged. §5 Register Summary counts updated (CONFIRMED 45→46, OPEN_FOUNDER 23→22). See the [Legal Counsel Handoff Pack](evidence/DEC-LEGAL-002-FOUNDER-DISP-001-legal-counsel-handoff-pack-2026-08-29.md), reconciled to no longer ask counsel to choose the redemption model.) Previously: 2026-08-28 (`DEC-TECH-005` register-sync correction — MTAIP-001 11thONUS Alignment Closure: status corrected `OPEN_ENGINEERING` → `CONFIRMED`, with Final decision/Decision date (2026-07-19)/Approved by fields populated verbatim from the already-committed, Founder-signed [Version 1.0 Engineering Authorization Record](../version-1-engineering-authorization-record.md) §8 — the register entry itself had never been updated to match that record, despite the region being fixed in code and the `dev`/`staging` Firebase projects being provisioned accordingly since commit `ba43da1`. This is a traceability correction only; the region decision itself was not reopened or reinterpreted. `DEC-LEGAL-006` has the identical, unaddressed gap and was deliberately left untouched — outside this task's authorization; see the [MTAIP-001 Infrastructure Disposition](../11thonus-infrastructure-disposition-v1.md) and [Alignment Closure Report](../../05-implementation/reports/mtaip-001-alignment-closure-report-2026-08-28.md). §5 Register Summary counts updated (CONFIRMED 44→45, OPEN_ENGINEERING 13→12)). Previously: 2026-08-07 (`DEC-AUTH-001` recorded CONFIRMED — Founder Authentication Foundation Decisions D-A1–D-A5 per task `AUTH-P0-001`: D-A1 official `AUTH-*` work-package series (distinct from `ENG-P2-002/003/004`, no renumbering); D-A2 MVP providers = Phone OTP + Google (email/Apple/passkeys deferred, future additive); D-A3 duplicate-identity merge is a separate governed capability, Authentication never auto-merges; D-A4 SMS is a production-launch concern, build proceeds on the Firebase Auth Emulator; D-A5 customer/staff authentication separated. Records decisions only — authorises no engineering). Previously, same day: `DEC-GOV-009` & `DEC-GOV-010` recorded CONFIRMED — Founder G1/G2: Technical-Review scope (Architecture Review may satisfy DoD §2.6 for in-baseline packages; `-02` needs coverage) and Deployment/Preview/Manual-QA scope (not concern-completion criteria for a domain-layer concern) — clarification only, DoD not weakened). Previously, same day: `DEC-GOV-008` recorded CONFIRMED — Founder Option C: concern-level completion reporting within Capability 2; reporting granularity only, capability numbering/boundary unchanged. Previously, same day: `DEC-PROD-012` CLOSED — Founder Option D: gender not collected at MVP; entry updated to approved/implemented/closed with decision wording and implementation reference). Previously: 2026-07-30 (`RES-006A` — `DEC-DATA-007` CONFIRMED: loyalty-code format (`ABC-234`, checksum-enhanced `ABC-234-X` deferred), plain opaque QR reference, transactional-uniqueness collision handling, and the corrected idempotency invariant (at most one immutable assignment per platform user) recorded as an Engineering Lead decision, no Founder involvement required; see the [`RES-005` Dependency & Scope Analysis](evidence/DEC-DATA-007-dependency-scope-analysis-2026-07-30.md) and [`RES-006` Decision Package](evidence/DEC-DATA-007-decision-package-2026-07-30.md)). Previously: 2026-07-30 (`RES-004A` — `DEC-ID-003` CONFIRMED: Approved Permission Model (inheritance as default, explicit override permitted, sensitive permissions never implicit) and a separate Identity and Accountability Principle recorded per Founder decision following the `RES-004` decision package; three implementation prerequisites — Sensitive Permission Catalogue, Override-Resolution Rule, Permission Evaluation and Audit Design — recorded as unresolved, not designed by this recording; see the [Decision Package](evidence/DEC-ID-003-decision-package-2026-07-30.md)). Previously: 2026-07-30 (`RES-003B` — `DEC-SEC-001` CONFIRMED: authentication recovery order (SMS OTP → Retry/Resend → Google Sign-In → Email Verification → Assisted Support), progressive phone verification, merchant-assistance boundary, and 8 identity-recovery principles recorded per Founder decision following the `RES-003A` review session; see the [Decision Package](evidence/DEC-SEC-001-decision-package-2026-07-30.md) and [Founder Decision Review Package](evidence/DEC-SEC-001-founder-decision-review-package-2026-07-30.md)). Previously: 2026-07-30 (`RES-002B` — `DEC-PROV-004` CONFIRMED: Approved with Conditions, Firebase-native OTP + Google Sign-In within a broader Founder-approved Identity and Authentication Strategy; `DEC-SEC-001` remains open and unresolved by this recording; see the [Decision Package](evidence/DEC-PROV-004-decision-package-2026-07-30.md)). Previously: 2026-07-26 (`DEC-PROV-005-DEC` — `DEC-PROV-005` CONFIRMED: Option C, native backend observability with dedicated frontend diagnostics, initial implementation target Sentry, architecture-only approval; see the [Evidence Pack](evidence/DEC-PROV-005-error-monitoring-evidence-2026-07-26.md)). Previously: 2026-07-16 (Phase 3B — Batch A decisions recorded: DEC-GOV-001, DEC-GOV-006, DEC-LOY-010, DEC-DATA-003 CONFIRMED)
...
- Category: Governance · Status: **OPEN_FOUNDER — partially narrowed by `DEC-GOV-011` (Knowledge Studio scope only, 2026-09-03); the broader eleven-role/Phase-12-admin-build question remains open** · Priority: D2
...
- Current confirmed position: no universal administrator; separation of duties required (TRD18). **Narrowed, 2026-09-03 (`DEC-GOV-011`, `FD-KS-1`): for Knowledge Studio specifically, the MVP role subset is `knowledge_editor` and `knowledge_approver` only — two of the eleven roles this question covers. This does not resolve the question for the other nine roles or for any other TRD18 administrative surface (Business Operations, Customer Support, Trust Reviews, Subscriptions/Billing, Rules Studio, Feature Flags, Emergency Controls), which remain unapproved and unstaffed pending their own separate authorization. No implication that TRD18 as a whole, or its full eleven-role catalogue, has been approved.**
- Founder decision required: Yes · Decision owner: Founder · Required by phase: Phase 12 (full scope); Knowledge Studio scope resolved pre-`ENG-P3-003A` · Blocks: Phase 12 admin build (remaining nine roles/other surfaces only — no longer blocks `ENG-P3-003A`/`B`, see `DEC-GOV-011`)
- Affected documents: TRD18; PRD10; `DEC-GOV-011` (records the Knowledge-Studio-scoped narrowing) · Affected domains: Administration
- Source references: audit traceability report §20; DR-ARCH-008; `ENG-P3-003-DESIGN-001` (v1.1, `CORR-001`)
- Dependencies: — · Risks if unresolved: over-built admin or violated duty separation (for the remaining nine roles/other surfaces)
- Final decision: — (full scope remains open) · Decision date: — · Approved by: — · **Partial/narrowed final decision (Knowledge Studio scope only): see `DEC-GOV-011`, 2026-09-03, Founder (Kenogo).**
- Implementation consequences: admin role seeding (remaining scope) · Document corrections required: TRD18 launch-subset note (remaining scope, not performed by this entry) · Notes: founder agenda Batch E (remaining scope). Knowledge Studio scope closed out by `DEC-GOV-011`/`ENG-P3-003-PROG-SYNC-001`.
...
**DEC-GOV-011 — Knowledge Studio MVP administrator authorization model (`FD-KS-1`)**
- Category: Governance · Status: **CONFIRMED (partial — Knowledge Studio scope only; narrows but does not resolve `DEC-GOV-007`)** · Priority: D2
- Decision question: Should a separate platform-administrator authorization model be built, and if so, which of TRD18's eleven administrator roles are enabled at MVP, specifically for Knowledge Studio (`ENG-P3-003`)?
- Context: `ENG-P3-003-DESIGN-001` (v1.1, `ENG-P3-003-DESIGN-001-CORR-001`, PR #223, merge `7fd7d7c`) found no Founder approval of TRD18 as a whole and identified `DEC-GOV-007` as an open, unresolved, blocking conflict with the design's proposed `knowledge_editor`/`knowledge_approver` role pair. The design's §18 bundled this and three related questions into one Founder-decision surface, `FD-KS-1`, to avoid presenting Founder-approval items as settled architecture.
- Options identified: as presented in `ENG-P3-003-DESIGN-001` §18 `FD-KS-1`(a)–(d): approve, decline, or amend the bundled baseline (separate platform-administrator identity model; Knowledge Studio MVP role subset; separate `KnowledgeDraft` editorial model; self-approval separation).
- Recommended direction: approve as bundled · Recommendation basis: `ENG-P3-003-DESIGN-001` §18.
- Current confirmed position — **Approved, with MVP boundaries**, Founder disposition quoted/summarized in full:
  - A separate platform-administrator authorization model for internal 11thONUS platform administration is approved to exist.
  - For Knowledge Studio MVP, only two roles are enabled: `knowledge_editor` and `knowledge_approver`. The remainder of TRD18's eleven-role administrator-role catalogue is **not activated** and remains deferred until each role's respective administrative capability is separately authorized.
  - The separate `KnowledgeDraft` editorial working model (`ENG-P3-003-DESIGN-001` §7–§8) is approved.
  - `DEC-DATA-005`'s canonical Commerce Knowledge lifecycle is preserved unchanged — **not reopened** by this decision.
  - Separation of duties is approved: an actor must not approve their own `KnowledgeDraft`.
  - `knowledge_editor` may edit any eligible non-published `KnowledgeDraft`, not only drafts they created — this resolves `ENG-P3-003-DESIGN-001` §18's "own drafts only vs. any" open item in favor of "any." Audit history must nonetheless preserve individual actor changes.
  - First-administrator bootstrap must be controlled, non-public, non-self-service, explicitly authorized, and auditable; no personal UID/email may be hard-coded; Engineering may determine the safest implementation mechanism within existing architecture (left as an implementation-package-level question).
  - MFA remains required under `DEC-SEC-002` (unchanged, not reopened); its technical enforcement mechanism remains Engineering-Lead-owned, consistent with `DEC-SEC-002`'s own recorded ownership.
- Founder decision required: Yes (recorded) · Decision owner: Founder · Required by phase: pre-`ENG-P3-003A` · Blocks: nothing further — `ENG-P3-003A`/`ENG-P3-003B` are now unblocked by this decision (still require their own separate implementation authorization, not granted here)
- Affected documents: `ENG-P3-003-DESIGN-001` (v1.1); `CDR-001` §2/§5 (`ENG-P3-003` row); `DEC-GOV-007` (narrowed, amended above — not resolved in full) · Affected domains: Commerce Knowledge, Administration (new)
- Source references: `ENG-P3-003-DESIGN-001-CORR-001` (PR #223, merge `7fd7d7c`); Founder disposition, 2026-09-03
- Dependencies: `DEC-GOV-007` (this entry narrows it; does not supersede it) · Risks if unresolved: n/a — resolved within its stated scope by this entry
- Final decision: **Approved with MVP boundaries, as stated above.** · Decision date: 2026-09-03 · Approved by: Founder (Kenogo)
- Implementation consequences: `ENG-P3-003A` (platform-administrator authorization primitive) and `ENG-P3-003B` (`KnowledgeDraft` model) may now be separately authorized to begin, per `ENG-P3-003-DESIGN-001` §17's package sequence — **this decision does not itself authorize implementation** · Document corrections required: `CDR-001` §2/§5 (`ENG-P3-003` row — design approved, implementation not started); `DEC-GOV-007` (amended above) — both performed by `ENG-P3-003-PROG-SYNC-001` · Notes: **this decision does not approve TRD18 as a whole, any of its other nine administrator roles, or any other TRD18 administrative surface (Business Operations, Customer Support, Trust Reviews, Subscriptions/Billing, Rules Studio, Feature Flags, Emergency Controls) — those remain unapproved and separately authorizable, exactly as `ENG-P3-003-DESIGN-001` §2A classified them.**
...
**DEC-SEC-004 — Platform Administrator MFA policy and Identity Platform upgrade (`FD-MFA-2`)**
- Category: Security · Status: **CONFIRMED (partially-scoped — dev-only Identity Platform upgrade; TOTP-only factor policy; controlled auditable recovery)** · Priority: D2
- Decision question: (1) Should `eleventh-on-us-dev` be upgraded from standard Firebase Authentication to Firebase Authentication with Identity Platform? (2) What multi-factor factor policy should platform administrators use? (3) Who may authorize a platform administrator's MFA factor reset, and what separation-of-duties requirements apply?
- Context: `AUTH-MFA-002` (PR #227, merge `f5f66c1`-based assessment) established that no real platform administrator can currently complete an MFA-authenticated session: MFA is unavailable on the standard Firebase Authentication tier (`eleventh-on-us-dev` runs standard `FIREBASE_AUTH`), TOTP must be explicitly enabled at project level after an Identity Platform upgrade, and no client-side enrollment/challenge UI exists. The upgrade is a one-way, mandatory prerequisite for any MFA capability. `DEC-SEC-002` (Administrator MFA, CONFIRMED) already requires MFA for platform administrators; this entry records the *operational* policy that makes it deliverable. This decision builds strictly within `DEC-SEC-002` and `FD-KS-1`/`DEC-GOV-011` (Knowledge Studio MVP administrators only) — it does not apply to Business users or customers, whose authentication is governed separately.
- Options identified:
  - **Decision 1 (Identity Platform upgrade):** (A) upgrade `eleventh-on-us-dev` now; (B) defer until a production project exists; (C) do not upgrade. Engineering recommendation (A) — the upgrade is the mandatory prerequisite for all subsequent AUTH-MFA-003 packages.
  - **Decision 2 (factor policy):** (A) TOTP only; (B) SMS only; (C) TOTP primary + SMS fallback; (D) both supported; (E) other. Engineering recommendation (A) — TOTP only (no telecom dependency, zero marginal cost, works offline, portable across all African jurisdictions, higher security than SMS which carries SIM-swap/SS7/SMS-interception risk).
  - **Decision 3 (recovery authorization):** (A) Founder only; (B) any active platform administrator; (C) Founder + one active administrator (dual authorization); (D) other. Engineering recommendation (C) — balances security with operational availability.
- Current confirmed position — **Approved, partially-scoped**, Founder disposition quoted/summarized in full (`FD-MFA-2`):
  - **Decision 1 (upgraded, Founder-approved):** **DEV-only** Identity Platform upgrade for `eleventh-on-us-dev` is authorized. **`eleventh-on-us-staging` and production are NOT authorized for Identity Platform upgrade** — those remain separately authorizable per environment when each is configured/created, exactly as the assessment's §6.2 environment-coverage row requires (Founder decision per environment).
  - **Decision 2 (factor policy, Founder-approved):** **TOTP-only** MFA factor policy for platform administrators. No SMS as a factor. **No customer/Business MFA** — this applies exclusively to the internal platform-administrator population; customer and Business authentication are unchanged and separately governed.
  - **Decision 3 (recovery, Founder-approved):** **Controlled, auditable reset/re-enrollment recovery** is required. The recovery path **must not bypass MFA**, must allow **no permanent exemption**, must accept **no client assertion** (reset/re-enrollment authority derives only from the server-side Admin SDK operational path, consistent with `bootstrapPlatformAdministrator`/`transitionPlatformAdministratorStatusPersisted` being Admin-SDK-only and never wired to a callable), and must allow **no self-service bypass**. The specific approver model is not fixed by this recording beyond those mandatory properties; the engineering mechanism (AUTH-MFA-003D) remains Engineering-Lead-owned subject to those constraints.
- Founder decision required: Yes (recorded) · Decision owner: Founder · Required by phase: pre-`AUTH-MFA-003A` · Blocks: nothing further — `AUTH-MFA-003A` (dev-only Identity Platform upgrade + TOTP enablement), `AUTH-MFA-003A1` (trusted discovery callable), `AUTH-MFA-003B`/`C`/`D`/`E` are unblocked by this decision at the governance level but each still requires its own separate implementation authorization; `ENG-P3-003B` (KnowledgeDraft model) is independently unblocked as already recorded
- Affected documents: `AUTH-MFA-002-platform-administrator-mfa-readiness-assessment-2026-09-04.md` (records the assessment this entry completes); `DEC-SEC-002` (extended by this operational policy, not reopened) · Affected domains: Security, Administration, Authentication
- Source references: `AUTH-MFA-002` assessment (§1, §4, §6, §13, §14 `FD-MFA-2`); `AUTH-MFA-001` implementation; `DEC-SEC-002`; `DEC-GOV-011`/`FD-KS-1`
- Dependencies: `DEC-SEC-002` (base requirement, unchanged), `DEC-GOV-011` (`FD-KS-1` — scopes the administrator model this decision's factor policy applies to) · Risks if unresolved: n/a — resolved within its stated scope by this entry
- Final decision: **Approved, partially-scoped, as stated above (dev-only Identity Platform upgrade; TOTP-only platform-administrator factor policy; controlled, auditable, non-bypassable recovery).** · Decision date: 2026-09-04 · Approved by: Founder (Kenogo)
- Implementation consequences: **this decision does not itself authorize implementation** — `AUTH-MFA-003A` (dev Identity Platform upgrade + TOTP enablement, configuration only), `AUTH-MFA-003A1` (discovery callable), `AUTH-MFA-003B`/`C` (enrollment/challenge UI), `AUTH-MFA-003D` (recovery/reset), and `AUTH-MFA-003E` (end-to-end validation) each require separate implementation authorization per the assessment's §12 package sequence · Document corrections required: `AUTH-MFA-002` assessment status updated to CLOSED reflecting these decisions (performed by `AUTH-MFA-002-CLOSE-001`) · Notes: the upgrade is a **one-way, permanent** Firebase operation (no downgrade path) — confirmed in the assessment §11.2 and at Founder review. TOTP enablement and the discovery callable are separately controlled. `PlatformAdministrator.mfaRequired` / account-enrollment state must NOT be used as proof of session MFA — only the verified `firebase.sign_in_second_factor` claim via the AUTH-MFA-001 trust chain is trusted evidence (architecture boundary unchanged, per `AUTH-MFA-001`/`AUTH-MFA-001-CLOSE-READINESS-001`). **The specific recovery approver/initiation model left open by this entry is now resolved by `DEC-SEC-005` (below) as `FD-MFA-R`** — this entry's Decision 3 and its TOTP/DEV-bootstrap policy remain unchanged and are not superseded; only the then-open approver-model specifics are fixed by `DEC-SEC-005`.

**DEC-SEC-005 — Platform Administrator MFA Recovery Policy (`FD-MFA-R`, R1–R10)**
- Category: Security · Status: **CONFIRMED** · Priority: **D1**
- Decision question: What is the approved Platform Administrator MFA recovery policy — who may initiate and approve recovery, how is sole-administrator recovery controlled, what security invariants apply (session revocation, lifecycle, persistent record, expiry, re-enrollment), and what audit vocabulary governs recovery? (Recorded as Founder disposition `FD-MFA-R` under task `AUTH-MFA-003D-FD-001`.)
- Context: `DEC-SEC-004` (`FD-MFA-2`) required controlled, auditable, non-bypassable MFA reset/re-enrollment recovery but deliberately left the specific recovery initiation/approver model unresolved (Engineering-Lead-determined). `AUTH-MFA-003D-DESIGN-001` and its correction `AUTH-MFA-003D-DESIGN-001-CORR-001` (PR #232) progressively designed the recovery architecture and corrected eight automated-review findings; the Founder then reviewed the corrected assessment and the PR and approved the ten decisions R1–R10 as one coherent package on 2026-09-07. This entry records that package. Recovery remains **TOTP-only** (unchanged under `DEC-SEC-004`); Business/customer MFA remains out of scope.
- Options identified: (R1) request initiation — target-initiated pre-MFA proof vs independent authorised MFA-satisfied administrator on the target's behalf (Founder chose the latter at MVP; no target-initiated pre-MFA surface); (R2) self-approval — prohibited without exception vs permitted with supplementary evidence (prohibited); (R3) approval cardinality — one independent MFA-satisfied approver vs two-person (one at MVP); (R4) break-glass authority — Founder-authorized + backend/service-account vs creating standing service-account authority (explicit Founder authorization per invocation, no public endpoint, no standing authority); (R5) session revocation — mandatory fail-closed before factor removal vs optional/downgrade (mandatory, fail-closed); (R6) lifecycle — separate recovery state vs suspending/removing on factor loss (lifecycle unchanged); (R7) persistent record — persistent recovery-request record bound to the exact original factor enrollment vs operation-only (persistent record, immutable provider factor identifier `targetFactorEnrollmentId`); (R8) approval expiry — 1-hour expiry, no revival/extension vs indefinite/configurable (1 hour from approval if execution has not begun); (R9) re-enrollment and fresh challenge — mandatory full re-establishment chain vs removal-only completion (mandatory chain); (R10) audit vocabulary — add the five recovery events vs defer vocabulary (five events approved; no `mfa_recovery_failed` unless separately justified and approved).
- Final decision: **Approved as one package (R1–R10), Founder disposition `FD-MFA-R`, decision date 2026-09-07, approved by Founder.** Full decision substance:
  - **R1 — Recovery Request Authority:** Normal recovery is initiated on behalf of the affected administrator by an independent authorised Platform Administrator presenting server-verified MFA evidence. The MVP does not implement a target-initiated pre-MFA recovery-proof mechanism (no new pre-MFA authentication surface at MVP; the locked-out target does not need a normal AUTH-03 session to initiate recovery). Where no eligible independent administrator exists, the separately governed Founder-authorised break-glass procedure applies.
  - **R2 — Self-Approval:** A Platform Administrator shall never approve or execute their own MFA recovery request. Self-approval is prohibited without exception within the normal recovery path (target ≠ approver; target ≠ normal-path executor where the executor exercises approval authority; no supplementary evidence overrides this prohibition).
  - **R3 — Approval Cardinality:** Normal recovery requires approval by one independent active Platform Administrator whose MFA satisfaction is verified server-side. Two-person approval is not required for MVP. Where no eligible independent approver exists, recovery proceeds only through the separately controlled Founder-authorised break-glass path. Security invariant: an administrator approver must satisfy the authoritative MFA chain `verified Firebase token → firebase.sign_in_second_factor → AuthenticatedCredential.verifiedSecondFactor → deriveVerifiedMfaSatisfied() → verifiedMfaSatisfied === true`; active status alone is insufficient.
  - **R4 — Break-Glass Authority:** Sole-administrator recovery uses a break-glass procedure requiring explicit Founder authorization **and** backend/service-account execution. Service-account capability is an execution mechanism only, not independent standing authority to approve recovery. Every break-glass execution is fully attributable and audited. No public break-glass endpoint; no autonomous service-account recovery authority.
  - **R5 — Session Revocation:** Every recovery execution revokes the target's existing authentication sessions **before** the approved MFA factor is removed. Session revocation is mandatory and **fail-closed** — if revocation cannot be confirmed, factor reset shall not proceed. Required order: `approved recovery → revoke target sessions → revocation succeeds → verify approved factor binding → remove approved factor`. No downgrade relying on natural token expiry. Security invariant.
  - **R6 — Administrator Lifecycle:** MFA recovery does not change the Platform Administrator lifecycle. Recovery state is maintained separately from `invited → active → suspended → removed`. Loss/reset of an MFA factor does not itself suspend or remove the administrator; privileged access remains unavailable until valid MFA is re-established. Recovery must not activate a suspended administrator, reactivate a removed administrator, change roles, or change Platform Administrator status.
  - **R7 — Persistent Recovery Record:** Recovery uses a persistent recovery-request record with a controlled lifecycle covering request, approval, execution, completion, denial, expiry, and failure. The approved recovery binds to the exact original MFA factor enrollment being reset. The persistent record must not store TOTP secrets, TOTP codes, or recovery credentials. The future implementation must retain an immutable provider factor identifier equivalent to `targetFactorEnrollmentId` (or the final architecture-equivalent identifier); retries must never remove a replacement factor.
  - **R8 — Approval Expiry:** An approved recovery authorization expires **one hour** after approval if execution has not begun. An expired recovery requires a new approval; it shall not be revived or extended in place. The exact one-hour policy is recorded as governing authority.
  - **R9 — Re-enrollment and Fresh Challenge:** MFA recovery is not complete merely because the former factor was removed or a replacement factor was enrolled. The affected administrator must complete `old factor reset → fresh TOTP enrollment → end enrollment session → fresh primary sign-in → genuine TOTP challenge → Firebase MFA-resolved session → server verifies firebase.sign_in_second_factor` before privileged Platform Administrator access is restored. No shortcut is authorized. Security invariant.
  - **R10 — Recovery Audit Vocabulary:** Extend the Platform Administration audit vocabulary for future implementation with `mfa_recovery_requested`, `mfa_recovery_approved`, `mfa_recovery_executed`, `mfa_recovery_denied`, `mfa_recovery_expired`. Recovery failure state is preserved in the persistent recovery request. Do **not** add `mfa_recovery_failed` unless implementation later demonstrates a justified need approved separately.
- Decision package coherence: R1–R10 form one approved recovery policy. **Normal path:** locked-out target → independent active MFA-satisfied administrator initiates recovery on the target's behalf → one independent active MFA-satisfied administrator approves → persistent request binds exact original factor enrollment → execution begins before 1-hour approval expiry → target sessions revoked → if revocation fails: STOP / fail closed → approved original factor removed → recovery completion audited → target enrolls new TOTP → enrollment session ends → fresh primary sign-in → genuine TOTP challenge → server verifies real second-factor evidence → privileged access available again. The initiating and approving administrator may be the **same** independent administrator unless existing architecture or prior authority explicitly requires separation; the two-person requirement is not invented. **Target self-approval remains prohibited.** Where no eligible independent administrator exists, the **break-glass / sole-administrator path** applies: no eligible independent administrator → explicit Founder authorization → backend/service-account execution → fully attributed audit → revoke target sessions → if revocation fails: STOP → remove exact approved factor → no role/lifecycle elevation → mandatory new TOTP enrollment → fresh sign-in → genuine challenge → privileged access restored only through verified MFA.
- Founder decision required: Yes (recorded) · Decision owner: Founder · Required by phase: pre-`AUTH-MFA-003D-IMPL-001` · Blocks: nothing further — this records the recovery policy governing the future implementation package; it does not authorize implementation
- Affected documents: `AUTH-MFA-003D-DESIGN-001-platform-administrator-mfa-recovery-assessment-2026-09-07.md` (finalized/closed as the approved design basis); `DEC-SEC-004` (open recovery-policy specifics resolved; TOTP/DEV-bootstrap policy NOT superseded) · Affected domains: Security, Administration, Authentication
- Source references: `AUTH-MFA-003D-DESIGN-001` (PR #232) and `AUTH-MFA-003D-DESIGN-001-CORR-001`; `AUTH-MFA-002`/`DEC-SEC-004`; `DEC-SEC-002`; task `AUTH-MFA-003D-FD-001`
...
**DEC-AUTH-002 — Authentication Architecture Direction: separately controlled external managed IdP boundary (`FD-AUTH-ARCH-001`)**
- Category: Authentication · Status: **CONFIRMED** · Priority: **D1**
- Decision question: Should 11thONUS continue to make Firebase Authentication / Google Identity Platform the target authentication architecture, or adopt a separately controlled external managed identity-provider boundary while retaining Firebase application/data services — and does that direction select a replacement provider?
- Context: `AUTH-ARCH-001` (PR #234, assessment plus `AUTH-ARCH-001-CORR-001`/`CORR-002`) established, and the Founder accepted, that Firebase Authentication cannot currently satisfy the fixed administrative MFA recovery invariants under `DEC-SEC-005` (`FD-MFA-R`), especially the exact-factor/replacement-factor recovery requirements (R5/R7), while Firebase Hosting, Cloud Functions, Firestore and Storage remain a sound application/data platform that does not automatically need migration if authentication changes. The current implementation state is pre-pilot. `AUTH-MFA-003D-IMPL-001` is authorised (`FD-AUTH-MFA-003D-IMPL-001` VALID) but its execution is blocked at the FEF Entry Gate provider-capability check.
- Options identified: (a) retain Firebase Authentication as the target; (b) adopt a separately controlled external managed IdP boundary while retaining Firebase application/data services (CHANGE); (c) bridge an external IdP into Firebase Auth custom tokens (dual-token bridge — assessed, not recommended).
- Current confirmed position: **Approved: Decision A — CHANGE (option b). Architecture direction only; no replacement provider is approved.**
- Founder decision required: No (received; recorded per task `AUTH-ARCH-001-FD-001` as Founder disposition `FD-AUTH-ARCH-001`) · Decision owner: Founder · Required by phase: Phase 2 (pre-pilot authentication architecture) · Blocks: unblocks `AUTH-ARCH-002` validation scoping; does not unblock any migration or implementation
- Affected documents: `AUTH-ARCH-001-authentication-identity-provider-architecture-reassessment-2026-09-07.md` (closed as the assessment basis; final Founder disposition section added, historical research preserved); `AUTH-MFA-003D-IMPL-001-work-package-2026-09-07.md` (execution remains blocked; to be re-baselined or replaced once the target IdP is selected) · Affected domains: Authentication, Security, Administration, Identity
- Source references: `AUTH-ARCH-001` (PR #234, exact Founder-reviewed head `be4c1c7692a7293a0ba5bba0b6cf4700cc61bf5e`); `DEC-SEC-005` / `FD-MFA-R` R1–R10 (unchanged); `DEC-SEC-004` / `FD-MFA-2` (unchanged); `AUTH-MFA-003D-PROVIDER-001` evidence (unmerged, evidence only) · Dependencies: `DEC-SEC-005` (fixed invariants, unchanged), `DEC-AUTH-001` (foundations, unchanged), `DEC-SEC-004` (factor policy, unchanged) · Risks if unresolved: n/a — direction resolved; provider risk remains open and is carried by `AUTH-ARCH-002`
- Final decision: **APPROVED: CHANGE — 11thONUS will adopt a separately controlled external managed identity-provider boundary for authentication; Firebase Authentication / Google Identity Platform is no longer the target authentication architecture under the currently approved 11thONUS requirements.** Firebase may continue to provide Hosting, Cloud Functions, Firestore, Storage and other separately justified infrastructure services; this decision does not itself authorize their migration, retention forever, or redesign. Controlled provider dependency is approved as architectural principle: providers may own credential verification, authentication ceremonies, MFA mechanisms and provider sessions, while 11thONUS retains authority over durable identity, roles, permissions, business relationships, recovery policy and domain authorization; provider identifiers, claims and lifecycle concepts must not silently become durable 11thONUS business/domain authority. Preferred boundary: external managed IdP → provider token/subject → controlled authentication adapter/reference → 11thONUS durable Customer Identity → Platform Administrator / Business / authorization domains; the provider subject remains an opaque authentication reference and is not authorized as Customer Identity ID, Business identity, Platform Administrator identity, or role/permission authority. Firebase Authentication is not characterized as defective generally: this is a product/provider-fit decision based on the now-known 11thONUS requirements (R5/R7); historical Firebase implementation effort counts as transition cost, blast-radius evidence and migration risk, not as justification for retaining the provider. **No replacement identity provider is approved.** Provider qualification stands as: Firebase Authentication unsuitable under current R5/R7; Cognito eliminated under the corrected exact-factor/replacement-factor analysis; Auth0 leading candidate, validation required, NOT selected; other candidates as recorded in `AUTH-ARCH-001`. The next provider decision requires `AUTH-ARCH-002 — External IdP Hard-Invariant Validation` (not started by this recording). `AUTH-MFA-003D-IMPL-001` stays AUTHORISED-VALID / BLOCKED-execution and will later need re-baselining or replacement once the target IdP is selected. Programme handoff note only (no persistence decision): the Founder intends to open `DATA-ARCH-001 — Pre-Pilot Persistence Architecture Reassessment` after `AUTH-ARCH-001` closes, to evaluate whether PostgreSQL should become the authoritative durable transactional datastore; nothing is provisioned, changed or decided here. · Decision date: 2026-09-08 · Approved by: Founder
- Implementation consequences: authorises no migration, no provider procurement, no implementation, and no resumption of `AUTH-MFA-003D-IMPL-001`; `AUTH-ARCH-002` scoping may proceed as a separate package (exact-factor F1/F2 semantics, complete R5 revocation contract incl. session deletion, refresh-token revocation, already-issued-JWT treatment, possible 11thONUS-owned cutoff, server-verifiable MFA evidence, Google/email-password/verification-reset/phone requirements, external-JWT→Functions/API architecture, App Check, ops/admin tooling, pricing/Enterprise entitlement, Burundi/Rwanda considerations — validation only, not started here) · Document corrections required: AUTH-ARCH-001 final-disposition section (performed by this recording); decision-log/tracking entries (this entry; documentation-changes-log; implementation-changes record) · Notes: possible 11thONUS-owned token/session revocation cutoff remains candidate architecture requiring validation and possible Founder/security authority — not approved. Historical assessment research and corrections preserved, not rewritten.
...
**DEC-DATA-008 — Persistence architecture direction: PostgreSQL as authoritative durable transactional datastore (`FD-DATA-ARCH-001`)**
- Category: Data · Status: **CONFIRMED** · Priority: **D1**
- Decision question: Should Firestore remain the presumed authoritative primary durable datastore, or shall PostgreSQL become the authoritative durable transactional datastore for 11thONUS — and, if PostgreSQL, through which managed/access architecture?
- Context: `DATA-ARCH-001` (PR #236, assessment plus `DATA-ARCH-001-CORR-001`) reassessed the TRD10 Firestore target against governed relational/domain requirements at `origin/main` `ee89903`, explicitly excluding provisional/ungoverned FD-COM-001/CB-004/CB-008 material from the basis. The verified implementation footprint is 80 production Functions files importing Firestore, 30 with collection calls, and 31 executable `runTransaction` calls across 22 files. Pre-pilot timing makes the transition predominantly code/test migration rather than customer-data migration (user/data risk low but unverified).
- Options identified: (a) retain Firestore as primary with bounded corrections (viable, not strongest); (b) direct/server-governed Cloud SQL for PostgreSQL through the Functions/API boundary (recommended); (c) Firebase SQL Connect (assessed, not preferred); (d) deliberate PostgreSQL + Firestore hybrid (not preselected); (e) other stores as primary (no repository requirement supports them).
- Current confirmed position: **Approved: CHANGE (option b). Architecture direction only; no implementation, provisioning, or migration authorized.**
- Founder decision required: No (received; recorded per task `DATA-ARCH-001-FD-001` as Founder disposition `FD-DATA-ARCH-001`) · Decision owner: Founder · Required by phase: Phase 2 (pre-pilot persistence architecture) · Blocks: unblocks future combined target-design scoping; does not unblock any migration, provisioning, or implementation
- Affected documents: `DATA-ARCH-001-pre-pilot-persistence-architecture-reassessment-2026-09-08.md` (closed as the assessment basis; Founder disposition section added, history preserved); `AUTH-ARCH-002` (IdP validation track, unchanged) · Affected domains: Data, Security, Identity
- Source references: `DATA-ARCH-001` (PR #236); TRD10 §§10.2–10.35; TRD11 §§11.14–11.30; TRD15; TRD17 (governed content only — FD-COM-001 mechanics excluded); `DEC-AUTH-002` (external-IdP compatibility) · Dependencies: `DEC-AUTH-002` (compatible direction, unchanged), TRD10 (requirements authority; prior Firestore selection under reassessment, not a circular mandate) · Risks if unresolved: n/a — direction resolved; sizing, schema, and migration risks carried by the future target-design package
- Final decision: **APPROVED: CHANGE — 11thONUS shall use PostgreSQL as the authoritative durable transactional datastore. Preferred infrastructure/access direction: Cloud SQL for PostgreSQL, accessed server-side through the 11thONUS Functions/API boundary.** 11thONUS retains ownership of domain identities, business relationships, domain IDs, authorization, business invariants, lifecycle semantics and persistence contracts; PostgreSQL provides the durable relational/transactional storage mechanism. Firestore is not automatically removed but is **no longer presumed to be the authoritative primary durable datastore**; bounded future workloads only if a future target-design package demonstrates a specific justified use — no dual authoritative storage, synchronization architecture, projection or cache strategy is authorized here. Firebase SQL Connect was assessed but is **not the preferred target access architecture** (PostgreSQL portability desirable; SQL Connect adds Firebase-specific access/runtime semantics including a Firebase-Authentication-coupled client authorization model; the approved external-IdP direction favors a server-governed boundary — not a general defect finding). PostgreSQL is the database model/technology choice and Cloud SQL the current preferred managed operational host; this is deliberately provider-managed, not provider-independent, and **no Cloud SQL provisioning is authorized**. Direction is compatible with `DEC-AUTH-002` (External managed IdP → Functions/API → 11thONUS domain → PostgreSQL). This decision does not depend on provisional FD-COM-001 assumptions. Programme note only: once the external IdP is selected/validated and this direction is closed, a future controlled target-design/transition package should determine the coordinated pre-pilot implementation (not instantiated here). · Decision date: 2026-09-08 · Approved by: Founder
- Implementation consequences: authorises no provisioning, schema, dependencies, data migration, repository/Functions changes, Data Connect, Firestore removal, hybrid, authentication change, Auth0 selection, or combined-programme start · Document corrections required: DATA-ARCH-001 disposition section (performed by this recording); decision-log/tracking entries (this entry; documentation-changes-log Entry 187; implementation-changes record) · Notes: historical assessment reasoning and CORR-001 evidence preserved, not rewritten.
...
**DEC-LEGAL-002 — Consumer/loyalty terms and business reward obligations** — Status: **OPEN_LEGAL** · Priority: D3 · Question: Reward Program terms, business obligation to honour rewards, dispute language, platform liability, subscription terms. Owner: Founder + legal adviser · Required by: Business Terms component — immediate (Capability 3 dependency, Founder-reprioritised 2026-08-29, `DEC-LEGAL-002-FOUNDER-DISP-001`); remaining legal/pilot-gate items — Phase 14/pilot (unchanged) · Blocks: pilot launch; business agreements; Capability 3 completion (Business Terms component only) · Sources: LCD-002; DR-LEGAL-002 · Dependencies: EXT-LEG-002 · Final decision/date/approved: — · Notes: FD-1 (`DEC-LEGAL-002-FOUNDER-DISP-001`, 2026-08-29) — Founder re-prioritised the Business Terms component as an immediate Capability 3 dependency; the runtime `assertCurrentBusinessTermsAccepted` fail-closed gate is confirmed correct and is not to be weakened. This does not change Status (remains OPEN_LEGAL), does not resolve the decision, and does not bring other Phase 14/15 legal activity forward. Founder product positions on FD-2–FD-7 recorded in the [Founder Decision Sheet](evidence/DEC-LEGAL-002-PREP-001-founder-decision-sheet-2026-08-29.md); legal-counsel handoff at [DEC-LEGAL-002-FOUNDER-DISP-001 Legal Counsel Handoff Pack](evidence/DEC-LEGAL-002-FOUNDER-DISP-001-legal-counsel-handoff-pack-2026-08-29.md). **`DEC-LEGAL-002-LEGAL-OPINION-RECON-001` (2026-08-29) — external Legal Opinion received and reconciled.** External legal counsel's Comprehensive Legal Opinion & Core Terms Framework response was received, filed verbatim as evidence, and reconciled against Founder FD-1–FD-7, `DEC-LOY-011`, and existing governed architecture across all 20 opinion sections (see [Reconciliation Matrix](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-reconciliation-matrix-2026-08-29.md)). Thirteen Founder legal-architecture positions (`LEG-FD-01`–`LEG-FD-13`) were recorded (see [Founder Legal Architecture Disposition Record](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-founder-legal-architecture-dispositions-2026-08-29.md)), most of which confirm or narrowly qualify counsel's conclusions; several specific counsel recommendations were **not adopted** as universal rules (the blanket "rewards have no monetary/cash value" phrasing — corrected per FD-6; a mandatory 60-day exit run-off period; a mandatory cash-conversion duty on Business exit; a universal 30-day programme-change notice; universal 7/14/24/48-day/hour suspension periods; "customer data as contractual consideration"; Kirundi as a general application language; forced scrolling as a universal acceptance requirement) — each fully reconciled, not left contradictory, per the Reconciliation Matrix. A live-authority conflict search found no tracked document currently asserting any of the not-adopted positions. `EXT-LEG-002` updated `PENDING` → `EVIDENCE_RECEIVED` (External Dependencies Register) as a result. **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`) and does not itself draft, approve, or configure any Terms content or version. One concrete Founder decision remains outstanding before Core Business Terms drafting can proceed to completion: selection of the dispute-resolution forum/seat/rules (Business↔Platform disputes clause) — see the [Post-Legal-Review Resolution Assessment](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-resolution-assessment-2026-08-29.md) and the [Terms Drafting Readiness Note](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-terms-drafting-readiness-2026-08-29.md) (14 of 16 Business Terms sections are drafting-ready on principle-based language; Disputes/corrections and Liability sections are not). Customer Terms are determined to be a separate future governed work package and are confirmed **not** a Capability 3 blocker (Capability 3's actual blocker remains the Business Terms component only, per direct inspection of `assertCurrentBusinessTermsAccepted`). No `DEC-SUB-*` status changed; no application/source/Firebase change made. **`DEC-LEGAL-002-FOUNDER-CLOSE-001` (2026-08-29) — LEG-FD-14/LEG-FD-15 recorded, resolving the two outstanding items above.** `LEG-FD-14` (B2B dispute resolution, APPROVED): Business↔11thONUS disputes proceed good-faith resolution → mediation where appropriate → binding arbitration, seat Kigali Rwanda, KIAC rules, English or French; jurisdictional overlays may modify where mandatory law requires; this does not impose the same architecture on customer disputes, which retain the existing local-court/complaint-mechanism principle. `LEG-FD-15` (liability architecture, APPROVED WITH JURISDICTIONAL/LEGAL QUALIFICATION): aggregate direct 11thONUS liability to a Business capped at total fees paid in the preceding 12 months (no invented nominal cap for a zero-fee/complimentary/pilot Business — left to future drafting/`DEC-SUB-013`, unresolved and untouched); counsel's suggested nominal $25 USD/BIF customer cap **not adopted** — replaced by "liability to customers limited to the maximum extent permitted by applicable law," subject to mandatory consumer rights; no limitation purports to override a non-excludable liability. Both recorded in the [Founder Legal Architecture Disposition Record v2.0](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-founder-legal-architecture-dispositions-2026-08-29.md) and the [Reconciliation Matrix v2.0](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-reconciliation-matrix-2026-08-29.md) (rows 8/9/13 updated, zero rows now classified F/deferred). **Core Business Terms drafting readiness is now 16/16 sections at the architecture/decision level** (see the [Terms Drafting Readiness Note v2.0](evidence/DEC-LEGAL-002-LEGAL-OPINION-RECON-001-terms-drafting-readiness-2026-08-29.md)) — this means the Founder architecture needed to draft every section now exists; it does **not** mean any Terms text is drafted, approved, configured, or effective. **`DEC-LEGAL-002` Status remains unchanged (`OPEN_LEGAL`)** — per this repository's governance convention, readiness to draft is not itself a criterion for closing this entry; closure requires actual Terms content drafted and Founder-approved, and a governed Terms version configured and verified (Resolution Plan steps 5–7), none of which this task performs. `EXT-LEG-002` remains `EVIDENCE_RECEIVED` (unchanged). Capability 3 remains `Open — engineering work packages complete; blocked on governed Terms-content configuration` (`CDR-001` §5, unmodified — unblocking requires an actual configured Business Terms version, not architecture readiness). Terms configuration remains **NOT CONFIGURED**. Customer Terms boundary reaffirmed unchanged (separate future work package, not a Capability 3 blocker). `DEC-LOY-011` remains CONFIRMED; `DEC-ID-005`/`DEC-LOY-009` remain OPEN_FOUNDER; all previously-unresolved `DEC-SUB-*` items remain unresolved. No application/source/Firebase change made. See [`DEC-LEGAL-002-FOUNDER-CLOSE-001-founder-legal-closure-report-2026-08-29.md`](../../05-implementation/reports/DEC-LEGAL-002-FOUNDER-CLOSE-001-founder-legal-closure-report-2026-08-29.md). **`DEC-LEGAL-002-BT-DRAFT-001` (2026-08-30) — Core Business Terms Instrument Architecture & Part I drafted.** On branch `docs/dec-legal-002-bt-draft-001` (from `origin/main` `5c2d1df`), a controlled drafting task produced: the complete proposed Core Business Terms section architecture (27 headings across 8 Parts, mapping every row of the Terms Drafting Readiness Note's §3 table — seventeen rows by direct count, a discrepancy against that note's own "16/16" narrative conclusion, disclosed but not corrected in that read-only record); and full clause text for Part I only (Parties and Agreement, Definitions, Purpose and Scope, Platform Role, Independent Business Relationship/No Agency, Business/Customer Relationship Boundary, Acceptance and Formation — implementing LEG-FD-03's five-element acceptance standard and declining forced scrolling as a global requirement). Six `[CONTROLLED INPUT REQUIRED: ...]` placeholders catalogued in a dedicated Controlled Inputs Register (operator legal identity; multi-entity/affiliate scope; single-text-vs-jurisdiction-variant assumption; service-tier differentiation; reacceptance-on-change engineering decision per LEG-FD-13; delegated-staff acceptance authority) — none invented. A full clause-level Drafting Traceability Matrix and an actual `grep`-based prohibited-concept search (30/60/7/14/24/48-day/hour periods, $25 cap, Kirundi, forced scrolling, "no monetary/economic value," gift-card/stored-value rules, `DEC-SUB-*`/`DEC-ID-005`/`DEC-LOY-009` resolution) found zero adoptions. Parts II–VIII remain headings/placeholders only — no clause text drafted. **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`); `EXT-LEG-002` unchanged (`EVIDENCE_RECEIVED`); Capability 3 unchanged (`Open — engineering work packages complete; blocked on governed Terms-content configuration`, `CDR-001` §5); Terms configuration unchanged (**NOT CONFIGURED**); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase change made; docs-only. See the [Core Business Terms Draft](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Drafting Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-001-drafting-report-2026-08-30.md). **`DEC-LEGAL-002-BT-DRAFT-001-CORR-001` (2026-08-30) — Founder corrections applied to Core Business Terms Part I (PR #203, APPROVE WITH CORRECTIONS disposition).** On the same branch `docs/dec-legal-002-bt-draft-001`, eleven itemized Founder corrections were applied in place to the Part I draft: §1.3 no longer states 11thONUS is categorically not required to verify corporate authority (representation preserved; reasonable-evidence request permitted, no routine verification requirement created); §3.2(c)/§6.3 no longer use "if and when established"/undecided-instrument phrasing — the Customer Terms / Platform Terms of Use instrument is stated as an approved separate legal-instrument architecture and future controlled work package under LEG-FD-10, still not drafted here and still not a Capability 3 blocker; §7.1 no longer predetermines that acceptance is a universal condition of continued platform participation — only that acceptance precedes submission for verification, with reacceptance-on-change reserved to §22; §7.6 no longer implies a Terms-acceptance retention standard — an auditable record is required, subject to applicable law and governed data-retention policy; the §2 "11thONUS"/"the platform" definition now uses the governed product category "customer-verified loyalty platform" rather than a narrow technical infrastructure list (detailed infrastructure unchanged in §4). Four controlled inputs were resolved conservatively and moved to a "Resolved this correction pass" section of the Controlled Inputs Register: business-entity scope (§1.2 — Terms bind only the registered entity, no automatic affiliate/franchise party absent express agreement), jurisdiction architecture (§3.3 — confirms the existing Layer-1 portable Core Terms + Layer-2 jurisdictional-overlay model, no new legal layer invented), service-tier differentiation (§4.4 — placeholder removed from Part I entirely, no tier invented, future commercial differentiation left to §18/`DEC-SUB-*`), and delegated-acceptance authority (§7.5 — only the registering Business Owner/authorized representative with authority to bind may give initial acceptance; ordinary staff/platform permissions confer no such authority; any future delegation requires explicit governance). CI-01 (operator legal identity) and CI-05 (reacceptance-on-change engineering decision, per LEG-FD-13) remain the only two open controlled inputs in Part I, per direct verification against the reviewed authorities. **Readiness-count correction:** the Terms Drafting Readiness Note's §3 table was independently re-counted by direct line inspection and confirmed to contain seventeen rows, every one marked Ready, against that note's own "16 of 16" narrative conclusion; this is recorded as a non-substantive counting/labelling error — no section added, no readiness decision changed — and the live enumeration in the BT-DRAFT-001 draft, Traceability Matrix, Controlled Inputs Register, and Drafting Report is corrected to **17 of 17 sections ready to draft**. The Terms Drafting Readiness Note itself (2026-08-29, historical evidence) is left unedited, per the instruction not to overwrite historical evidence. An actual `grep`-based re-search confirmed zero accidental resolution of any `DEC-SUB-*` item, zero privacy/retention decision resolved, no staff-acceptance-authority overreach, no remaining hypothetical Customer Terms wording, no remaining universal continued-participation reacceptance language, no narrow technical platform redefinition remaining, and no adoption of any previously-rejected external-legal-opinion recommendation. **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`); `EXT-LEG-002` unchanged (`EVIDENCE_RECEIVED`); Capability 3 unchanged (`Open — engineering work packages complete; blocked on governed Terms-content configuration`, `CDR-001` §5); Terms configuration unchanged (**NOT CONFIGURED**); `DEC-LOY-011` remains CONFIRMED; `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR #203 not merged by this task. See the [Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-001-CORR-001-correction-report-2026-08-30.md). **`DEC-LEGAL-002-BT-DRAFT-002` (2026-08-30) — Core Business Terms Part II (Business Participation) drafted.** On a fresh branch `docs/dec-legal-002-bt-draft-002` (from `origin/main` `cff1091`, the verified `DEC-LEGAL-002-BT-DRAFT-001-CORR-001` merge/PR #203), full clause text was drafted for §8 (Business Eligibility, Registration and Onboarding), §9 (Account Authority — Owners, Staff and Permissions), and §10 (Prohibited Conduct) — the three Part II sections, per governing task scope. Drafted entirely on existing authority: `ENG-P2-002`/`ENG-P2-003`/`ENG-P2-004`-DESIGN-001 (Owner/Manager/Staff role and permission-resolution architecture), `ENG-P3-002`-DESIGN-001 (onboarding mandatory/optional/out-of-scope classification and the confirmed-ungoverned verification-mechanism boundary), Founder FD-4 and LEG-FD-01/LEG-FD-06 (platform-integrity/suspension-grounds authority, applied to Prohibited Conduct), and Part I §1.3/§5/§7.5 of this same instrument (cross-referenced, not redrafted) — no new Founder or legal position was required and none was created; no `[CONTROLLED INPUT REQUIRED: ...]` marker was needed in Part II. Part I clause text (§§1–7) was preserved unedited; only two additive §2 definitions ("Business Owner," "Staff") and header/table metadata were added. Business Type, Team/Staff invitation, Subscription Plan selection, and multi-branch establishment are preserved as optional/deferred/out-of-scope, matching the governed onboarding architecture — none elevated to a mandatory eligibility condition; Business Code is not used in the contractual text at all. Business verification is drafted as a discretionary approve/decline/restrict/request-information standard with no SLA, automatic-approval rule, exhaustive criteria, or fee invented, consistent with `ENG-P3-002`'s explicit finding that the verification mechanism itself is ungoverned. Account Authority is limited to the governed Owner/Manager/Staff role model (no "Admin"/"Editor" or other unsupported role invented) and the Terms-acceptance-authority boundary is drawn by direct cross-reference to already-Founder-approved Part I §7.5, not restated as new law. The Prohibited Conduct catalogue (fraud, fabricated/false loyalty activity, Reward Program record manipulation, unauthorized access, identity/account misuse, platform interference, false information to 11thONUS, unlawful use, defeating verification/trust controls, customer-information misuse) is drawn from FD-4/LEG-FD-06's already-reconciled, non-exhaustive platform-integrity grounds — the same authority basis the Terms Drafting Readiness Note itself cites in rating this row Ready — and cross-references consequences to the undrafted Part IV §15 without inventing any notice period, cure period, or automatic-termination/automatic-reward-extinction rule; `DEC-LOY-011` and FD-2/FD-3/FD-4 are preserved unmodified. CI-01 (operator legal identity) and CI-05 (reacceptance-on-change engineering decision) remain the only two open controlled inputs, unchanged by this task. A `grep`-based prohibited-concept search (mandatory Business Type, Team/Subscription-Plan-as-onboarding-requirement, Business Code as public/commercial identifier, unsupported multi-branch assumptions, invented KYC/KYB requirements, unsupported roles, staff Terms-acceptance authority, automatic verification approval, fixed numeric notice/cure periods, `DEC-SUB-*`/`DEC-ID-005`/`DEC-LOY-009` resolution language, and reward earning/fulfilment/value rules) returned zero matches. **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`); `EXT-LEG-002` unchanged (`EVIDENCE_RECEIVED`); Capability 3 unchanged (`Open — engineering work packages complete; blocked on governed Terms-content configuration`, `CDR-001` §5); Terms configuration unchanged (**NOT CONFIGURED**); `DEC-LOY-011` remains CONFIRMED; `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR not merged by this task; Part III drafting not begun. See the [Core Business Terms Draft v2.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v2.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v2.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part II Drafting Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-002-drafting-report-2026-08-30.md). **`DEC-LEGAL-002-BT-DRAFT-002-CORR-001` (2026-08-30) — PR #204 review findings corrected.** Four Codex automated review findings (all P2, on reviewed head `ab10dd5a17`) were corrected on the same branch: (1) the invented §8.6–§8.8 verification approve/decline/restrict outcome architecture was removed — `ENG-P3-002-DESIGN-001`'s `pending_verification → trial` mechanism is explicitly ungoverned and "must not be invented"; corrected clauses state only that verification/participation requirements are separately governed, with no new Controlled Input created; (2) a new "Authorized Representative" definition was added and "Business Owner"/§9.1/§9.3 corrected so the single governed platform Owner (`ownerUserId`/owner membership, `ENG-P2-002-DESIGN-001` §7) is no longer conflated with every individual holding §1.3 legal authority to bind the Business — Part I §7.5's already-approved acceptance principle read and left unedited; (3) §8.3's self-defeating "expressly provide otherwise" exception was removed so the four onboarding exclusions can be changed only through separate governance/authorization; (4) exactly three stale "Part I only"/"only Part drafted"/"Part II §9, not drafted" labels were corrected, with every other accurate "not drafted in this task" reference left untouched. New §10.5 clarifies §10 is a contractual prohibition only, not a fraud-detection/adjudication/enforcement policy; assessed that a separate fraud/abuse operational-policy decision is required only before later enforcement design, not before Part II approval. Part I clause body text verified byte-for-byte unchanged by direct diff. CI-01/CI-05 remain the only open controlled inputs; no new one created. **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`); Capability 3 unchanged; Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR #204 not merged by this task; Part III not begun. See the [Core Business Terms Draft v2.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v2.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v2.1](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part II Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-002-CORR-001-correction-report-2026-08-30.md). **`DEC-LEGAL-002-BT-DRAFT-002-CORR-002` (2026-08-30) — Final Founder wording corrections applied.** On the same branch `docs/dec-legal-002-bt-draft-002` (PR #204, reviewed head `45031bfd`), two narrow wording corrections were applied following Founder re-review: (1) the §2 "Staff" definition's categorical "Staff are not... Authorized Representatives" was corrected so that Staff platform status alone confers neither Business Owner nor Authorized Representative status, while preserving that an individual who is Staff may independently hold legal authority to bind the Business under the Business's own corporate/legal arrangements — never inferred from Staff status alone; (2) §9.6's "to the same extent as if the Business had undertaken those acts itself" was replaced with itemized, proportionate account responsibility (deciding whom to authorize; permissions granted; in-scope Staff activity), with explicit carve-outs for unauthorized access, credential compromise, out-of-scope/malicious activity, and 11thONUS-attributable failures (each conditioned on the Business not having breached its own §9.5/§9.7/§9.8 obligations), and an explicit statement that this is account/access responsibility only, not a Part VI §19 liability regime; (3) §9.3's open-ended "except to the extent these Terms expressly provide otherwise" was removed and replaced with language aligned to Part I §7.5's own governance-required-for-delegation boundary — Part I §7.5 itself read and left byte-for-byte unedited. All four CORR-001 fixes and their review-thread resolutions were re-verified intact both before and after this pass; no new automated or human review finding appeared on PR #204. CI-01/CI-05 remain the only open controlled inputs; no new one created. **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`); Capability 3 unchanged; Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR #204 not merged by this task; Part III not begun. See the [Core Business Terms Draft v2.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v2.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v2.2](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part II Final Correction Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-002-CORR-002-correction-report-2026-08-30.md). (Part III and its correction passes, `DEC-LEGAL-002-BT-DRAFT-003`/`-CORR-001`/`-CORR-002`, are recorded above in this record's "Last controlled update" chain, not restated here.) **`DEC-LEGAL-002-BT-DRAFT-004` (2026-08-31) — Core Business Terms Part IV (Platform Action, Business Exit and Complaints, §§15–17) drafted.** See this record's "Last controlled update" field (top of file) for the full narrative. Summary: §15 (Suspension and Restriction), §16 (Business Exit and Termination; Outstanding Rewards), and §17 (Complaints and Dispute Facilitation) drafted entirely on existing FD-2/FD-3/FD-4, `DEC-LOY-011`, `DEC-LOY-005`, LEG-FD-01/06/07/08/11/12, and governed lifecycle-status authority; no new Founder or legal position required; `DEC-ID-005` explicitly not resolved (§15.7/§16.8). **This does not change `DEC-LEGAL-002`'s Status** (remains `OPEN_LEGAL`); Capability 3 unchanged; Terms configuration unchanged (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` unchanged. No application/source/Firebase/dependency/config change made; docs-only. PR not yet opened/merged by this task; Part V not begun. See the [Core Business Terms Draft v4.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md), [Drafting Traceability Matrix v4.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md), [Controlled Inputs Register v4.0](evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md), and the [Part IV Drafting Report](../../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-004-drafting-report-2026-08-31.md).
