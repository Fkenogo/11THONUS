...
> **Status:** This document (the evidence-gathering deliverable) is complete. **`RES-001` itself, and `EXT-TECH-001`, are not complete.** Per the Resolution Plan's own Ownership Matrix, `RES-001`'s completion criterion is `EXT-TECH-001` status moving `PENDING` → `EVIDENCE_RECEIVED`; the live External Dependencies Register still reads `PENDING` (confirmed: "Delivery is feasible (A-untested)"), and this package's own §7/§10/§11 state the real-SMS carrier delivery test has not been performed. Evidence-gathering only — no governance decision made or recorded, no code implemented, no application code modified, no register status changed.
> **Date:** 2026-07-29
> **Classification:** Technical evidence package. Primary input into `DEC-PROV-004`; supporting input into `DEC-SEC-001`. Per [`ENG-P2-RES-000`](../../../05-implementation/roadmap/ENG-P2-RES-000-capability-2-resolution-plan.md) `RES-001` — **this document alone does not satisfy `RES-001`'s completion criteria; the outstanding delivery test does.**
...
- **External technical evidence** (verified via live web research on 2026-07-29; every claim in §§3–11 is traceable to a specific row below). **Corrected before merge:** the original draft of this section named source domains only, with no URLs, access dates, or per-source reliability notes — insufficient for a reviewer to reproduce this research or judge source quality, and inconsistent with this repository's own established evidence-pack convention (e.g., `DEC-LEGAL-006-DEC-TECH-005-source-register-2026-07-18.md`). The table below corrects this.

### Source Register

All sources accessed 2026-07-29. "Directly fetched" means the page content was retrieved and read via `WebFetch`; "search synthesis" means the claim is drawn from a `WebSearch` results summary across multiple links, not a single page read in full — a lower-confidence category, flagged explicitly per row.

| ID | Title / Page | Publisher | Type | URL | Access method | Claim supported (§) | Reliability note |
|---|---|---|---|---|---|---|---|
| S1 | Firebase Phone Number Verification (overview) | Google / Firebase | Official documentation | https://firebase.google.com/docs/phone-number-verification | Directly fetched | PNV is a distinct, carrier-network product from classic phone sign-in (§3) | Primary, official |
| S2 | Authenticate with Firebase with a Phone Number Using JavaScript | Google / Firebase | Official documentation | https://firebase.google.com/docs/auth/web/phone-auth | Directly fetched | reCAPTCHA requirement, rate limiting, test phone numbers, "SMS not guaranteed" guidance (§3, §5, §6, §7) | Primary, official |
| S3 | Firebase Phone Number Verification — Pricing | Google / Firebase | Official documentation | https://firebase.google.com/docs/phone-number-verification/pricing | Directly fetched | PNV's narrow 8-country list (Finland, France, Germany, Indonesia, Malaysia, Pakistan, Spain, Enterprise) does **not** apply to classic phone sign-in; billing-account requirement for PNV specifically (§3) | Primary, official — **this is the source that must not be misread as classic phone sign-in's country list; see §3/§10 for the conflation risk this page created during research** |
| S4 | Use SMS regions to protect your app from SMS abuse | Google Cloud (Identity Platform) | Official documentation | https://docs.cloud.google.com/identity-platform/docs/admin/sms-regions | Directly fetched | SMS Region Policy allowlist/denylist mechanics, default-deny-all-regions behavior, no African country named either way (§3, §10) | Primary, official |
| S5 | Firebase Authentication Limits | Google / Firebase | Official documentation | https://firebase.google.com/docs/auth/limits | Directly fetched | Classic phone sign-in quotas (900/min, 3,000/day, 50/min & 500/hour per-IP), Blaze-plan requirement, two-week quota-increase lead time (§3, §9, §10) | Primary, official |
| S6 | Firebase Auth pricing/SMS-cost search synthesis (MetaCTO, RapidNative, Logto, B4X forum) | Multiple secondary sources | Search synthesis | (no single URL — aggregated `WebSearch` summary over 4+ secondary blog/forum sources) | Search synthesis | Per-SMS price range cited ($0.01 US/Canada/India up to $0.34–$0.46 highest-cost regions); no Burundi-specific tier found (§9) | Secondary, aggregated — none of the underlying blog posts is a Firebase-official pricing page; the actual Burundi rate remains unverified and can only be established empirically (§9, §10) |
| S7 | Burundi telecom/carrier search synthesis (Wikipedia "Lumitel", Grokipedia "Telephone numbers in Burundi", esimdb.com, thingstodoinburundi.com, OperatorWatch) | Multiple secondary sources | Search synthesis | (no single URL — aggregated `WebSearch` summary over 5+ sources, incl. https://en.wikipedia.org/wiki/Lumitel and https://www.operatorwatch.com/2023/05/little-appetite-for-5g-in-burundi-with.html) | Search synthesis | Three-carrier market (Lumitel, Econet Leo, Onatel); ~97% 2G / ~53% 3G / ~32% 4G population coverage; Lumitel's cited $20M/75%-by-2030 4G investment (§4) | Secondary, aggregated — no single official regulator (ARCT Burundi) or carrier-published coverage-map source was directly fetched in this pass; coverage percentages should be treated as directional, not authoritative, until cross-checked against a primary regulator source |
| S8 | Africa's Talking — Bulk SMS API / eSMS Africa Burundi page (search synthesis) | Africa's Talking; eSMS Africa | Search synthesis (vendor marketing pages) | https://africastalking.com/sms/bulksms ; https://www.esmsafrica.io/burundi/ (fetch attempted, page returned empty/JS-rendered content — **not successfully read**) | Search synthesis; direct fetch of eSMS Africa's Burundi page failed | Africa's Talking explicitly advertises Burundi coverage and OTP as a primary SMS use case (§4, §9, §11) | Secondary, vendor-marketing — this is the weakest-confidence claim in the package supporting `DEC-PROV-004`'s external-route option; the eSMS Africa fetch failure means its carrier-level Burundi claims (if any) are **not** verified here and should be re-attempted before relying on this source further |
| S9 | Twilio Verify — supported countries/deliverability search synthesis | Twilio (via search synthesis, no Twilio page directly fetched) | Search synthesis | (no single URL — `WebSearch` summary referencing https://www.twilio.com/docs/verify/verify-countries-and-regions-deliverability among others) | Search synthesis | Twilio's Burundi support is **not confirmed** by public documentation; would require direct sales contact (§4, §9) | Secondary, aggregated, low confidence — included specifically to disclose an asymmetry (Africa's Talking explicitly claims Burundi; Twilio's status is unconfirmed), not as a claim that Twilio excludes Burundi |
| S10 | Firebase phone-auth SMS delivery-failure search synthesis (Firebase status page, GitHub SDK issues, Firebase Group threads) | Google / Firebase (status page); GitHub (community-filed issues) | Search synthesis over official + community sources | https://status.firebase.google.com (Authentication incident pages referenced); GitHub issues on `firebase/firebase-android-sdk` and `firebase/flutterfire` (specific issue URLs not individually re-verified in this pass) | Search synthesis | SMS delivery is not guaranteed 100% by any operator; documented cases of Firebase's own usage dashboard showing "sent" while users report non-receipt (§5, §8) | Secondary/mixed — the status page itself is primary/official; the specific GitHub issue reports are community-filed and not independently re-verified line-by-line in this pass |
| S11 | Firebase Authentication phone sign-in SMS code expiry search synthesis (FlutterFire docs, Firebase Android docs, `FirebaseAuthSettings` API reference, GitHub issue) | Google / Firebase (official refs); community (Medium post, GitHub issue) | Search synthesis over official + community sources | https://firebase.google.com/docs/auth/android/phone-auth ; https://firebase.google.com/docs/reference/android/com/google/firebase/auth/FirebaseAuthSettings referenced among others | Search synthesis | No documented application-level setting to configure SMS code validity/expiry; `FirebaseAuthSettings` exposes only testing/auto-retrieval controls, not code-validity duration (§5 — corrects this package's own earlier, unverified claim) | Secondary/mixed — official API reference pages support the *absence* of a documented setting; this is evidence of omission, not a definitive confirmation that no such setting exists anywhere in Firebase's private implementation, and is stated with that caveat in §5 |

**Assumptions from insufficient repository/external evidence (explicitly separated from the sources above, per the task's own requirement):** which Burundi carrier(s) Firebase's own SMS delivery infrastructure actually routes through, and whether delivery quality differs by carrier (§4); Burundi's specific per-SMS pricing tier for classic Firebase Auth (§9); and Firebase-to-Burundi delivery success rates generally (§4, §10) — none of these were found in any source, primary or secondary, in this research pass. They are the substance of the real-SMS delivery test recommended in §7/§10/§11.
...
**Timeout considerations (corrected — verified fact, not the assumption originally stated here):** classic Firebase Auth phone sign-in does **not** expose an application-level setting to configure the SMS code's validity/expiry window — this was verified directly and the original draft of this section incorrectly described expiry as "configurable." What platform SDKs *do* expose (e.g., Android's auto-retrieval timeout, `setAppVerificationDisabledForTesting`) governs client-side auto-retrieval and test-verification behavior, not the code's actual validity period on Firebase's backend. `ENG-P2-001` should therefore design its resend/retry UX around a fixed, Firebase-determined expiry rather than assuming a tunable window.
