...
*(From `RES-002`; two factual corrections applied during `RES-002A` pre-merge review, marked inline — engineering conclusions otherwise unchanged.)*
...
- **Approach:** use Firebase Authentication's built-in phone sign-in (SMS OTP), with a Blaze-plan billing account and the SMS Region Policy configured to allow Burundi, plus the application-level phone-auth integration described below.
- **Benefits:** the Firebase Auth *client* is already integrated (`apps/web/src/infrastructure/firebase/auth.ts`, from `ENG-P1-001`), giving this option a real head start over Option A's alternative; single-vendor operational surface (one dashboard, one support channel, one set of quotas); abuse-control primitives (rate limiting, reCAPTCHA, region policy) are Firebase-provided rather than custom-built; up to 10 test phone numbers directly support the Test Strategy `RES-001` §7 already defined.
- **Limitations:** **corrected by pre-merge review** — no first-party confirmation of Burundi delivery reliability exists (the central evidence gap); standard quotas (900/min, 3,000/day) are a hard ceiling without a two-week-notice increase request; no Burundi-specific pricing tier is published, so actual per-SMS cost is unverified until measured (see §6's new Cost row); and, contrary to this section's original text, **real application-level integration work is still required** — a repository-wide search found no `RecaptchaVerifier`, `signInWithPhoneNumber`, confirmation-code handling, or phone-auth UI anywhere in `apps/web` or `functions/src`. The existing Auth client only calls `getAuth`/`connectAuthEmulator`; the actual phone sign-in flow has not been built. This does not change the recommendation (§7) — Option A's integration work is still bounded and SDK-provided, unlike Option B's fully custom OTP service — but "zero additional integration work" understated Option A's true scope and is withdrawn.
...
- **Engineering implications:** **corrected by pre-merge review** — no new *provider* SDK or custom OTP-generation/validation *backend* logic is required (Firebase handles code generation, storage, and expiry), but `ENG-P2-001` must still build the actual sign-in flow against the existing `firebase/auth` client: `RecaptchaVerifier` setup, the `signInWithPhoneNumber` call, and confirmation-code UI/handling — none of which exists in the repository today.
...
*(From `RES-002`; two rows corrected during `RES-002A` pre-merge review — see notes below the table.)*

| Criterion | Option A — Firebase-native | Option B — External (Africa's Talking) |
|---|---|---|
| Engineering complexity | **Corrected by pre-merge review** — Moderate: the OTP-generation/validation *backend* is SDK-provided (no custom service to build), but the application-level sign-in flow (`RecaptchaVerifier`, `signInWithPhoneNumber`, confirmation-code UI) does not exist yet and must still be built; materially smaller than Option B regardless | High — new OTP service, new schema, new integration, new abuse controls to design, *in addition to* the same application-level sign-in flow work Option A also requires |
...
| Cost | **New row, added by pre-merge review** — Unmeasured/unknown. No Burundi-specific Firebase SMS pricing tier is published (`RES-001` §9); actual per-SMS cost is unverified until measured. | **New row, added by pre-merge review** — Unmeasured/unknown. No Africa's Talking Burundi pricing was captured in `RES-001`'s evidence. `RES-001` §9 explicitly deferred this comparison to `DEC-PROV-004` (this document); it remains unresolved here — see §9/§11/§12. |
...
**Reading the table:** both options carry the same central unresolved risk (Burundi delivery reliability, unconfirmed for either) and the same unresolved cost question (neither option's actual Burundi-specific per-SMS cost has been measured). Option A does not introduce any additional *engineering* or *implementation* risk beyond those two shared gaps; Option B adds a materially larger, avoidable implementation-risk layer on top of the same unresolved delivery question, without evidence that this layer would resolve or reduce that question — Africa's Talking's own coverage claim is exactly as unverified by direct testing as Firebase's. Cost is not a basis for preferring either option in this package — it is disclosed as unresolved, not weighed.
...
**Support from evidence:** Option A carries strictly less implementation risk than Option B for an identical, currently-unresolved delivery-reliability question — choosing Option B would not close that evidence gap, since Africa's Talking's Burundi coverage claim is itself unverified by direct testing, the same category of unverified claim `RES-001` found for Firebase. **Corrected by pre-merge review:** Option A's engineering scope is smaller than Option B's, not zero — configuration items (Blaze billing, SMS Region Policy, reCAPTCHA, test numbers) are bounded and SDK-provided per `RES-001`, but the application-level sign-in flow itself (`RecaptchaVerifier` setup, `signInWithPhoneNumber`, confirmation-code handling/UI) still needs to be built, as confirmed by a repository-wide search finding none of it exists yet (§5). This work is required either way `ENG-P2-001` proceeds; it does not change the recommendation, since Option B requires this same work *plus* a fully custom OTP backend. Cost (§6, new Cost row) is not part of this recommendation's basis — both options' actual Burundi cost remains unmeasured and is disclosed as an open item, not weighed here.
...
- **Operational risk: Corrected by pre-merge review** — the SMS Region Policy's default-deny-all-regions behavior (disclosed in `RES-001` §10) is a Firebase-specific mechanism and applies to **Option A only**; it does not apply to Option B, which (per §5) bypasses Firebase's own phone-auth mechanism entirely. Under Option A, Burundi must be explicitly allowlisted in the SMS Region Policy, or all customer registration attempts would silently fail. Option B would carry its own, distinct operational risk instead — misconfiguration of Africa's Talking's own sender-ID/coverage/route settings — which this package has not evaluated in the same depth, since Option A is the recommended option (§7).
- **Cost risk (new, per §6):** neither option's actual Burundi-specific per-SMS cost has been measured; `RES-001` §9 explicitly deferred this comparison to this document, and it remains unresolved here (§6, §12). This is a disclosed gap, not a basis for reconsidering the recommendation, since cost was never part of §7's reasoning.
...
**Founder Decision: Approve with Conditions (received 2026-07-30).** This supersedes `RES-002`'s original "Ready with Conditions" framing, which described readiness *for Founder consideration* — that consideration has now occurred and produced an affirmative decision. The evidence in §4–§7 supported that decision: the comparative evaluation (§6) is evidence-grounded and did not depend on delivery-test data, or on cost data, to identify Option A as preferable, which the Founder's own approval confirms. What remains open is **not** whether the decision was made, but **three distinct follow-on items**: (1) the launch-readiness validation §9 describes, gating production activation, not the decision; (2) the governance-prerequisite action (§3, §11) gating formal *recording* of the decision in the Decision Register, a repository-mechanical step distinct from the Founder's own product-level approval; and (3) **corrected by pre-merge review** — the cost comparison between Option A and Option B, which `RES-001` §9 explicitly deferred to this document and which this document has disclosed (§6, §11) but not resolved. Cost was not part of the evidentiary basis for the recommendation (§7) or the Founder's decision, so it does not affect readiness, but it remains a named open item for a future task to close.
