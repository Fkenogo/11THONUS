...
> **Status:** Analysis only. **No application code, PRD, TRD, Decision Register entry, or other governing document was modified by this task.** This document is read-only research output preparing the Founder's countersign decision (see the companion [Founder Decision Package](FD-IDENTITY-001-founder-decision-package-2026-08-01.md)) and the eventual Capability 2 redesign it authorizes.
...
**Bottom line:** `FD-IDENTITY-001` requires amending, not superseding, `DEC-PROV-004` and `DEC-SEC-001` (§4), does not touch `DEC-ID-003`/`DEC-DATA-007`/`DEC-ID-001` in substance (§4), and requires a new capability — internally named **Trust Lifecycle Management (TLM)**, per the Founder's own follow-on naming recommendation — inserted into the Capability Delivery Roadmap between the redefined Capability 2 (Identity) and the new Capability "Authentication" (§5/§6). No engineering work package has been implemented against the pre-`FD-IDENTITY-001` model, so the migration cost is entirely a **documentation and decision-register correction cost**, not a rework cost.
...
§4's Canonical Terminology table has no entry for "Authentication," "Verification," or "Trust Level" — if `FD-IDENTITY-001`/TLM introduces new customer- or product-facing vocabulary, new terminology rows are needed (a later, product-copy-stage task, not this one).
...
- **§12.55 Functional Requirements (FR-SEC-001–018)**: the formal, RTM-traced requirement set. None of these requirements, read individually, mandate phone verification before registration — the mandatory-verification requirement lives in PRD2, not here. No FR-SEC-* text change is strictly required by `FD-IDENTITY-001`, though FR-SEC-016 ("Customer profile and KYC information shall be exposed only according to purpose and consent") is a natural anchor point for TLM's future risk-based-disclosure requirements.

### 3.8 TRD10 — `docs/02-technical/trd/10-firestore-data-architecture.md`

**§10.6.1 `users` collection**: `status: "pending" | "active" | "locked" | "suspended" | "closed" | "archived"` — **no `phoneVerified`, `emailVerified`, or trust-level field exists anywhere in this schema.** This is a structural gap, not a contradiction: Progressive Trust/TLM (verified phone, verified email, account age, purchase history, device history) currently has nowhere to persist. Schema design is future implementation work, not something this task should specify (see §6, out of scope).
...
`FD-IDENTITY-001`/TLM communications (Founder-facing, and later customer-facing product copy) should disambiguate these explicitly — recommended as a documentation task at rollout, not a repository defect to fix now.
...
- Point (7) *"Identity trust shall follow a progressive model: Anonymous; Authenticated; Verified"* — this 3-tier model should be explicitly reframed as an early, coarser precursor to the full Trust Lifecycle Management model (continuous multi-signal trust: verified phone, verified email, account age, purchase history, device history, merchant history — `FD-IDENTITY-001` Principle 4), not discarded, since "Authenticated" and "Verified" remain meaningful trust states within TLM's richer model.
...
- A new TLM work package (suggested `ENG-P2-00Y` or a new Phase, pending Programme-owner judgment) is sequenced *after* Identity and Authentication both exist, since TLM consumes signals (verified phone, verified email, account age, purchase history) that only exist once those two capabilities are live.
...
4. **Schema gap has no owner yet.** TRD10's `users` collection has no `phoneVerified`/trust-level field (§3.8) — until a future implementation task designs this, TLM has no data model. Not a blocker to Founder-approving `FD-IDENTITY-001` itself, but a prerequisite before `ENG-P2-001`(narrowed)/Authentication/TLM implementation can begin.
...
7. **Schema design task** (TRD10 `users`/`customerProfiles` trust-signal fields) — sequenced after step 4 confirms TLM's exact scope, since schema design should follow capability definition, not precede it.
8. **`ENG-P2-001`(narrowed)/Authentication/TLM implementation** — begins only once steps 1–7 are complete and the (re-scoped) Capability Authorisation Gate is satisfied. This is the point at which "engineering resumes," per the task brief's own framing — explicitly out of scope for this task.
