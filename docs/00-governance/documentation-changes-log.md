...
- **P-2 (Hosting CSP):** in both `firebase.json` blocks, `connect-src` gains `https://europe-west1-eleventh-on-us-dev.cloudfunctions.net` (the `authenticate` callable) and `frame-src` gains `https://eleventh-on-us-dev.firebaseapp.com` (Firebase Auth's `signInWithPopup` resolver iframe — added after a Codex P1 review finding). No wildcard; reCAPTCHA/Google + Identity Toolkit origins and restrictive directives preserved; regression test added/extended.
