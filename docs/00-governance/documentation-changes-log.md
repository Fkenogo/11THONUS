...
## Entry 127 — `DEC-LEGAL-002-BT-DRAFT-001-CORR-001`: Core Business Terms Part I Founder Corrections

- **Date:** 30 August 2026
- **Performed by:** Claude (AI agent), per Founder task `DEC-LEGAL-002-BT-DRAFT-001-CORR-001` — correcting PR #203 (`docs/dec-legal-002-bt-draft-001`) on the Founder's APPROVE WITH CORRECTIONS disposition.
- **Entry verification:** worked directly on the existing branch `docs/dec-legal-002-bt-draft-001` (no new branch), fetched and confirmed in sync with `origin` before editing (head `33f6449`); PR #203 confirmed `OPEN`/`MERGEABLE`. Pre-existing untracked working-tree files left entirely untouched.
- **Corrections applied to Part I of the Core Business Terms draft:** (1) §1.3 — removed the categorical "not required to independently verify corporate authority" statement, preserving the authority representation while permitting 11thONUS to request reasonable evidence where necessary for verification/security/compliance/dispute-resolution/platform-integrity; (2) §3.2(c)/§6.3 — removed "if and when established"/undecided-instrument phrasing; Customer Terms / Platform Terms of Use is now stated as an approved separate legal-instrument architecture and future controlled work package under LEG-FD-10, not a Capability 3 blocker; (3) §7.1 — removed the universal predetermination that acceptance is a condition of "continued participation on the platform," leaving only the governed certainty that acceptance precedes submission for verification, with reacceptance-on-change reserved to §22; (4) §7.6 — removed the implied Terms-acceptance retention standard, replaced with an auditable-record requirement subject to applicable law and governed data-retention policy; (5) the §2 "11thONUS"/"the platform" definition — replaced the narrow technical infrastructure list with the governed product category "customer-verified loyalty platform," detailed infrastructure kept in §4; (6) §1.2 — resolved the business-entity-scope controlled input (Terms bind only the registered entity/sole proprietor, no automatic affiliate/franchise party); (7) §3.3 — resolved the jurisdiction-architecture controlled input per the already-governed Layer-1/Layer-2 (portable Core Terms + jurisdictional overlay) model; (8) §4.4 — removed the service-tier-differentiation placeholder from Part I in full, no tier invented; (9) §7.5 — resolved the delegated-acceptance controlled input conservatively (registering owner/authorized representative only; ordinary staff permissions do not confer acceptance authority; any future delegation requires explicit governance); (10) §7.4 reviewed and confirmed already-correctly non-resolving on the reacceptance-implementation boundary, left unchanged; (11) readiness-count correction — independently re-verified the Terms Drafting Readiness Note's §3 table (17 distinct rows, all marked Ready, by direct line count) and corrected the live enumeration in the draft/Traceability Matrix/Controlled Inputs Register from "16/16" (or the v1.0 draft's unresolved "sixteen/seventeen" framing) to **17/17**, recorded explicitly as a non-substantive counting/labelling correction — no section added, no readiness decision changed. The Readiness Note itself (2026-08-29, historical evidence) was left unedited.
- **Controlled Inputs Register updated:** CI-02 (business-entity scope), CI-03 (jurisdiction architecture), CI-04 (service-tier differentiation), and CI-06 (delegated-acceptance authority) resolved and moved to a "Resolved this correction pass" section; CI-01 (operator legal identity) and CI-05 (reacceptance-on-change engineering decision) remain the only two open controlled inputs in Part I.
- **Validation:** an actual `grep`-based re-search confirmed zero accidental resolution of any `DEC-SUB-*` item, zero privacy/retention decision resolved, no staff-acceptance overreach, no remaining hypothetical Customer Terms wording, no remaining universal continued-participation language, no narrow technical platform redefinition remaining, and no adoption of any previously-rejected external-legal-opinion recommendation. See the [Correction Report](../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-001-CORR-001-correction-report-2026-08-30.md) §17 for the full grep output.
- **Status unchanged:** `DEC-LEGAL-002` (`OPEN_LEGAL` — Notes-field addendum only); `EXT-LEG-002` (`EVIDENCE_RECEIVED`); Capability 3 (`Open — engineering work packages complete; blocked on governed Terms-content configuration`, `CDR-001` §5, unmodified); Terms configuration (`NOT CONFIGURED`); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` items (all unchanged).
- **Files:** the four BT-DRAFT-001 deliverables corrected in place; the original Drafting Report given a version-header pointer and one risk-bullet resolution note only (its substantive v1.0 narrative left intact as historical record); one new correction-report file created; this log entry; a further Notes-field addendum to `DEC-LEGAL-002` in `docs/00-governance/decisions/decision-register.md` (append-only). No `apps/`, `functions/`, Firestore/Storage Rules, Firebase configuration, or dependency file touched. No pre-existing untracked working-tree file touched.
- **Rollback:** all changes confined to the still-unmerged branch `docs/dec-legal-002-bt-draft-001` (PR #203); revert the correction commit or restore the six modified files individually and delete the new report file to fully roll back — `origin/main` is untouched.
- **Report:** [DEC-LEGAL-002-BT-DRAFT-001-CORR-001 Correction Report](../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-001-CORR-001-correction-report-2026-08-30.md).

**Final gate: CORE BUSINESS TERMS PART I FOUNDER CORRECTIONS COMPLETE — PR #203 AWAITS RE-REVIEW.**

---

## Entry 126 — `DEC-LEGAL-002-BT-DRAFT-001`: Core Business Terms Instrument Architecture & Part I Draft

- **Date:** 30 August 2026
- **Performed by:** Claude (AI agent), per Founder task `DEC-LEGAL-002-BT-DRAFT-001` — "Core Business Terms — Instrument Architecture and Part I Draft."
- **Entry verification:** repository confirmed on branch `docs/dec-legal-002-founder-disp-001` at start, already fully merged into `origin/main` via PR #202 (merge commit `5c2d1df`); no locks, no in-progress git operations; pre-existing untracked working-tree files identified and left untouched throughout. New branch `docs/dec-legal-002-bt-draft-001` created directly from `origin/main` `5c2d1df` and checked out.
- **Authorities inspected in full:** the Terms Instrument Architecture & Drafting Readiness Note (v2.0), the Terms Content Architecture (v3.0), the Founder Legal Architecture Disposition Record (LEG-FD-01–15, v2.0), the Legal Counsel Handoff Pack (FD-1–FD-7, v2.0), the Post-Legal-Review Resolution Assessment (v2.0), the Decision Register's `DEC-LEGAL-002`/`DEC-LOY-011` entries, and this log's own naming/entry conventions. No conflict was found among these authorities — LEG-FD-14/15 explicitly resolve, without contradicting, the two items the earlier disposition record had left open.
- **Discrepancy disclosed, not corrected:** the readiness note's narrative states "16 of 16 sections ready to draft," but its own §3 table (verified by direct row count) lists seventeen rows, each independently marked Ready. Recorded as an observed count/label artifact in a read-only historical evidence document, not corrected in that document, and not treated as a contradiction of substance.
- **Deliverables:** (1) [Core Business Terms — Draft](decisions/evidence/DEC-LEGAL-002-BT-DRAFT-001-core-business-terms-draft-2026-08-30.md) — Part 0 complete proposed architecture (27 headings, 8 Parts, full readiness-row mapping) + Part I full clause text (§§1–7: Parties and Agreement, Definitions, Purpose and Scope, Platform Role, Independent Business Relationship, Business/Customer Relationship Boundary, Acceptance and Formation); (2) [Drafting Traceability Matrix](decisions/evidence/DEC-LEGAL-002-BT-DRAFT-001-drafting-traceability-matrix-2026-08-30.md) — every Part I clause traced to a specific governing authority or flagged as a controlled input; (3) [Controlled Inputs Register](decisions/evidence/DEC-LEGAL-002-BT-DRAFT-001-controlled-inputs-register-2026-08-30.md) — six catalogued placeholders (CI-01 through CI-06), each classified by what must resolve it and by whom; (4) [Drafting Report](../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-001-drafting-report-2026-08-30.md) — full 37-point report including an actual `grep`-based prohibited-concept search (result: zero adoptions of any of the task's prohibited concepts — every hit was a correctly-worded declination, a flagged controlled input, or an unchanged-status confirmation).
- **Drafting boundaries observed:** no universal 30/60/7/14/24/48-day-or-hour period, no $25 or other arbitrary customer liability cap, no Kirundi-as-application-language statement, no forced-scrolling requirement, no "rewards have no monetary/economic value" phrasing, no `DEC-SUB-*` value/plan name/price/billing interval/trial/grace-period/proration/tier mechanic, no gift-card/stored-value rule, and no resolution of `DEC-ID-005`/`DEC-LOY-009` appears anywhere in the drafted text — confirmed by the grep search in the Drafting Report §19, not merely asserted.
- **Status unchanged:** `DEC-LEGAL-002` (`OPEN_LEGAL` — Notes-field addendum only, no Status change); `EXT-LEG-002` (`EVIDENCE_RECEIVED`); Capability 3 (`Open — engineering work packages complete; blocked on governed Terms-content configuration`, `CDR-001` §5, unmodified); Terms configuration (`NOT CONFIGURED`, `platformConfig/businessTerms` neither read nor written); `DEC-ID-005`/`DEC-LOY-009`/all unresolved `DEC-SUB-*` items (all unchanged).
- **Files:** four new evidence/report files (listed above); this log entry; a Notes-field addendum to `DEC-LEGAL-002` in `docs/00-governance/decisions/decision-register.md` (append-only, no Status/field-value change). No `apps/`, `functions/`, Firestore/Storage Rules, Firebase configuration, or dependency file touched. No pre-existing untracked working-tree file touched.
- **Rollback:** all changes confined to the unmerged branch `docs/dec-legal-002-bt-draft-001`; delete the branch and close its PR without merging to fully roll back — `origin/main` is untouched.
- **Report:** [DEC-LEGAL-002-BT-DRAFT-001 Drafting Report](../05-implementation/reports/DEC-LEGAL-002-BT-DRAFT-001-drafting-report-2026-08-30.md).

**Final gate: CORE BUSINESS TERMS STRUCTURE ESTABLISHED — PART I DRAFTED — AWAITING FOUNDER REVIEW; NOT APPROVED / NOT EFFECTIVE / NOT CONFIGURED.**

---

## Entry 125 — MTAIP-001 11thONUS Alignment Closure

- **Date:** 28 August 2026
- **Performed by:** Claude (AI agent), per Founder task "MTAIP-001 11thONUS ALIGNMENT CLOSURE." Closes the MTAIP-001 alignment for 11thONUS on the basis of the already-completed [MTAIP-001 Product Alignment Assessment](../05-implementation/reports/mtaip-001-product-alignment-assessment-11thonus-2026-08-28.md) and explicit Founder dispositions in the task brief. Governance-alignment closure only — does not reopen the infrastructure assessment, does not conduct a wider architecture reconciliation, does not implement any infrastructure change.
- **Entry verification:** repository HEAD confirmed at `99f840f` (branch `docs/eng-p3-002-ui-governance-chain-sync`, one merge behind `origin/main` `cf6867b`, upstream gone but no locks/incomplete merges); the completed assessment, decision register, `DEC-TECH-005`, `DEC-TECH-010`, `DEC-PROV-006` all located; no existing Infrastructure Disposition or equivalent record found (confirmed by repository-wide search) — created fresh, not a duplicate.
- **Classification:** recorded **F — Firebase-native** as the sole controlled 11thONUS infrastructure classification, per Founder disposition. Future loyalty/commerce/rewards domains are **not** given a secondary "E — Infrastructure undecided / experimental" classification; they are recorded as unresolved future infrastructure characteristics within an F-classified product, reconsiderable only if a future capability demonstrates a material non-Firebase requirement.
- **`DEC-TECH-005` traceability correction:** narrow verification performed (not a re-investigation) — confirmed the region is fixed in code (`functions/src/config/region.ts`, `apps/web/src/infrastructure/firebase/functions.ts`, both committed in `ba43da1`, an ancestor of `origin/main`) and the `dev`/`staging` Firebase projects (`.firebaserc`) use it. Also discovered, via `git log -p` on the register file, that this confirmation was never actually written to `decision-register.md` — despite a permanent, Founder-signed, committed [Version 1.0 Engineering Authorization Record](../version-1-engineering-authorization-record.md) (2026-07-19) recording the Founder's exact confirmed wording for both `DEC-TECH-005` and `DEC-LEGAL-006`, and despite that same record's own §14 asserting the register already reflected it. Multiple later tasks (e.g. the `DEC-PROV-005` decision-recording report, 2026-07-26) independently rediscovered this exact gap, correctly left it untouched as outside their own scope, and recommended a dedicated follow-up audit — this task is that follow-up, for `DEC-TECH-005` only. Corrected the register entry's Status/Final decision/Decision date/Approved by fields to `CONFIRMED`, quoting the Founder's own already-approved wording verbatim (no new interpretation), and updated the §5 Register Summary counts (CONFIRMED 44→45, OPEN_ENGINEERING 13→12). **`DEC-LEGAL-006` carries the identical gap and was deliberately left untouched** — outside this task's authorization; flagged for the Founder as a separate, dedicated follow-up.
- **Infrastructure Disposition:** created [`11thonus-infrastructure-disposition-v1.md`](../11thonus-infrastructure-disposition-v1.md) (`docs/00-governance/`, following the existing freeze-declaration document convention), covering classification, authoritative data responsibilities, authentication/identity, hosting, compute, background processing, storage, realtime/event architecture, external integrations, backup/recovery status, cost drivers, provider dependencies, portability risk, retention rationale, unresolved future characteristics, and reconsideration triggers — concise and decision-oriented, not a repeat of the full assessment.
- **Backup/recovery:** `DEC-TECH-010` and `DEC-PROV-006` left open in the Decision Register (no change) — recorded in the Infrastructure Disposition as a bounded follow-on item, not a blocker to this closure, that must be resolved before meaningful production business data accumulates. No solution designed or implemented.
- **Outbox live-trigger wiring:** recorded as architecturally valid and implemented/tested to the extent the accepted assessment established, but not connected to a live production trigger. Not implemented by this task; remains with whichever future engineering capability first requires production event processing. Not an MTAIP-001 non-conformity.
- **MTAIP-001 assessment document:** corrected in place with the smallest edit possible — a preface note recording Founder acceptance and the classification correction — rather than rewriting §18/§19's original text, which is preserved unchanged as the evidence record the Founder reviewed.
- **Section 22 review:** all five matters from the assessment's §22 resolved consistently with the Founder dispositions above as routine closure (governance clarification, traceability, non-blocking follow-on) — none required a new architecture decision, provider migration, material infrastructure change, or contradiction of the F-Firebase-native classification, so none was escalated.
- **Files:** `docs/00-governance/decisions/decision-register.md` (`DEC-TECH-005` entry + §5 summary + header); `docs/00-governance/11thonus-infrastructure-disposition-v1.md` (new); `docs/05-implementation/reports/mtaip-001-product-alignment-assessment-11thonus-2026-08-28.md` (preface note only); this entry. No runtime code, Firebase configuration, or dependency changed.
- **Rollback:** all changes are uncommitted in the working tree at the time of this entry — to roll back, revert the four files above to their pre-task content (each edit is additive/in-place text only; no file was deleted).
- **Report:** [MTAIP-001 Alignment Closure Report](../../05-implementation/reports/mtaip-001-alignment-closure-report-2026-08-28.md).

**Final gate: MTAIP-001 ALIGNMENT FOR 11thONUS CLOSED AT GOVERNANCE LEVEL — CLASSIFIED F — FIREBASE-NATIVE. NO APPLICATION INFRASTRUCTURE, PROVIDER, RUNTIME, OR CONFIGURATION CHANGE WAS MADE.**

---

## Entry 124 — `CAP-P2-G2-001`: G2 Evidence Ratification, Final Closure & Handover

- **Date:** 17 August 2026
- **Performed by:** Claude (AI agent), per Founder task "CAP-P2-G2-001 — Capability 2 G2 Evidence Ratification, Final Closure & Handover." A fresh, explicit Founder authorization to execute the Capability-2 G2 closure gate, confirming that existing valid evidence may satisfy deployment/Preview Review/Manual QA where the relevant customer-facing surface is already deployed and manually validated, the evidence remains applicable to the current implementation, a domain-layer component has no governed production endpoint or independent customer-facing surface, and deploying a synthetic/fake surface would distort the approved architecture. Not a waiver of G2 — the review, readiness determination, and closure decision were still performed. Does not authorize Capability 3, `AUTH-10`, or any new deployment for the sole purpose of generating evidence.
- **Entry verification:** `origin/main` fetched and confirmed at `7c2fab788e5ba96cb542a81e1b9d9e0ffa27246a`. All four Capability-2 concerns reconfirmed `Complete` on `main`; their closure PRs confirmed merged ancestors; post-merge CI confirmed green on the last five merges (`gh run list --branch main`). Capability 2 confirmed not already `Complete`; Capability 3 confirmed `Not started`. No conflicting G2/closure task, branch, or PR found (only unrelated PR #34 open). Two historically related, already-merged branches (`RES-007`/`RES-007B`, PRs #40/#41, merged 2026-07-31) were checked and confirmed superseded — their "Ready with Conditions" disposition predates Authentication/ITM/`ENG-P2-004`, and all four items their own gate cited (`EXT-TECH-001`, `DEC-PROD-012`, BaseMetadata conformance, Phase 1 exit) are independently confirmed resolved on `main` today. Dirty primary worktree confirmed untouched throughout — all verification performed against a detached-HEAD worktree at `origin/main` `7c2fab7`.
- **Authentication/Customer Identity evidence-currency check:** reconstructed the `AUTH-HOSTED-PREVIEW-002` hosted-preview evidence (Email/Password registration + returning sign-in, Google sign-in, Customer Identity creation/reference resolution, duplicate prevention, `AUTH-08` events, EN/FR, Confirm-Password, no credential persistence) and confirmed **zero diff** in `functions/src/domains/authentication/**` or `apps/web/src/authentication/**`/`functions/src/domains/identity/**` since 2026-08-14 through `main` `7c2fab7` — the only intervening work (`ENG-P2-004B/C/D`, `ITM-DESIGN-001`, `ITM-A/B/C/D`) is confirmed scoped to `domains/permissions/**`/`domains/trust/**` only. **Evidence ratified as current; no repeat hosted validation performed.**
- **`ENG-P2-004`/ITM G2 disposition:** independently re-verified (fresh repository-wide grep, not trusting prior reports) that `ENG-P2-004`'s `authorizeAndExecute.ts` and ITM's `domains/trust/**` are library/domain-only, with zero Cloud Function anywhere importing `domains/trust` and `ENG-P2-004` invoked only via governed internal test fixtures — no governed Capability-3 production consumer exists for either. **No synthetic production endpoint was built for either package** — `ENG-P2-004` disposed **PASS (fixture/security validation sufficient for current capability boundary)**; ITM disposed **N/A (no independently deployable surface) / DEFERRED BY DESIGN (production-consumer validation belongs to a later consumer)**.
- **Firebase read-only verification (`eleventh-on-us-dev`):** `authenticate` callable confirmed present and healthy (`europe-west1`, `v2`, `nodejs20`). `firebase hosting:channel:list` returned only the `live` channel — `auth-preview-002` no longer present, **resolving the PRE-G2 assessment's teardown fact-gap as PASS**. No Firebase action taken (nothing required deleting); no unexpected resource drift found.
- **Manual QA / Preview Review ratification:** every existing Authentication manual test (Google, Email/Password registration and returning sign-in, Confirm-Password, EN/FR surface, backend identity/reference integrity) checked against the currency finding above and **ratified/reused — no new Founder Manual QA required**, no evidence-invalidating change found. The existing Authentication preview review disposed **PASS / existing evidence reused**; no combined preview UI was built to manufacture an artifact (Customer Identity/`ENG-P2-004`/ITM have no independent customer-facing surface).
- **Automated capability-level sanity validation:** run on a clean, detached-HEAD worktree at `origin/main` `7c2fab7` (dirty primary worktree untouched). `pnpm install --frozen-lockfile` clean; `lint`/`format:check`/`typecheck`/`build` all clean; unit tests **functions 943/943, web 397/397**; `firebase emulators:exec … test:emulator` **338/339** on first run — one `idempotencyService.emulator.test.ts` dual-caller concurrency-timing test hit its fixed 5000ms timeout, the same disclosed flake class as prior closure reports (e.g., `ITM-D`'s own 336→339 pattern) — **339/339 clean** on immediate rerun with no code change, classified environmental, not a regression. GitHub Actions CI independently confirmed green on `main`'s last five merges including PR #118 (run `32007350241`, success). **No persistent regression found.**
- **Known observations:** Phone OTP live-SMS readiness (separate `EXT-TECH-001` track, non-blocking), the disclosed non-sensitive-permission ALLOW gap (deferred, Founder Decision FD-5), ITM `reasonReferences`/`verificationState` (governed, non-blocking), the `parseEventType` naming discrepancy (pre-existing, separately disclosed, non-blocking), retained Firebase test users (explicitly authorized to remain) — all reviewed and none promoted to a blocker; none silently dropped.
- **Final G2 matrix:** every required criterion resolved to PASS, N/A, or correctly DEFERRED BY DESIGN; zero criterion left unresolved; zero blocker. **G2 = PASS.**
- **Capability-2 closure:** with G2 = PASS, **Capability 2 is now recorded `Complete`.** Closure records synchronized (docs-only): `CDR-001-capability-delivery-roadmap.md` (§2 table row, §5 Capability 2 subsection, header dated append), `engineering-implementation-programme.md` (header dated append, P2 status note), this entry, `IMPLEMENTATION_CHANGES.md`, and a new persistent closure report. No historical report or entry was rewritten — all prior text preserved, superseded only by dated notes, per established convention.
- **Capability 3 handover:** Capability 2's closure is confirmed the governed prerequisite before Capability 3 authorization; that prerequisite is now satisfied. **Capability 3 itself remains `Not started`** — no architecture or code was begun; a dedicated, separately authorized Founder task is required to begin it, starting from `origin/main` at this task's own merge commit.
- **Files:** `CDR-001-capability-delivery-roadmap.md`, `engineering-implementation-programme.md`, this entry, `IMPLEMENTATION_CHANGES.md`, and the new [`CAP-P2-G2-001` closure report](../05-implementation/reports/CAP-P2-G2-001-g2-evidence-ratification-and-capability-2-closure-2026-08-17.md). No runtime code, Firebase configuration, or dependency changed.
- **Rollback:** `git revert` the merge commit recorded in the closure report — the entire change set is documentation-only and purely additive; no schema, no deployed resource, no data, no Firebase state changed by this task.
- **Report:** [`CAP-P2-G2-001` closure report](../05-implementation/reports/CAP-P2-G2-001-g2-evidence-ratification-and-capability-2-closure-2026-08-17.md).

**Final gate: CAPABILITY 2 G2 PASSED — CAPABILITY 2 MERGED AND CLOSED; CAPABILITY 3 AWAITS FRESH FOUNDER AUTHORIZATION.**

---

## Entry 123 — `CAP-P2-ITM-D`: Independent Final Risk-Gate Review, Merge & ITM Closure

- **Date:** 17 August 2026
- **Performed by:** Claude (AI agent), per Founder task "CAP-P2-ITM-D — Independent Final Risk-Gate Review, Merge & ITM Closure." Authorizes independent final review of `ITM-D` and, if all gates pass, merge/closure of PR #117 and ITM concern-level closure assessment. Does not authorize G2, Capability 2 closure, Capability 3, Reward Engine, new risk-gated actions, new permissions, new trust signals, operator visibility, trust regression, or `AUTH-10`.
- **Entry verification:** PR #117 confirmed `OPEN`, head `7fb9bf2766d6bf6f88d78090ad020ba320213651` (exact expected head, zero later commits at review start), CI `SUCCESS` (3m42s), `MERGEABLE`/`CLEAN`, 2 commits. `origin/main` confirmed unchanged at `31a176a8` since the PR was cut. `ITM-A`/`ITM-B`/`ITM-C` confirmed merged and byte-identical (zero diff scoped to their own files). No `ITM-D` work found elsewhere. Only PR #34 (unrelated) open. Dirty primary worktree confirmed untouched.
- **Requirement-vocabulary authority (independent, not trusting the prior report):** re-read `ITM-DESIGN-001` §9.1/§9.2 directly. Confirmed the closed `TRUST_UNVERIFIED_OR_ABOVE`/`TRUST_PROVISIONAL_OR_ABOVE`/`TRUST_ESTABLISHED_OR_ABOVE` vocabulary is explicitly authorized ("the requirement vocabulary is ITM-owned and closed"), maps 1:1 onto the already Founder-countersigned `unverified`/`provisional`/`established` bands (`AD-ITM-1`, no new band/threshold/evidence category), cannot be bypassed by a caller-invented numeric threshold (`isRiskRequirementId` gates every input), and answers only "what degree of trust," never "which product action." No Founder decision required.
- **Product-policy separation, requirement truth table, ENG-P2-004/Authentication boundaries:** independently re-verified by direct code inspection and repository-wide grep — zero invented action identifier/threshold/fraud category anywhere in the diff; `evaluateRiskGate`'s comparison matches the nine-row truth table exactly (non-strict `>=` via ITM-A's `isAtLeastTrustLevel`); zero `domains/permissions`/`domains/authentication` import anywhere in the ITM-D diff (not merely `riskGate/` — the wider `services/`/`repositories/` layer independently checked too).
- **`unavailable` fail-closed (primary security gate):** traced every call seam — the one production call site (`touchRiskGateBoundaryFixtureCommand.ts`) uses an allow-list check (`if (decision.decision !== "sufficient")`), so `unavailable`/`insufficient`/any future value are all denied by construction; `checkRiskGateService.ts`'s catch-all for an unexpected error shape also fails closed to `unavailable` rather than defaulting to `sufficient`. Representing `unavailable` as a decision value (not a thrown failure) matches `ITM-DESIGN-001` §13's own explicit design choice — no redesign warranted.
- **One genuine, narrow coverage gap found and fixed — not a defect:** `evaluateRiskGate` only ever reads `effectiveTrustLevel`/`ruleVersion` (never `.basis`), so provider type, authentication-event count, and a stale persisted `trustLevel` cache are structurally incapable of influencing a decision — but no test previously exercised these three scenarios directly through `checkRiskGate` itself (only inherited, indirectly, from ITM-C's own separate suite). Added three new integration tests (`checkRiskGateService.emulator.test.ts`, committed `c31318fbb84a930f4321cd01dee613b6c8c16b91`) proving each scenario directly — all passed against the real Firebase Emulator Suite. No implementation code changed.
- **Standard-participation protection, fixture isolation, ITM-C sole authority, result/explainability contract, rule-version consistency, pure-evaluator purity, error taxonomy:** all independently re-verified by direct code inspection and fresh re-runs of the existing boundary/domain tests — no material finding on any of these fronts.
- **Review tooling:** `gh pr view 117 --json reviews,reviewRequests` returned empty for both; no automated Codex/external review bot configured on this repository (disclosed); this independent review served as the merge gate.
- **Full validation re-run:** functions **943/943**; web **397/397**; `emulators:validate` **336/336** at the original submission head, then **339/339** after this review's three additional tests (confirmed via the PR's own CI run on final head `c31318f`, `SUCCESS`, 3m48s); typecheck/lint/format/build clean; secret/PII scan — none found.
- **Merge:** with all gates clean, PR #117 merged via `gh pr merge 117 --merge` as `54c93e9fbe13772a0689a24048699b83c5dbcd37`. Post-merge: `origin/main` confirmed at `54c93e9`; `c31318f` confirmed an ancestor; post-merge CI (`main`, run `32006357967`) confirmed `SUCCESS` (3m41s).
- **ITM acceptance matrix / closure assessment:** all sixteen `ITM-DESIGN-001` §15 per-package acceptance-criteria rows independently reconstructed and classified PASS or N/A — zero FAIL, zero unresolved DEFERRED-BY-DESIGN, zero NOT-CURRENTLY-GOVERNED gap. All four `AD-ITM-1`–`AD-ITM-4` dispositions re-confirmed respected. No outstanding Founder ITM decision (`FDR-1`–`FDR-4` all resolved, §22). **ITM concern-level status: `Complete`.**
- **Status change:** `ITM-A` = Complete/merged. `ITM-B` = Complete/merged. `ITM-C` = Complete/merged. `ITM-D` = **Complete/merged**. **ITM overall = Complete.** Per this task's own explicit instruction, Capability 2 is **not** marked `Complete` — it moves to `Open — concerns complete, capability-level closure gates pending`; the next governed activity is a PRE-G2/Capability-2 closure-readiness assessment, **not executed by this task**. Capability 3 remains `Not started`. G2 remains `Not started` — **not begun**.
- **Files:** `CDR-001-capability-delivery-roadmap.md` (§2/§5 ITM lines + header dated append), `engineering-implementation-programme.md` (header dated append), this entry, `IMPLEMENTATION_CHANGES.md`, and a closure section (§54) appended to the `CAP-P2-ITM-D` implementation report, plus the three-test addition committed to PR #117 itself before merge.
...
- **Report:** [`CAP-P2-ITM-D` report](../05-implementation/reports/CAP-P2-ITM-D-risk-gate-contract-and-itm-closure-2026-08-17.md) (§54 closure section appended in place).

**Final gate: ITM-D MERGED AND CLOSED — ITM CONCERN COMPLETE; CAPABILITY 2 PRE-G2 ASSESSMENT REQUIRED.**

---

## Entry 122 — `CAP-P2-ITM-D`: Risk-Gate Contract, Integration Validation & ITM Closure

- **Date:** 17 August 2026
- **Performed by:** Claude (AI agent), per Founder task "CAP-P2-ITM-D — Risk-Gate Contract, Integration Validation & ITM Closure." Authorizes `ITM-D` implementation only. Does not authorize Reward Engine, Capability 3, new high-risk product actions, new permissions, new trust signals, trust regression, fraud scoring, operator visibility, customer-facing trust UI, G2/Release Readiness execution, or `AUTH-10`.
- **Entry verification:** `origin/main` at `31a176a8ccbaacb9a0c154a9025ef0ecce1758fc`, matching the task's expected baseline exactly. `ITM-A` (PR #111 merged `eea8726`, PR #112 closure-sync merged), `ITM-B` (PR #113 merged `7fc7a77`, PR #114 closure-sync merged), `ITM-C` (PR #115 merged `e090d86`, PR #116 closure-sync merged) all confirmed merged with post-merge CI green (`gh run list --branch main`). No `ITM-D` branch, worktree, or PR found anywhere (`git branch -a`, `gh pr list`). Only PR #34 open (unrelated). Capability 3 and G2 confirmed `Not started`. Dirty primary worktree confirmed untouched — a clean linked worktree (`.claude/worktrees/itm-d`, branch `feat/cap-p2-itm-d-risk-gate`) was used for all work.
- **Codebase/design analysis (before writing code):** full re-read of `ITM-DESIGN-001` §3/§9/§10/§13/§15/§22; direct inspection of ITM-C's `getEffectiveTrust`/`EffectiveTrustResult` contract, ITM-A's `trustLevel.ts` (`isAtLeastTrustLevel`), the closed 14-category error taxonomy, `ENG-P2-004`'s evaluator (`evaluateAuthorizationDecision`)/boundary-fixture pattern (`permissionBoundaryFixtureRepository.ts`, `touchPermissionBoundaryFixtureCommand.ts`) as the closest existing precedent for a pure decision function plus a governed internal test fixture, and `functions/src/index.ts` (confirmed zero trust-domain wiring exists anywhere — no production consumer to integrate against).
- **ITM-D scope reconstruction:** per §9.1/§15, ITM-D owns exactly: the bounded, closed `riskRequirement` vocabulary (never a caller-invented threshold); consuming ITM-C's already-derived effective trust as the sole authority; the requirement-satisfaction comparison; fail-closed behavior for malformed input/unsupported rule version/unavailable trust; and this closure-validation record. Confirmed non-scope: role/permission authorization (`ENG-P2-004`), authentication, product-action policy, Reward Engine logic, high-risk-action catalogue invention, new trust bands/signals, operator access.
- **Trust-gate architecture:** pure decision function `functions/src/domains/trust/riskGate/evaluateRiskGate.ts` (no Firestore, no `domains/identity`/`domains/permissions`/`domains/authentication` import — verified by an additive `riskGateDomainBoundary.test.ts`) consuming an explicit `EffectiveTrustReadResult` union, plus a read-only Firestore orchestrator `functions/src/domains/trust/services/checkRiskGateService.ts` (`checkRiskGate`) that calls ITM-C's unmodified `getEffectiveTrust` and translates its thrown `TrustDomainError` into the union — the risk-gate contract itself never throws (a decision function, per §13).
- **Requirement contract:** closed identifiers `TRUST_UNVERIFIED_OR_ABOVE`/`TRUST_PROVISIONAL_OR_ABOVE`/`TRUST_ESTABLISHED_OR_ABOVE` (`riskRequirement.ts`), each mapped internally to a minimum `TrustLevel` band — expressed directly in terms of ITM-C's own closed band set rather than inventing a production action/threshold vocabulary, since no risk-gated action is governed yet (§14/§20; task Phase G). Versioned via `RISK_REQUIREMENT_RULE_VERSION`.
- **Comparison semantics:** non-strict `>=` via ITM-A's existing `isAtLeastTrustLevel` — equality satisfies; verified across the full Phase M matrix (all nine requirement × trust-level combinations).
- **Rule-version consistency:** an effective-trust result derived under an unrecognized ITM-C rule version fails closed (`UNSUPPORTED_TRUST_RULE_VERSION`, `VALIDATION_FAILED`) rather than being silently compared (Phase O).
- **Standard-participation protection:** mechanically proven via static import-boundary tests — `checkRiskGateService`/`riskGate/` never imported by `functions/src/index.ts` or any file outside the trust domain; the non-gated fixture command (`touchStandardParticipationFixtureCommand.ts`) has zero source-level dependency on the risk gate; no file outside `domains/trust` references `effectiveTrustLevel` (no ad hoc global gate pattern anywhere in the codebase).
- **Governed test fixture:** mirrors `ENG-P2-004D`'s `permissionBoundaryTestFixtures` precedent — a bounded, synthetic `riskGateBoundaryTestFixtures` collection touched only after a `"sufficient"` decision (`touchRiskGateBoundaryFixtureCommand.ts`), and a structurally separate `standardParticipationTestFixtures` collection touched unconditionally with no risk-gate dependency at all (`touchStandardParticipationFixtureCommand.ts`). Neither is a Cloud Function endpoint; neither mints a production risk-gated action identifier.
- **ENG-P2-004/Authentication separation:** proven both structurally (no `domains/permissions`/`domains/authentication` import anywhere in `riskGate/`) and behaviorally (the fixture command never loads `businessMemberships`, evaluates role templates, verifies tokens, or establishes sessions).
- **Test matrix/TDD:** genuine RED captured (`Cannot find module './evaluateRiskGate'`) before implementation; 17 pure unit tests + 5 boundary tests + 7 real-Firestore-emulator tests (`checkRiskGateService`) + 4 real-Firestore-emulator fixture tests — the full Phase M/N/T matrix (all nine trust-level × requirement combinations, malformed requirement, unsupported rule version, missing/malformed identity, stale-cache-cannot-influence [inherited from ITM-C], recovery-neutral [inherited], provider-agnostic [inherited], determinism, no PII).
- **ITM-A/B/C regression:** zero diff to any ITM-A/B/C file; full `domains/trust` and repo-wide suites re-run green.
- **Full validation:** functions **943/943**; web **397/397**; `emulators:validate` **336/336** (one unrelated, pre-existing `ENG-P2-004D` concurrency-test 5s-timeout flake observed on the first run, confirmed non-reproducing on an immediate rerun — disclosed, not a regression); `tsc --noEmit`/build clean (both workspaces); `eslint .` clean; `prettier --check` clean; no secret/PII material found in the diff.
- **Status change:** `ITM-A` = Complete. `ITM-B` = Complete. `ITM-C` = Complete. `ITM-D` = **Implemented, pending Founder review — not merged**. ITM overall remains **Not complete until final merge/closure**. Capability 2 remains `Open — partially implemented; not closed`. Capability 3 remains `Not started`. G2 remains `Not started`.
- **Files:** `functions/src/domains/trust/riskGate/**` (new), `functions/src/domains/trust/services/checkRiskGateService.ts`, `checkRiskGateService.emulator.test.ts`, `touchRiskGateBoundaryFixtureCommand.ts`, `touchStandardParticipationFixtureCommand.ts`, `riskGateBoundaryFixture.emulator.test.ts`, `riskGateStandardParticipationBoundary.test.ts` (new), `functions/src/domains/trust/repositories/riskGateBoundaryFixtureRepository.ts` (new), `CDR-001-capability-delivery-roadmap.md` (§2 table row + header dated append), `engineering-implementation-programme.md` (header dated append), this entry, `IMPLEMENTATION_CHANGES.md`, and the new `CAP-P2-ITM-D` implementation report. **No dependencies, config, Firebase/Rules, or deployment change.**
- **Rollback:** the change is entirely additive (new files only, plus documentation-only edits to four governance records) — reverting the PR's merge commit (once merged) fully removes it; no schema, no deployed resource, no data to roll back.
- **Report:** [`CAP-P2-ITM-D` report](../05-implementation/reports/CAP-P2-ITM-D-risk-gate-contract-and-itm-closure-2026-08-17.md).

**Final gate: ITM-D READY FOR FOUNDER REVIEW/MERGE.**

---

## Entry 121 — `CAP-P2-ITM-C`: Independent Final Derivation Review, Merge & Closure

- **Date:** 16 August 2026
- **Performed by:** Claude (AI agent), per Founder task "CAP-P2-ITM-C — Independent Final Derivation Review, Merge & Closure." Authorizes an independent final review of `ITM-C` and, if all gates pass, merge/closure of PR #115 only. Does not authorize `ITM-D`, Capability 3, G2, or any ITM policy modification.
- **Entry verification:** PR #115 confirmed `OPEN`, head `8786d09b7f8bedaeccadef1ca4152ff9b6702e3d` exactly matching the expected head with zero later commits at review start, CI `pass` (3m32s), `mergeable: MERGEABLE`, `mergeStateStatus: CLEAN`, 2 commits. `ITM-A` (PR #111/#112) and `ITM-B` (PR #113/#114) confirmed merged. No `ITM-D` branch or PR found anywhere. Dirty primary worktree confirmed untouched (this session performs no operation against it, by harness-enforced worktree isolation).
- **Independent requirement reconstruction (not trusting the prior implementation report as source of truth):** re-read `ITM-DESIGN-001` §6.6/§6.6.1–§6.6.4/§22 directly. Confirmed `ITM-C`'s scope (effective trust derivation, account-age calculation, current-time boundary, rule-version interpretation, recovery-neutral interpretation, monotonic MVP semantics, effective-result contract) and non-scope (event ingestion, trust-record creation, persisted-evidence mutation, risk-gated authorization, Reward Engine policy, regression, operator visibility) against the design text itself.
- **Algorithm/boundary verification:** direct code inspection of `deriveEffectiveTrust.ts` confirmed the exact approved algorithm (`unverified` absent evidence; `established` iff `accountAgeDays >= 30`; else `provisional`) with no provider/event-count/recovery weighting, no numeric score, and `accountAgeDays = floor((now − registeredAt)/86400000)` matching §6.6.4 exactly (no calendar/timezone/client-time logic).
- **Stored-`trustLevel`-cannot-be-authority (primary merge gate):** confirmed structurally — `TrustRecordReadResult` (the pure function's own input type) has no `trustLevel` field at all, so a stale/forged value cannot reach the derivation as a lever. Wrote **new, independent adversarial tests** (`effectiveTrustService.independentReview.emulator.test.ts`, written separately from the implementer's own suite) that forge a persisted `trustRecords` document directly via raw Firestore writes — bypassing ITM-B's own repository entirely — across all three directions this task's Phase F named: (1) forged `established` + no evidence → effective `unverified`; (2) forged `established` + evidence + age < 30 → effective `provisional`; (3) forged `unverified` + evidence + age ≥ 30 → effective `established`; plus a fourth (forged `provisional` + no evidence + age ≥ 30 → still `unverified`, confirming age alone is never sufficient). Only direction (3) had prior test coverage from the implementer's own suite — directions (1), (2), and (4) were a genuine coverage gap, now closed. All four passed against the real Firebase Emulator Suite.
- **Recovery neutrality, auth-evidence, monotonic-MVP, rule-version, Customer Identity source, effective-result contract, ITM-D boundary, pure-function/determinism:** independently re-verified by direct code inspection — `TrustRecordReadResult` carries only a single `hasSuccessfulAuthentication` boolean (no recovery field, no count, no provider field); no downward-transition code path exists; malformed evidence fails closed via a thrown error rather than preserving a stale high band; `CURRENT_TRUST_RULE_VERSION = 1` with malformed/unsupported versions both failing closed distinctly; `registeredAt` sourced from the unmodified `customerIdentityRepository.getCustomerIdentityById`; `EffectiveTrustResult` contains only `customerIdentityId`/`effectiveTrustLevel`/`ruleVersion`/`evaluatedAt`/bounded `basis` — no score, no PII, no operator field; `getEffectiveTrust`'s signature takes no action identifier or threshold parameter; repository-wide grep for risk-gate terms (`riskGate`, `checkRiskGate`, `riskRequirement`, `sufficient`/`insufficient`) across the diff returned zero matches.
- **Concurrency/read-consistency (Phase O):** added two new tests proving a read before ITM-B's evidence commits returns `unverified`, the same read after commit returns `provisional`, and ten concurrent reads never mutate the underlying trust record (byte-identical document before/after). No transaction was added to the read-only orchestrator — correctly, since a plain read needs none.
- **Regression:** `git diff` over `functions/src/domains/trust/{models,repositories}` and `services/trustSignalIngestionService.ts`/`trustEventHandler.ts`/`trustSignalErrors.ts` confirmed byte-identical to `origin/main` — zero `ITM-A`/`ITM-B` semantic change.
- **Privacy/security:** grepped the full diff for email/phone/token/OTP/password/raw-claim/demographic/fraud-score terms — none found (only the test file's own assertion of their absence).
- **Review tooling:** no automated Codex/external review bot configured on this repository (`gh pr view 115 --json reviews` empty); disclosed per this task's own instruction — independent review served as the merge gate.
- **Full validation re-run:** functions **917/917**; `domains/trust` emulator suite isolated (real Firebase Emulator Suite) **37/37** (31 from the implementer's suite + 6 new independent-review tests); web **397/397**; `tsc --noEmit`/build clean (both workspaces); `eslint .` clean; `prettier --check` clean; PR CI **pass** (3m34s on the final reviewed head).
...
- **Merge:** with all gates clean, PR #115 merged via `gh pr merge 115 --merge` as `e090d86814a7583d0ac03ae26575f3a231414547`. Post-merge: `origin/main` confirmed at `e090d86`; `883bb6d` (final reviewed head) confirmed an ancestor; post-merge CI (`main`, run `31958885277`) confirmed `success`.
...
- **Files:** `CDR-001-capability-delivery-roadmap.md` (§2/§5 ITM lines + header dated append), this entry, `IMPLEMENTATION_CHANGES.md`, and a closure section appended to the `CAP-P2-ITM-C` implementation report. **No dependencies, config, Firebase/Rules, or deployment change.**
- **Rollback:** `git revert` the merge commit `e090d86` — the underlying change is entirely additive (new files only, plus this closure-sync commit's own additive test file); no schema, no deployed resource, no data to roll back.
- **Report:** [`CAP-P2-ITM-C` report](../05-implementation/reports/CAP-P2-ITM-C-effective-trust-derivation-2026-08-16.md) (closure section appended in place).
