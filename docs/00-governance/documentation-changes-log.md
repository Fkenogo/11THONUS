...
- **Summary:** With the Founder Model-T tuple correction merged (Entry 098 / PR #94 / `386fd8a`), the previously-blocked AUTH-05 (PR #92) is resumed and reconstructed on current `main`.
  - **F2 disposition — resolved by AUTH-CORR-002.** The cross-account-merge block was a model/`-08`-layer keying problem; AUTH-CORR-002 made the embedded projection, dedupe, unlink matching, last-reference handling and event payloads provider-qualified `(referenceType, referenceId)`. AUTH-05's orchestration (`accountLinkingService.ts`) already threaded `referenceType` through every `-08` call, so **no AUTH-05 F2 change was required**; verified against corrected `main`.
...
- **Evidence:** unit `accountLinkingService.test.ts` 15/15 (+5 F1, RED→GREEN) and `accountLinkingEndpointService.test.ts` unchanged; real-emulator `accountLinkingService.emulator.test.ts` 10/10 (+4: same-UID multi-provider + dual `-09` resolution; same-UID unlink without last-reference trip; F1 suspended link/unlink rejection). functions **511/511**, web **300/300**; `emulators:validate` full suite 201 pass / 2 inherited timeout flakes (`ENG-P1-002-CR1` + cross-package Scenario-7 race — untouched, both pass 13/13 in isolation). typecheck/lint/format/build clean.
