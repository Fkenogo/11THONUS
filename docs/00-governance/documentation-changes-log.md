...
- **Tests:** 39 new web tests (web suite 259→**298**, all passing this run incl. the previously-flaky phone-auth-harness latency test). `tsc`/`eslint .`/`prettier --check .`/`pnpm build` clean. Functions unit **491/491** unchanged; `pnpm emulators:validate` 189/190 — the one failure is the inherited `ENG-P1-002-CR1` command-dispatcher / identity-lifecycle concurrency flake in the byte-identical `functions/` tree (verified unrelated: `git diff origin/main -- functions/` is empty).
