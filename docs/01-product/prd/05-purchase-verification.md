> **Title:** PRD Section 5 — Purchase Verification Lifecycle (PVL)  
> **Version:** 1.0 · **Status:** Draft for review (pre-freeze) · **Classification:** Authoritative Product  
> **Governing document:** 11thONUS Platform Constitution  
> **Source-of-truth path:** `docs/01-product/prd/05-purchase-verification.md`  
> **Last controlled update:** 2026-07-16 (Phase 3B — §5 note added per DEC-DATA-003: monetary fields are reporting-only)
...
> **Confirmed (Founder decision, 16 July 2026 — DEC-DATA-003):** Unit Value and Currency are optional reporting metadata only. They are never read by the Loyalty Engine and never influence Verified Units, Reward Program progression, Loyalty Cycles or reward eligibility — unless a future founder decision explicitly introduces amount-based Reward Programs. The technical schema (TRD Chapter 10, §10.10.1) has been updated to carry these optional fields under this same constraint.
...
→ Customer Reviews, then one of:

- **Verify** → Verified → Verified Units created
- **Reject** → Rejected → closed (business may review)
- **Raise Dispute** → Under Review → Business Review → Correct Record (replacement) or Cancel Record

# 7\. Purchase States

Every Purchase Record shall exist in one state only.

> **State-model note (Phase 1 consolidation):** The canonical stored lifecycle begins at `waiting_for_customer` (TRD Consolidation Audit §7.5). "Draft" and "Recorded" below describe transient creation moments within the server workflow; they are not stored lifecycle states in the canonical model. The canonical stored states are: waiting_for_customer, verified, rejected, under_review, corrected, cancelled, expired, archived.
