...
**Accepting-identity authority, stated explicitly (not merely implied by the callable's parameter list, §37.6):** `acceptingCustomerIdentityId` is **never a request parameter and never client-supplied in any form** — it is resolved server-side, exclusively, from the authenticated principal on the incoming request (the same `resolveAuthenticatedCredential`/`authenticatedBusinessActor.ts` resolution path §25 already reuses unmodified for every other Business-scoped operation). `acceptBusinessTerms`'s only client-supplied argument is `businessId` (§37.6); the accepting identity itself is derived, not asserted. `businessId` establishes *which Business's Terms* are being accepted and is independently re-verified against the caller's owner/membership relationship (per §21/§25), but it never substitutes for, or itself establishes, *who* performed the acceptance — that is always the server-resolved authenticated principal, matching the same non-negotiable separation §37.5 draws for `termsVersion` (client-displayed vs. server-authoritative). A future `ENG-P3-002A` implementer must not accept `acceptingCustomerIdentityId`, `ownerUserId`, or any equivalent identity claim as a request field for this callable.
