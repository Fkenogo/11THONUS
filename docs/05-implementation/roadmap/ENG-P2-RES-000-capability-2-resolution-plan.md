...
6. [SCOPED by `ENG-P2-GATE-001`, 2026-08-02 — see below] `DEC-PROD-012` status in the Decision Register is a Final Decision, or a formally recorded defer-and-omit adoption (from `RES-006`) — required only for `ENG-P2-001-02` (Customer Profile)'s `gender` field and the corresponding `customerProfiles` schema-freeze in `ENG-P2-001-05` (Identity Persistence), per `DEC-PROD-012`'s own `Blocks: profile schema freeze` field and this Gate's own §"Explicitly not required by this gate" precedent for narrowly-scoped decisions (`DEC-LEGAL-005`). Not a blocker for `ENG-P2-001-01`, `-03`, `-04`, `-06`, `-07`, `-08`, `-09`, `-10`, or the non-`gender` portions of `-02`/`-05` — see the [`ENG-P2-GATE-001` Determination](ENG-P2-GATE-001-dec-prod-012-scope-determination.md) for the full evidence trail. This item's pre-scoping text (blanket relative to the then-undecomposed `ENG-P2-001`) is preserved in git history, not deleted — `DEC-PROD-012` itself remains `OPEN_FOUNDER`, not closed or recorded by this correction.
