...
> **Version:** 1.3 · **Status:** Design package — all Founder-owned decisions (`DEC-CKS-001`, `DEC-CKS-002`, Capability-3 wording) DISPOSITIONED/APPROVED; `DEC-DATA-005` (Engineering-Lead-owned) RESOLVED; `DEC-TECH-008` remains open, non-blocking; NO architecture/governance blocker remains — NOT an implementation authorization (Founder review of this revision + a fresh implementation authorization still required) · **Classification:** Working (execution-layer architecture record)
...
**New-reference eligibility (which states may be selected by a new `Business.primaryCategoryId`/`businessTypeId` write, or a new `RewardProgramVersion.qualifyingKnowledgeNodeIds` entry once Phase 4 exists):** only `active`-status `KnowledgeNode`s (combined, for customer-facing display, with a `published`-status `KnowledgeTranslation` in the requested or fallback language, per §11/§15/§18). `draft`, `in_review`, `retired`, and `archived` nodes are never eligible for a **new** reference.
...
- A validated reference contract: given a `KnowledgeNode` id and an expected `nodeType`, confirm it exists, is in a usable (`active`/`published`-equivalent, pending §9) status, and is of the expected type — this is the validation `ENG-P3-002`'s onboarding submission step (and `ENG-P3-001A`'s own `Business.primaryCategoryId` validation addition, §15) both need.
