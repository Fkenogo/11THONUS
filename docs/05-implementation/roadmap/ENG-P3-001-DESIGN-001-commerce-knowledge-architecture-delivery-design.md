...
> **Version:** 1.5 · **Status:** Design package — all Founder-owned decisions (`DEC-CKS-001`, `DEC-CKS-002`, Capability-3 wording, `DEC-CKS-003`) DISPOSITIONED/APPROVED; `DEC-DATA-005` (Engineering-Lead-owned) RESOLVED; `DEC-TECH-008` remains open, non-blocking; one independent-review F3 correction applied (§9.4/§15/§18, reference-validity vs. translation-display-availability separation); NO architecture/governance blocker remains · **Classification:** Working (execution-layer architecture record)
> **v1.5 (2026-08-21, `ENG-P3-001C` independent final review):** Founder disposition `DEC-CKS-003` (Business Category/Business-Type classification consistency on update) recorded — see §36. Dated addendum only; no existing section's text altered.
...
## 36. Founder Disposition — `DEC-CKS-003` (Business Category/Business-Type Classification Consistency on Update), dated 2026-08-21, `ENG-P3-001C`

Recorded per the same convention as `DEC-CKS-001`/`DEC-CKS-002` (§26): a dated Founder disposition addendum on this design document, not a new decision-register.md entry and not a new standalone product-decision document — this question is scoped entirely to how `ENG-P3-001C`'s classification-reference validation composes with the existing Business profile-update contract, not a new architecture or a change to the seed/schema/taxonomy questions §26/§9.4 already settled.

**Question:** when a governed Business classification update (create or profile-update) would change `primaryCategoryId` such that the Business's existing (or newly supplied) `businessTypeId` no longer belongs to the resulting category, what must the platform do?

**FOUNDER DISPOSITION — APPROVED (verbatim policy, recorded 2026-08-21):**

> A Business classification update must never persist an incompatible pair of `primaryCategoryId`/`businessTypeId`. If changing `primaryCategoryId` makes the existing `businessTypeId` invalid: (1) the platform MUST NOT silently keep the incompatible type; (2) the platform MUST NOT automatically choose/remap a replacement Business Type; (3) the platform MUST NOT silently clear `businessTypeId` as a side effect. The update must fail closed unless the same governed update intentionally establishes a valid resulting classification state. A valid resulting state is either: A) active Business Category + compatible active Business Type, or B) active Business Category + no Business Type, where `businessTypeId` is optional and the command contract provides an intentional way to remove/omit the type. This disposition is intended to preserve data integrity and user intent. Do not invent automatic mapping.

**Recorded implication for this design and for `ENG-P3-001C`:** no schema change is required — `Business.businessTypeId` was already optional (§7/§20, unchanged by this disposition). `functions/src/domains/business/services/businessProfileCommand.ts` and `functions/src/domains/business/models/business.ts`'s `updateBusinessProfile` merge already used a pre-existing, `ENG-P2-002C`-established key-presence idiom (`"businessTypeId" in params`, distinguishing "key absent → leave unchanged" from "key present with `undefined` value → intentional clear") for every other already-clearable optional field on this same patch type; `ENG-P3-001C`'s classification-validation integration (independent final review, this same task) reused that identical idiom rather than inventing a new one, and confirmed by direct test (`businessClassificationValidation.emulator.test.ts`, items 21/22, added during this review) that: (a) an update that never mentions `businessTypeId` never clears it, and (b) a category change paired with an explicit `businessTypeId: undefined` in the patch — a genuine key-presence removal, not a value coercion — is accepted and results in state B (category-only) exactly as this disposition requires. A category change that leaves an incompatible `businessTypeId` untouched in the patch is rejected (state-A/state-B violation), proven by the pre-existing test item 20. No automatic remap path exists anywhere in `businessClassificationValidation.ts` (confirmed by direct code read, this same review) — the module only validates a caller-supplied resulting pair, never selects one.

**Status:** `DEC-CKS-003` APPROVED. No further disposition needed. No STOP condition triggered — the existing contract, verified rather than assumed, already satisfies this policy; the two required-test gaps identified during independent review (explicit-clear and unrelated-update-preserves-type) were closed as test additions within `ENG-P3-001C`'s own scope, not a contract or schema change.
