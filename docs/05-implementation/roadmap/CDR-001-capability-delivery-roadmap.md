...
- **Objective:** a customer can register, prove who they are at the minimum level required, and receive a scannable loyalty identity.
...
- **Dependencies:** Capability 1; Decision Dependencies `DEC-SEC-001`, `DEC-DATA-007`, `DEC-PROV-004`, `DEC-ID-003` — all `CONFIRMED` (Capability 2 Resolution Sprint, merged to `main` 2026-07-31 — see the [Decision Register](../../00-governance/decisions/decision-register.md)).
- **Validation outcome:** not started — `ENG-P2-001`/`ENG-P2-004` remain `Blocked`: the Decision Dependencies above are now resolved and Phase 1 exit is now formally Approved (`ENG-P1-EXIT-001`, 2026-07-31), but two further, unrelated items in the Resolution Plan's [Capability Authorisation Gate](ENG-P2-RES-000-capability-2-resolution-plan.md#7-capability-authorisation-gate) (§7) remain open: `EXT-TECH-001` evidence pending, `DEC-PROD-012` open. `BaseMetadata` conformance (Gate item 7) is now fully resolved — documentation contract corrected (`RES-005.2a`, 2026-07-31), code conformance now also complete (`RES-005.2b`, 2026-07-31).
