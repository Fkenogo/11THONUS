...
| 2 | Customer Identity | Open — partially implemented; not closed. **Concern-level status (`DEC-GOV-008`; authoritative in §5):** Customer Identity `Complete` (all 10 `ENG-P2-001` packages merged; `CAP-P2-007`/`-008`); Authentication `Not started — Unauthorised`; ITM `Not started — Unauthorised` (internal). Concern Completion ≠ Capability closure — see §5. |
