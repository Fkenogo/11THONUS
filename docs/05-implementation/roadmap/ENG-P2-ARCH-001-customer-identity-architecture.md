...
| **Regeneration** | [Clarified `ENG-P2-001-04` Founder Review, 2026-08-04 — see the [Implementation Report](../reports/ENG-P2-001-04-implementation-report-2026-08-04.md) §6; original wording preserved in git history] Customer-initiated re-issuance of the QR (e.g., if a printed/shared QR image is considered compromised). The customer's `qrReference` **association** — exactly one QR slot per identity, mapped to the same Loyalty Number — is unchanged; regeneration issues a new reference **value** into that slot, and the prior value is invalidated (see the Invalidation row below). It is the association that never changes, not the specific reference string, which is deliberately replaced so a compromised prior value stops working. Regeneration produces a new scannable artifact for the same permanent identity, not a new identity. |
