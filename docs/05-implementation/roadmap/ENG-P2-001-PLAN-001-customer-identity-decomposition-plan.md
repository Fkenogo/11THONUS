...
- **Scope:** QR payload (plain opaque reference to the loyalty code — never a signed token, never personal data, per `DEC-DATA-007`); generation at `Registered` alongside the Loyalty Number; regeneration (new encoding, same underlying reference — `ENG-P2-ARCH-001` §5); invalidation of prior codes on regeneration (fail closed); mapping to customer identity; secure lookup with rate-limiting against enumeration.
...
| `ENG-P2-001-05` | Identity Persistence | Firestore collections, Rules, indexes | `-01`,`-02`,`-03`,`-04` | Blocks `-06`–`-10` | Awaiting Founder Authorisation |
| `ENG-P2-001-06` | Identity Lifecycle and Status | State machine | `-01`,`-05` | Blocked by Ambiguity 1 (§14); blocks `-07`,`-09` | Awaiting Decision (Ambiguity 1) |
...
Per this task's explicit instruction, `DEC-PROD-012` is **not** closed or recorded by this task. Findings:
...
**Conclusion: per the current, literal Capability Authorisation Gate, `ENG-P2-001` is not yet implementation-ready** — `DEC-PROD-012` (`OPEN_FOUNDER`) remains open and Gate item 6 reads as a blanket precondition. This plan does not mark Capability 2 implementation-ready.
...
- **`DEC-PROD-012` closure:** **not required** for `-01` specifically (it has no profile/`gender` dependency) — but see §10's finding that the *Gate as currently worded* may still block it as part of `ENG-P2-001` overall. This is the same ambiguity, restated at the package level: `-01` is technically decision-clean, but the Gate's literal text may not distinguish it from `ENG-P2-001` as a whole.
...
|---|---|---|---|---|---|
| 1 | Whether `Recovered` is a persistent status, a transition, an event, or an audit marker | Engineering-Lead-level architecture clarification (small addendum to `ENG-P2-ARCH-001` §3) | `-06`, `-07` | Recommend the smallest clarification: treat `Recovered` as a **transient transition marker**, not a persistent status — an identity that completes recovery lands back in `Active` (or whatever state it was in before losing access), with the *event* of having recovered preserved permanently in the audit trail (`-10`), not as an ongoing status value. This keeps the `users.status` enum small and matches how `Dormant`→`Active` already works (no lingering "was dormant" marker). | No — this is an engineering-architecture clarification, not a constitutional or product decision; Engineering Lead may resolve it directly in a follow-up addendum to `ENG-P2-ARCH-001`. |
| 2 | `DEC-PROD-012`'s narrow "profile schema freeze" scope vs. `ENG-P2-RES-000` §7 Gate item 6's blanket "`ENG-P2-001` may begin" wording | Founder (owns `DEC-PROD-012`) and/or Engineering Lead (owns the Gate document's own wording) | Potentially all of `ENG-P2-001`, or narrowly just `-02`'s `gender` field, depending on resolution | Recommend the Founder either (a) resolve `DEC-PROD-012` outright (it only needs a value-set choice plus optional legal input, `EXT-LEG-001`), removing the ambiguity entirely, or (b) explicitly narrow Gate item 6's scope to "Customer Profile `gender` field freeze only" in a future governance task, unblocking `-01`/`-03`–`-10` immediately. | **Yes** — either resolving `DEC-PROD-012` itself or narrowing the Gate's scope is a Founder-level (or Founder-delegated Engineering-Lead) governance action this task does not perform. |
