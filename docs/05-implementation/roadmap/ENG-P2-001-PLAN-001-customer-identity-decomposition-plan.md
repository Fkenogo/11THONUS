...
> **Updated 2026-08-04:** the domain-foundation layer implemented at `functions/src/domains/qrIdentity/` — see the [Implementation Report](../reports/ENG-P2-001-04-implementation-report-2026-08-04.md). Test-first (39/39 new tests passing), zero Firebase dependency (machine-enforced by a scoped `eslint.config.js` rule). A textual tension between this section's own "same underlying reference" phrasing and its "prior reference fails to resolve" / "new resolvable reference" requirements (both in this same section) was identified and reconciled — see the Implementation Report §6 for the full analysis; regeneration produces a genuinely new reference, invalidating the prior one. Rate-limiting, image rendering, scanning, and Firestore persistence remain deferred to future packages. Pending Founder-authorized review and merge. `ENG-P2-001-01`–`-03`, `-05` through `-10`, and Capability 2 overall are unaffected — none advanced by this update.
