...
> **Updated 2026-08-05:** implemented at `functions/src/domains/identity/{models,repositories}/` — see the [Implementation Report](../reports/ENG-P2-001-07-implementation-report-2026-08-05.md). Test-first, all new/modified tests passing against the real Firebase Emulator Suite. **Scope narrowing disclosed:** the current Founder task brief excludes authentication-provider relinking (narrower than this section's own original text below, which described "restoring access references — linking a new/updated Authentication reference" as in-scope) — the current, more specific brief was followed; the recovery result exposes only a provider-neutral handoff reference for a future Authentication capability. Implemented: a provider-neutral `RecoveryProof` contract; a bounded, exact-match, non-enumerable identity-lookup boundary (Customer Identity ID / Loyalty Number / current QR reference — Authentication-subject-reference and support-managed-reference routes assessed and deferred, no reverse index exists yet without new schema); `-06`'s recovery repository extended additively with proof validation and a new `recoveryProofReferences` doc-ID-keyed collection for proof-reuse prevention; the existing `IdentityRecovered` event additively extended with `resultingStatus`/`recoveryProofReference`/`proofMethodCategory`. Firestore Rules required no change. Pending Founder-authorized review and merge. `ENG-P2-001-01`, `-03`, `-04`, `-05`, `-06` remain merged as previously recorded; `-02`, `-08` through `-10`, `ENG-P2-001` as a whole, and Capability 2 overall are unaffected — none advanced by this update.
...
> **Updated 2026-08-05:** implemented at `functions/src/domains/identity/repositories/authenticationReferenceRepository.ts` — see the [Implementation Report](../reports/ENG-P2-001-08-implementation-report-2026-08-05.md). Test-first, all new tests passing against the real Firebase Emulator Suite. **Scope narrowing disclosed:** the current Founder task brief defines duplicate prevention structurally — refusing a second identity from linking a `(referenceType, referenceId)` pair another identity already owns — rather than this section's own original framing below (heuristic contact-attribute matching plus a support-review queue deliverable). The current, more specific brief was followed; heuristic detection and the review-queue UI remain unimplemented and deferred. Implemented: a new doc-ID-keyed `authenticationReferences/{referenceType}:{referenceId}` collection as the authoritative cross-identity uniqueness source, transactionally maintained alongside the existing per-identity projection; transactional, idempotent link/unlink; a cross-identity conflict fails closed while still committing a privacy-safe `AuthenticationReferenceConflictDetected` audit event; unlink preserves history on the authoritative record. **Ambiguity 4 (§14) remains unresolved and untouched** — this update implements detection/fail-closed refusal only, never merge execution, exactly matching the Ambiguity's own "detection/queueing is unblocked; merge execution is not" boundary. Firestore Rules required no change. **Founder Review (2026-08-05):** positively reviewed, `PR #63`; the disclosed relink-after-unlink policy is now Founder-approved as the **Authentication Reference Permanence Principle** — see §14 Ambiguity 5, now resolved. Pending Founder-authorized merge (merge itself withheld pending this correction). `ENG-P2-001-01`, `-03`, `-04`, `-05`, `-06`, `-07` remain merged/implemented-pending-merge as previously recorded; `-02`, `-09`, `-10`, `ENG-P2-001` as a whole, and Capability 2 overall are unaffected — none advanced by this update.
...
> **Corrected 2026-08-05 (same day, before merge):** Founder review of PR #64 withheld merge pending clarification of the purpose allow-list and result minimisation — see the resolved [Ambiguity 6](#14-decision-and-ambiguity-register) and [Implementation Report §38](../reports/ENG-P2-001-09-implementation-report-2026-08-05.md#38-correction-2026-08-05--founder-review-lookup-purpose-and-result-minimisation-clarification). The allow-list below and `authenticationReferences` in the result type are both now superseded by that correction.

> **Updated 2026-08-05:** implemented at `functions/src/domains/identity/repositories/identityLookupRepository.ts` — see the [Implementation Report](../reports/ENG-P2-001-09-implementation-report-2026-08-05.md). Test-first, all new tests passing against the real Firebase Emulator Suite. Implemented: exact-match-only lookup by Customer Identity ID, Loyalty Number, QR reference, and Authentication Reference, each gated by a caller-declared `IdentityLookupPurpose` against a small hardcoded per-lookup-type allow-list (not the role/permission system this task explicitly excludes); a bounded result never exposing phone/email/trust/profile data; a new privacy-safe audit event for support/recovery/authentication-purpose lookups and any failed QR lookup. **Scope note (disclosed, not silently narrowed):** this section's own original text names "merchant-assisted search" and "lookup by an authorised contact reference (support-assisted recovery entry point)" — implemented as a declared `merchant_transaction` purpose scoped to Loyalty Number/QR only (no Purchase/Transaction domain exists yet to bind an actual transaction context to, confirmed by repo search) and a declared `recovery`/`support` purpose covering Customer-Identity-ID/Loyalty-Number/QR (Authentication-Reference-based recovery remains out of scope, matching `-07`'s own already-disclosed deferral — `-07`'s recovery orchestration itself is unmodified by this task). No rate limiting implemented (explicitly deferred); no Firestore Rules change required. Pending Founder-authorized review and merge. `ENG-P2-001-01`, `-03`–`-08` remain merged/implemented-pending-merge as previously recorded; `-02`, `-10`, `ENG-P2-001` as a whole, and Capability 2 overall are unaffected — none advanced by this update.
...
> **Updated 2026-08-05:** implemented at `functions/src/domains/identityAudit/` — see the [Implementation Report](../reports/ENG-P2-001-10-implementation-report-2026-08-05.md). Test-first, all new tests passing against the real Firebase Emulator Suite. Every material identity event named in this section's own scope already existed in `-01`/`-03`–`-09`'s merged code — **zero new domain events were created**; this package is a bounded read-side audit-record projection, privacy classification, query, and observability-signal layer over the already-governed outbox. **Scope note (disclosed, not silently narrowed):** this section's own original text says "reusing the existing sanitization boundary from `ENG-P1-003`'s observability foundation" for diagnostics — that adapter (`apps/web/src/observability/*`) is confirmed frontend-only (Sentry, browser-side), architecturally inapplicable to Cloud Functions code; operational signals instead reuse the existing *backend* structured logger (`shared/logging/logger.ts`/`OperationalLog`, `ENG-P1-002`), which is the correct backend-side equivalent already present in this codebase. Retention: no numeric period is governed anywhere in TRD21/TRD10 for identity audit events — disclosed as an open gap, no deletion/archival job implemented (per this task's own explicit instruction not to invent one). `ENG-P2-001-01`, `-03`–`-09` remain merged as previously recorded; `-02`, `ENG-P2-001` as a whole, Authentication, ITM, and Capability 2 overall are unaffected — none advanced by this update. Pending Founder-authorized review and merge.
...
| `ENG-P2-001-02` | Customer Profile | Mutable profile data model | `-01` | ~~`gender` field blocked by `DEC-PROD-012` (§14)~~ **`DEC-PROD-012` CLOSED 2026-08-07 (Option D — gender omitted from MVP); no longer blocked (§14)** | ~~Awaiting Founder Authorisation (partial — non-`gender` scope)~~ **Implemented (2026-08-07, TDD, domain layer — `models/customerProfile.ts`, 20 unit tests; gender omitted per `DEC-PROD-012` Option D) — pending Founder-authorized review/merge. Persistence wiring is `-05`/future surface (see [implementation report](../reports/ENG-P2-001-02-implementation-report-2026-08-07.md)).** |
...
| `ENG-P2-001-07` | Identity Recovery | Identity-owned recovery mechanics | `-01`,`-05`,`-06` | Depends on Authentication interface (§7) stability | **Implemented (2026-08-05), TDD, all new/modified tests passing (real Firebase Emulator Suite) — see [Implementation Report](../reports/ENG-P2-001-07-implementation-report-2026-08-05.md) — pending Founder-authorized review/merge** |
| `ENG-P2-001-08` | Identity Linking and Duplicate Prevention | Linking + structural duplicate detection (not auto-merge) | `-01`,`-05` | Merge execution blocked by Ambiguity 4 (§14) | **Implemented (2026-08-05), TDD, all new tests passing (real Firebase Emulator Suite) — see [Implementation Report](../reports/ENG-P2-001-08-implementation-report-2026-08-05.md) — pending Founder-authorized review/merge; structural (same-reference) duplicate prevention only, heuristic contact-attribute detection and review-queue UI deferred; merge execution remains blocked by Ambiguity 4, untouched** |
| `ENG-P2-001-09` | Identity Query and Lookup Interfaces | Bounded read interfaces | `-05` | — | **Implemented (2026-08-05), TDD, all new tests passing (real Firebase Emulator Suite) — see [Implementation Report](../reports/ENG-P2-001-09-implementation-report-2026-08-05.md) — pending Founder-authorized review/merge; exact-match only, no fuzzy/merchant-search UI, no rate limiting** |
| `ENG-P2-001-10` | Identity Audit and Observability | Audit trail, sanitized diagnostics | `-01`,`-06`,`-07`,`-08` | — | **Implemented (2026-08-05), TDD, all new tests passing (real Firebase Emulator Suite) — see [Implementation Report](../reports/ENG-P2-001-10-implementation-report-2026-08-05.md) — pending Founder-authorized review/merge; zero new domain events, read-side audit projection only** |
...
| 5 | ~~Whether an unlinked authentication reference may later be re-linked, and by which identity~~ **RESOLVED 2026-08-05, Founder Review — `ENG-P2-001-08` (`PR #63`)** | Founder-level product/governance policy | `-08` | **Authentication Reference Permanence Principle** (Founder-approved, verbatim): "Once an authentication reference has been linked to a Customer Identity, its historical ownership remains permanently associated with that identity. If unlinked, it may be restored only to the same Customer Identity. Linking it to a different identity is prohibited unless a future governed manual-review and transfer process explicitly authorises the change." Implemented exactly as approved in `authenticationReferenceRepository.ts` — see the [`ENG-P2-001-08` Implementation Report](../reports/ENG-P2-001-08-implementation-report-2026-08-05.md) §4A: same-identity relink permitted, cross-identity relink rejected (regardless of the authoritative record's `linked`/`unlinked` status), historical ownership retained on the authoritative record, no automatic transfer or merge. This is the **current, approved MVP policy** — a future governed manual-review-and-transfer capability remains explicitly deferred, not designed here. | No further input required — resolved by this Founder Review; a future decision task governs only the deferred manual-transfer capability, not this permanence rule itself. |
| 6 | ~~Per-lookup-type purpose allow-list and whether the shared lookup result should expose `authenticationReferences` to every purpose~~ **RESOLVED 2026-08-05, Founder Review — `ENG-P2-001-09` (`PR #64`)** | Founder-level product/governance policy | `-09` | **Lookup-Purpose and Result-Minimisation Policy** (Founder-directed, verbatim intent): Customer Identity ID lookup — trusted internal, support and governed recovery use only. Loyalty Number lookup — trusted internal, governed recovery, support and authenticated participating-business transaction use. Active QR lookup — trusted internal and authenticated participating-business transaction use; additional purposes only where explicitly governed (`support` removed — no governing citation existed). Authentication Reference lookup — Authentication, governed recovery, support and trusted internal use (`recovery` added as a new Founder-approved capability grant at the lookup level only; does not wire into or expand `-07`'s own unmodified recovery orchestration). Caller-supplied purpose values are never authority by themselves; validation belongs to a future trusted application boundary, which must fail closed. Every lookup returns only the minimum fields required for its purpose — `IdentityLookupResult.authenticationReferences` is now optional, populated only when `purpose === "authentication"`, so merchant-transaction and all other non-authentication purposes never receive linked-provider metadata. Implemented exactly as directed — see the [`ENG-P2-001-09` Implementation Report](../reports/ENG-P2-001-09-implementation-report-2026-08-05.md) §38. This is the **current, approved policy**; the future trusted application boundary that verifies caller entitlement to a declared purpose remains explicitly deferred, not designed here. | No further input required — resolved by this Founder Review; a future decision task governs only the deferred caller-authority-verification boundary, not this purpose/result policy itself. |
