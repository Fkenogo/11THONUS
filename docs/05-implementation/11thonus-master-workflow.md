...
- **Next governed action — [UPDATED 2026-08-07, `CAP-P2-004`].** `ENG-P2-001-02` is now **implemented and merged** (PR #76), so all ten `ENG-P2-001` child packages are merged and the Customer Identity **concern** is `Implemented — Validation/Closure Pending` (see the concern-status bullet above / [`CDR-001` §5](roadmap/CDR-001-capability-delivery-roadmap.md)). The next governed action is **not uniquely established and awaits fresh Founder instruction** — candidate governed tracks include the Customer Identity concern's outstanding closure conditions (`ENG-P2-001-02` architecture/Technical Review; `-02` persistence wiring; documentation currency — per `CAP-P2-002`), the Authentication or ITM concerns (both unauthorised), `ENG-P2-004`, RTM Finding F11 (deferred), or defining formal concern-completion criteria. **[Historical, superseded:]** the paragraph below described `-02` as "technically authorised to begin, awaiting authorization" before it was implemented. Parallel governed tracks also remain available but none is uniquely mandated: the Identity/Authentication/ITM engineering-design decomposition of `ENG-P2-001` (`IDENTITY-ALIGN-001`, "not yet performed"), the RTM Finding F11 synchronisation (Founder-approved deferred engineering work), or a registered successor package (`OBS-OPS-001` / `ENG-SEC-001` / `ENG-CI-001`). Authentication and ITM remain separately governed and **unauthorised**; the `-02` Customer Profile implementation itself remains unauthorised until the Founder issues a fresh authorization.
