...
- **Next governed action — [UPDATED 2026-08-09, `AUTH-05` resumed after `AUTH-CORR-002`].** The Founder's Model-T correction (`AUTH-CORR-002`, provider-qualified authentication references) is **merged** (PR #94, merge `386fd8a`, post-merge CI green), so the previously-blocked `AUTH-05` (Account linking) is **resumed** and reconstructed on current `main`. **F2** (cross-account merge) is **resolved by AUTH-CORR-002** — AUTH-05's orchestration was already tuple-aware, no F2 change. **F1** (acting-identity access-state enforcement) is **corrected** (TDD): before any link/unlink the acting identity is loaded via the merged `getCustomerIdentityById` and gated on `identity.status`, reusing AUTH-03's returning-user gate and existing closed taxonomy (`active`→proceed; `suspended`→`ACCOUNT_SUSPENDED`; else→`AUTH_FORBIDDEN`). Same-UID phone+Google link as two distinct references both resolving via `-09` to one identity; the same-UID second provider unlinks without tripping last-reference. No new error category; no credential persisted; `CustomerAuthenticated` stays AUTH-08; no AUTH-02/`-08`/`-09`/AUTH-03/AUTH-04 change. functions **511/511**, web **300/300**, AUTH-05 emulator file **10/10**; `emulators:validate` 201 pass / 2 inherited timeout flakes (untouched; pass in isolation). **Implemented, pending Founder-authorized review/merge** (PR #92, reconstructed on `main`); **not** self-merged; **`AUTH-06`+ not started/authorized**. Capability 2 remains `Open — partially implemented; not closed`; Customer Identity `Complete`; ITM/`ENG-P2-004` unchanged. See the [`AUTH-05` report §17](reports/AUTH-05-account-linking-2026-08-09.md).
