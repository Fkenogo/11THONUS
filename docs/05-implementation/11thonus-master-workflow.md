...
| P2 | Identity, Roles and Business Context | Customer/business/staff identity and RBAC | Phase 1 exit met | `DEC-SEC-001`, `DEC-ID-003`, `DEC-DATA-007` — open; `DEC-PROV-004` — **CONFIRMED** (2026-07-30) | Customer registers with a safe loyalty identity; owner can create a business; owner can invite staff; role switching works; security-rule and authorization tests pass (§22.12) | Blocked | 3 open D1 decisions (§12) + Phase 1 not exited | Phase 1 exit + the 3 remaining decisions |
...
Per the live [Decision Register](../00-governance/decisions/decision-register.md) (checked directly, not reconstructed from memory, on 2026-07-22): all four of Phase 2's D1 decision dependencies remain open — `DEC-SEC-001` (`OPEN_ENGINEERING`), `DEC-ID-003` (`OPEN_FOUNDER`), `DEC-DATA-007` (`OPEN_ENGINEERING`), `DEC-PROV-004` (`OPEN_PROVIDER`). Phase 2 additionally cannot begin until Phase 1 exits (TRD22 §22.11 exit criteria fully met, not merely `ENG-P1-001`). **Update (`ENG-P2-RES-ADMIN-002`, 2026-07-30):** `DEC-PROV-004` has since closed — now `CONFIRMED` (Approved with Conditions, per `RES-002`/`RES-002A`/`RES-002B`; see the [Decision Register](../00-governance/decisions/decision-register.md)). `DEC-SEC-001`, `DEC-ID-003`, and `DEC-DATA-007` remain open; 3 of Phase 2's 4 D1 decision dependencies are now unresolved, not 4. Phase 1 exit remains separately unmet.
...
- **Unresolved blockers:** three open D1 decisions (§12) plus Phase 1 not exited. *(Corrected `ENG-P2-RES-ADMIN-002`, 2026-07-30: was four — `DEC-PROV-004` is now `CONFIRMED`.)*
- **Next gate owner:** Founder (`DEC-ID-003`), Engineering Lead (`DEC-SEC-001`, `DEC-DATA-007`). `DEC-PROV-004` resolved (`CONFIRMED`, 2026-07-30) — no longer a pending gate owner item.
