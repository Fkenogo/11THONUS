...
- **Next governed action — [UPDATED 2026-08-09, `AUTH-05` resumed after `AUTH-CORR-002`].** The Founder's Model-T correction (`AUTH-CORR-002`, provider-qualified authentication references) is **merged** (PR #94, merge `386fd8a`, post-merge CI green), so the previously-blocked `AUTH-05` (Account linking) is **resumed** and reconstructed on current `main`. **F2** (cross-account merge): its **keying** half is resolved by AUTH-CORR-002 (AUTH-05 already tuple-aware); on the resumption review-gate the Founder additionally **directed a defensive same-Firebase-principal gate** — before `-08`, `linkAuthenticationProvider` verifies the new provider's verified uid equals the acting uid, failing closed (`AUTH_FORBIDDEN`, existing category), defense-in-depth alongside (never replacing) `-08`'s cross-identity control. **F1** (acting-identity access-state enforcement) is **corrected** (TDD): before any link/unlink the acting identity is loaded via the merged `getCustomerIdentityById` and gated on `identity.status`, reusing AUTH-03's returning-user gate and existing closed taxonomy (`active`→proceed; `suspended`→`ACCOUNT_SUSPENDED`; else→`AUTH_FORBIDDEN`). Same-UID phone+Google link as two distinct references both resolving via `-09` to one identity; the same-UID second provider unlinks without tripping last-reference. No new error category; no credential persisted; `CustomerAuthenticated` stays AUTH-08; no AUTH-02/`-08`/`-09`/AUTH-03/AUTH-04 change. functions **513/513**, web **300/300**, AUTH-05 emulator file **11/11**; CI is the authoritative emulator gate. **Implemented, pending Founder-authorized review/merge** (PR #92, reconstructed on `main`); **not** self-merged; **`AUTH-06`+ not started/authorized**. Capability 2 remains `Open — partially implemented; not closed`; Customer Identity `Complete`; ITM/`ENG-P2-004` unchanged. See the [`AUTH-05` report §17](reports/AUTH-05-account-linking-2026-08-09.md).
- **Next governed action — [UPDATED 2026-08-10, `AUTH-05` merge/closure sync].** `AUTH-05` is now **MERGED and closed.** PR #92 merged as commit `6c18ca6da373d5105ff895dfce4644910f0d8d95`, merged 2026-08-10T07:40:27Z by Kenogo, **post-merge CI green** (run 31366820398, workflow "CI", `success`). `origin/main` = `6c18ca6`. This bullet **supersedes the "pending Founder-authorized review/merge" / "AUTH-06+ not started" wording in the `AUTH-05` bullet above** (preserved for audit continuity — programme-currency reconciliation only, no code or capability change). **The next governed action is `AUTH-06`** (Recovery credential proof), which requires its own fresh Founder implementation authorization. Capability 2 remains `Open — partially implemented; not closed`; Customer Identity `Complete`; ITM/`ENG-P2-004` unchanged.
- **Next governed action — [UPDATED 2026-08-10, `AUTH-06`].** `AUTH-06` (Recovery credential proof) is now **implemented, test-first (TDD), pending Founder-authorized review/merge** — Founder-authorized as the **sixth** Authentication implementation package (the fresh implementation authorization given in this task, recorded per the AUTH-01/-02/-03/-04/-05 convention). Backend-only under `functions/src/domains/authentication/services/*` plus one additive `recoverAuthenticatedIdentity` `onCall` in `index.ts`: it performs the provider proof (verifies a recovery provider credential via the AUTH-02 `TokenVerifierPort`), **resolves it to its OWNING identity via the AUTH-02 `-09` resolver** (the recovery target is derived from the proof, never client-supplied — a foreign/unlinked credential fails closed `RESOURCE_NOT_FOUND`), constructs an `accepted` `RecoveryProof` (`phone_otp`→`phone_otp`, `google_sign_in`→`linked_provider`; opaque CSPRNG `proofReference`; `authority: "customer_initiated"`), and hands it to the merged `-07` `recoverCustomerIdentityByReference` — which transitions status via `-06`, rejects proof reuse, and emits `IdentityRecovered`. **No active-state gate** (recovery is *for* non-active identities; `-06` enforces eligibility); **emits no domain events** (`IdentityRecovered` is `-06`/`-07`'s; `AuthenticationRecoveryProofProvided` stays AUTH-08; `CustomerAuthenticated` stays AUTH-08); idempotency consumed from `-07` (namespaced key + credential-bound request hash); no credential material persisted/logged/returned; closed 14-category taxonomy (no new category); no `-06`/`-07`/`-08`/`-09`/AUTH-01–AUTH-05 change; the out-of-band lookup surface and post-recovery provider relink are explicitly out of scope. TDD: functions **522/522** (+9 AUTH-06 unit tests), web **300/300** unchanged, `pnpm emulators:validate` **211/211** (+6 AUTH-06 emulator tests, no inherited flake recurred); typecheck/lint/format/build clean; CI is the authoritative emulator gate. **Implemented, pending Founder-authorized review/merge** (PR to be opened on this branch); **not** self-merged; **`AUTH-07`+ not started/authorized**; dirty primary worktree untouched. Capability 2 remains `Open — partially implemented; not closed`; Customer Identity `Complete`; ITM/`ENG-P2-004` unchanged. See the [`AUTH-06` report](reports/AUTH-06-recovery-credential-proof-2026-08-10.md).
- **Next governed action — [UPDATED 2026-08-10, `AUTH-06` merge/closure sync].** `AUTH-06` is now **MERGED and closed.** PR #95 merged as commit `04e11715a445f67cd472962721579e9f3a640dfe`, merged 2026-08-10T09:27:45Z, **post-merge CI green** (run 31374698932, workflow "CI", `success`). `origin/main` = `04e1171`. This bullet **supersedes the "pending Founder-authorized review/merge" / "AUTH-07+ not started" wording in the `AUTH-06` bullet above** (preserved for audit continuity — programme-currency reconciliation only, no code or capability change). **The next governed action is `AUTH-07`** (Session/access gating), which requires its own fresh Founder implementation authorization. Capability 2 remains `Open — partially implemented; not closed`; Customer Identity `Complete`; ITM/`ENG-P2-004` unchanged.
- **Next governed action — [UPDATED 2026-08-10, `AUTH-07`].** `AUTH-07` (Session / access gating) is now **implemented, test-first (TDD), pending Founder-authorized review/merge** — Founder-authorized as the **seventh** Authentication implementation package (the fresh implementation authorization given in this task, recorded per the AUTH-01/-02/-03/-04/-05/-06 convention). Backend under `functions/src/domains/authentication/{models/privilegedReauthentication.ts, services/sessionAccessService.ts}` and frontend under `apps/web/src/authentication/{signOutFlow.ts, privilegedReauthenticationFlow.ts}`: (1) **session establishment** via the existing AUTH-01 `createSessionContext` (Firebase remains the token authority — no bespoke token store); (2) the **identity-protected-action gate** (`authorizeIdentityProtectedAction`: resolve via AUTH-02 `resolveAuthenticatedCredential`/`-09` then gate access state — `active`→session; `suspended`→`ACCOUNT_SUSPENDED`; else→`AUTH_FORBIDDEN`; a credential resolving to no identity fails closed `AUTH_REQUIRED`; browsing is never gated); (3) **server-enforced privileged re-authentication** (`authorizePrivilegedAction` + the pure `assertFreshAuthentication`): freshness measured from the trusted `authenticatedAt` against an injected **server** clock, eligible iff `0 ≤ age ≤ maxAgeMs` (default **5 minutes**, configurable per TRD12 §12.29 — Founder decision AD-2), with absent/future/stale authentication failing closed `AUTH_REQUIRED` and the access-state gate running before freshness; and (4) **sign-out** (`signOutCurrentSession`, client-session clear) plus a `reauthenticateForPrivilegedAction` client helper that force-refreshes the token so the backend sees the new `auth_time`. **Additive AUTH-01/AUTH-02 dependency adjustment (Founder decision AD-1, discovered at the AUTH-07 entry gate):** privileged re-auth needs the token's trusted authentication instant, which the merged AUTH-02 verifier did not surface; AUTH-01's `AuthenticatedCredential` gains an **optional, non-breaking** `authenticatedAt?: Date` and the verifier derives it **server-side** from the verified `auth_time` claim, failing closed when absent/malformed — `verifiedAt`/resolution/provider-keying/reference semantics unchanged; `verifiedAt` is never substituted for authentication time. **Emits no domain events** (`CustomerAuthenticated` and `AuthenticationRecoveryProofProvided` stay AUTH-08); no credential material persisted/logged/returned; closed 14-category taxonomy (no new category); no `-08`/`-09`/shared/`index.ts`/`firestore.rules` change; no change to AUTH-03/04/05/06 behaviour except consuming the additive field; no client write path opened. TDD: functions **547/547** (+ AUTH-07 unit tests), web **304/304** (+4 frontend), `pnpm emulators:validate` **216/216** (+4 AUTH-07 emulator tests, no inherited flake recurred); typecheck/lint/format/build/e2e clean; secret scan clean; CI is the authoritative emulator gate. **Implemented, pending Founder-authorized review/merge** (PR opened on this branch); **not** self-merged; **`AUTH-08`+ not started/authorized**; dirty primary worktree untouched; no unrelated worktree cleanup. Capability 2 remains `Open — partially implemented; not closed`; Customer Identity `Complete`; ITM/`ENG-P2-004` unchanged. See the [`AUTH-07` report](reports/AUTH-07-session-access-gating-2026-08-10.md).
