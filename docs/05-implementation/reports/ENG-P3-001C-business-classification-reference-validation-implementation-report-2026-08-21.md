...
## 24. Correction (independent final review, ENG-P3-001C, 2026-08-21) — §14/§19 "permissions/ untouched" overstatement

§14 states "`functions/src/domains/permissions/` is untouched … zero files in that directory changed," and §19's self-review bullet repeats "No permission change: confirmed — `functions/src/domains/permissions/` untouched." Both overstate the actual diff. Two files under `functions/src/domains/permissions/service/` **did** change: `staffInvitation.emulator.test.ts` and `staffMembershipIntegration.emulator.test.ts`, each with an identical `beforeAll` addition that seeds a real, valid Commerce Knowledge fixture (`cat_food`/`ind_test`) via `createKnowledgeNodePersisted`/`transitionKnowledgeNodeStatusPersisted`, guarded by an existence check. §23's own file list already names these two files correctly as "test fixtures only, not production permission code" — the accurate statement is in §23, not §14/§19.

Independently re-verified during this review (diffed directly against `795539a~1`): both changed hunks are fixture-seeding only — no assertion was weakened, no permission/authorization logic (production code under `functions/src/domains/permissions/service/*.ts`, non-test) was touched, and no new permission identifier was introduced. The corrected, accurate statement is: **permission TEST fixtures changed (2 files, seeding-only); permission PRODUCTION logic did not change (0 files).** §14's substantive TOCTOU/authorization-boundary conclusion is unaffected by this correction — it was never based on the incorrect file count, only on direct reading of `authorizeAndExecute.ts`, which is accurate.

This is a wording correction only; no code or governance conclusion changes as a result.
