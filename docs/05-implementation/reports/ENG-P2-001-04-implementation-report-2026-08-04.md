...
**A genuine textual tension found and reconciled — re-examined and confirmed under Founder review, 2026-08-04 (PR #59 held pending this analysis):**

*Exact wording.* `ENG-P2-ARCH-001` §5's Regeneration row (original, preserved in git history): *"Customer-initiated re-issuance of the QR encoding... The underlying `qrReference`/Loyalty Number relationship is unchanged — regeneration produces a new scannable artifact for the same permanent identity, not a new identity."* The same table's Invalidation row: *"A prior QR image ceases to resolve successfully once regeneration completes — old codes must fail closed, not silently continue to work."* `ENG-P2-001-PLAN-001`'s `-04` Scope bullet paraphrases the Regeneration row directly ("new encoding, same underlying reference — `ENG-P2-ARCH-001` §5"), while its own Tests-required and Rollback-boundary lines require "a new resolvable reference" and state "the prior reference fails to resolve."

*Locus of the tension.* Not a cross-document conflict: `ENG-P2-001-PLAN-001`'s `-04` section lists `ENG-P2-ARCH-001` §5 under its own "Governing requirements" field, so the Plan is explicitly subordinate to and derived from the Architecture document, not a competing source. The tension is internal to `ENG-P2-ARCH-001` §5 itself — between its own Regeneration row ("relationship... unchanged") and its own Invalidation row ("old codes must fail closed") — which `ENG-P2-001-PLAN-001` faithfully inherited by paraphrasing both.

*Resolution, forced by internal logical necessity, not preference.* `ENG-P2-ARCH-001` §5's own Invalidation row requires a prior value to stop resolving. That requirement is unimplementable if the reference literally never changes — a QR image encoding a reference that "never changes" is, by definition, still valid, and cannot "fail closed." The only reading that keeps `ENG-P2-ARCH-001` §5 internally self-consistent is: "relationship unchanged" refers to the customer↔identity↔loyalty-number **association** (exactly one QR slot per identity, always mapped to the same Loyalty Number) — not literal identity of the reference **value**, which is exactly what regeneration replaces, and what invalidation retires. This reading is further supported by TRD10's schema (`qrReference: string` — one field per customer profile, consistent with a stable slot whose *value* can be reassigned) and by `PRD2` §9's "regenerate if required" (meaningless if regeneration changed nothing observable).

**Regeneration therefore produces a genuinely new `QrReference` value; the Customer Identity ID and Loyalty Number never change; the prior reference is marked `invalidated`, never deleted.** `restoreQrIdentityForRecovery` is unaffected by this reading either way — it returns the current active association unchanged regardless of how many regenerations preceded it.

**Determination (Founder-reviewed):** this required a documentation correction, not a code correction and not a Founder policy decision — the code already implemented the only internally-consistent reading. `ENG-P2-ARCH-001` §5's Regeneration row and `ENG-P2-001-PLAN-001`'s `-04` Scope-bullet paraphrase have both been corrected using the amendment-in-place pattern (original wording preserved in git history, not deleted). No test or code change was required.
