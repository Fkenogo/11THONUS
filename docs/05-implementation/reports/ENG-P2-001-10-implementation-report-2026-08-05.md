...
> **Superseded 2026-08-05 — see [§38 Correction](#38-correction-2026-08-05--founder-review-audit-read-model-minimisation).** This section's original conclusion — that Loyalty Number/QR reference exposure in audit results was an acceptable, already-governed exception — was overridden by Founder review before merge. Audit-QUERY results (not the underlying stored events, which are unchanged) no longer carry these values at all.
...
## 38. Correction (2026-08-05) — Founder Review: Audit Read-Model Minimisation

Founder review of PR #65 positively reviewed the implementation but withheld merge authorisation on one confirmed defect: condition 6 of the merge-verification checklist ("no raw phone number, email, Loyalty Number, QR reference, Authentication subject reference, credential, token, OTP, profile data, purchase data, or reward data is exposed through audit results") failed — `toIdentityAuditRecord` passed `event.payload` straight through, so `queryAuditRecordsByEventType(db, "loyaltyNumber.loyalty_number_issued.v1", authority)` returned the raw Loyalty Number, and the equivalent QR/Authentication Reference/recovery-proof events leaked their own raw identifiers the same way. This section records the resulting **Audit Read-Model Minimisation Principle** and the correction applying it.

**38.1 Principle (Founder-directed, verbatim intent).** The underlying domain events and outbox records retain governed operational identifiers required for processing and immutable audit evidence — unchanged by this correction. Audit-QUERY results must not expose raw Loyalty Numbers, QR references, Authentication subject references, phone numbers, emails, credentials, tokens, OTPs, or other sensitive/customer-linked identifiers unless a future explicitly governed audit purpose requires them. No masking, truncation, or unsalted hashing substitutes for omission, since each of these identifier spaces is enumerable.

**38.2 Final audit projection model.** A new `functions/src/domains/identityAudit/models/auditPayloadProjection.ts` exports `projectAuditPayload(eventType, rawPayload): AuditSafePayload` — an explicit per-event-name allow-list (`pick()` of only named fields from the raw payload), never a pass-through. `toIdentityAuditRecord` (`auditEnvelope.ts`) now calls this function instead of assigning `event.payload` directly; `IdentityAuditRecord.payload` is typed `AuditSafePayload` (`Record<string, unknown>`) instead of `unknown`. Unrecognised event types fail closed: `{ payloadOmitted: true }`, never an arbitrary object — this is itself bounded and typed, not a silent pass-through.

**38.3 Event-by-event payload mapping (the audit payload catalogue).**

| Event | Raw payload fields | Audit-projected fields | Omitted (raw, enumerable) |
|---|---|---|---|
| `customer_identity_registered` | `customerIdentityId` | *(none — `customerIdentityId` already top-level on the envelope)* | — |
| `customer_identity_activated` / `_suspended` / `_locked` / `_closed`, `identity_became_dormant` | `customerIdentityId`, `previousStatus`, `authority`, `reason` | `previousStatus`, `authority`, `reason` | — |
| `customer_identity_archived` | `customerIdentityId`, `authority`, `reason` | `authority`, `reason` | — |
| `identity_recovered` | `customerIdentityId`, `previousStatus`, `resultingStatus`, `authority`, `reason`, `recoveryProofReference`, `proofMethodCategory` | `previousStatus`, `resultingStatus`, `authority`, `reason`, `proofMethodCategory` | `recoveryProofReference` |
| `authentication_reference_linked` | `customerIdentityId`, `referenceId`, `referenceType`, `authority`, `reason` | `referenceType`, `authority`, `reason` | `referenceId` |
| `authentication_reference_unlinked` | `customerIdentityId`, `referenceId`, `authority`, `reason` | `authority`, `reason` | `referenceId` |
| `authentication_reference_conflict_detected` | `customerIdentityId`, `referenceType`, `authority`, `reason` | `referenceType`, `authority`, `reason` | — *(no raw reference ever present, unchanged)* |
| `identity_lookup_attempted` | `customerIdentityId`, `lookupType`, `purpose`, `outcome` | `lookupType`, `purpose`, `outcome` | — *(no raw lookup value ever present, unchanged from `-09`)* |
| `loyalty_number_issued` | `customerIdentityId`, `loyaltyNumber` | *(none)* | `loyaltyNumber` |
| `qr_identity_issued` | `customerIdentityId`, `qrReference` | *(none)* | `qrReference` |
| `qr_identity_invalidated` | `customerIdentityId`, `qrReference` | *(none)* | `qrReference` |
...
**38.4 Distinguishing immutable evidence from the audit-query projection.** The underlying `outboxEntries` document (the immutable stored event, unchanged) still contains the raw Loyalty Number/QR reference/etc. exactly as issued — that is the audit *evidence of record*, per TRD11's own outbox architecture. `IdentityAuditRecord.payload` (this package's own audit-*query* read model) is a privacy-minimised derivative of that evidence, never itself persisted, computed fresh on every query. A new emulator test proves both halves of this distinction directly: it reads the raw `outboxEntries` document (still contains `"ABC-234"`) and the query result (does not) in the same assertion.

**38.5 Code changes.** New: `functions/src/domains/identityAudit/models/auditPayloadProjection.ts` (+ `.test.ts`, 10 tests). Modified: `auditEnvelope.ts` (`payload: event.payload` → `payload: projectAuditPayload(event.eventType, event.payload)`; `IdentityAuditRecord.payload` retyped `AuditSafePayload`); `auditPrivacyClassification.ts` (`extractEventName` exported, reused by the new projection module rather than duplicated).

**38.6 Tests changed.** `auditEnvelope.test.ts` — updated the existing baseline test's expected `payload` to the projected shape, added a dedicated projection-integration test. `identityAuditQueryRepository.emulator.test.ts` — new `describe("Audit Read-Model Minimisation Principle (Founder correction)")` block, 5 tests: Loyalty Number issuance (raw value absent from the query result, present on the raw stored document); QR issued/invalidated/regenerated (no current, old, or new reference); Authentication Reference linked/unlinked (no raw subject reference, `referenceType`/`authority`/`reason` still present); recovery (no raw proof reference, `proofMethodCategory` still present); unrecognised event type (fails closed to `{ payloadOmitted: true }`). Two test-authoring defects were caught and fixed before these were accepted as valid: three new seed events omitted an explicit `aggregateId` override, so `queryAuditRecordsByCustomerIdentityId` silently returned zero records for two of the five new tests (hard failures) and, for a third, made the assertion vacuously true against an empty result array — every one of these five tests now asserts a specific non-zero `result.records` length before asserting on content, closing that vacuous-pass risk.

**38.7 Validation after correction.** `npx tsc --noEmit` clean. `pnpm lint` clean. `pnpm format:check` clean. Full `functions` unit suite: 53 files / 395 tests passed (385 pre-existing + 10 new). Real Firebase Emulator Suite: 12 files / 164 tests passed (159 pre-existing + 5 new), clean on first run. `apps/web`: 259/259 — one pre-existing, already-disclosed timing flake on a first attempt, confirmed transient on immediate clean retry; `apps/web` is untouched by this correction. `pnpm build` clean for both workspaces.

**38.8 Merge status.** PR #65 remains unmerged. This correction was committed to the existing `feat/eng-p2-001-10-identity-audit-observability` branch, not a new branch. Merge remains withheld pending fresh, separate Founder authorisation.
