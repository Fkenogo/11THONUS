...
| **2** | `QualifyingItem` domain: model, repository, validation, **permission**, authorization, callables | `rewardProgram/models/rewardProgram.ts`; new `repositories/qualifyingItemRepository.ts`; `services/rewardProgramKnowledgeValidation.ts`; new `services/qualifyingItemCommands.ts`; **new `permissions/models/qualifyingItemPermissionCatalogue.ts`**; **`permissions/evaluator/evaluatePermission.ts` (Step 5d branch)**; `index.ts` (create/list/rename/retire callables + parsers) | none | A, B, C, F(app), G(app), U, V, **Y, Z, AA** (§17 amendment) | 1 | No |
...
**No new governance package is proposed.** `DEC-LOY-016` and `DEC-LOY-017` together are sufficient authority for slices 1–5; slice 7 needs an operational verification, not a decision.

> **Amendment 001 — slice 2 permission scope (`DEC-LOY-017`).** FQ-1 is closed (§17). Slice 2 additionally delivers the permission-model amendment determined below. The **smallest coherent** amendment, derived from the existing catalogue's own conventions rather than guessed:
>
...
>
> Net effect: slice 2 gains one new catalogue module, one evaluator branch, and their tests. No other slice changes. **Design recommendation only — not implemented by this report or its amendment.**
