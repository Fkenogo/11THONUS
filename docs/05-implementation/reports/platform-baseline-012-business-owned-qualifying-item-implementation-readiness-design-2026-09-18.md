...
>
> **Amendment 001 (2026-09-18, `PLATFORM-BASELINE-012-FD-ROLE-CLOSE-001`):** the Founder answered this report's §17 **FQ-1**. Recorded as [`DEC-LOY-017`](../../00-governance/decisions/decision-register.md) / [`FD-QUALIFYING-ITEM-ROLE-AUTHORITY-001`](../../00-governance/decisions/evidence/FD-QUALIFYING-ITEM-ROLE-AUTHORITY-001-founder-decision-2026-09-18.md). Sections amended in place: **§16** (the permission scope of what CORR-001 later renamed PB-013A.2, and the governance note), **§17** (FQ-1 closed), **§17A** (new — the four directly-inspected findings restated as explicit carried-forward `PLATFORM-BASELINE-013` requirements), **§25** (risk 4 retired), and the **Final Disposition**. No other section changed. The amendment is documentation only: **no permission catalogue, evaluator, code, schema, or migration change was made, and `PLATFORM-BASELINE-013` is not authorized to begin.**
>
> **Correction 001 (2026-09-18, `PLATFORM-BASELINE-012-CORR-001`):** an independent review of PR #259 (disposition A — independently verified/approvable) raised exactly one **P2**: the delivery sequence in §16 placed a purchase-request contract change before its consuming UI, so `main` would have been incoherent between those two merges — contradicting §16's own coherence claim. **§16 is rewritten** (lettered `PB-013A.1 / A.2 / B / C / D / E` packages with an explicit atomic-merge-boundary rule and a dependency chain), **§16A is new** (root-cause analysis, disclosing a *second* instance of the same grouping error on the Reward Program configuration path that the review did not name), and §11, §12, §15, §17, §19, §26 and the Final Disposition are updated for the renumbering. **The product architecture is unchanged:** `DEC-LOY-016` and `DEC-LOY-017` are untouched, `qualifyingItemId` remains the final structural purchase identity (not weakened to solve sequencing), `itemLabel` remains a display snapshot and never qualification authority, the server still resolves the applicable programme/version, and Commerce Knowledge remains optional classification only. **No transitional compatibility mode was introduced.** Sequencing only — no implementation file, schema, migration, permission, or governance record was changed.
...
2. Each added item shows as a row with its name, a rename affordance, and a "retire" affordance. Optionally (PB-013D, §16) a collapsed "Classify (optional)" disclosure per item wrapping the re-scoped `QualifyingNodeSelector`.
...
**Added by Amendment 001 (`DEC-LOY-017`), permission-model cases:**

| Case | Assertion | Suite |
|---|---|---|
| Y | `qualifyingItem.manage` resolves `allowed` for `owner` and for `manager`, and `denied` (`NO_APPLICABLE_GRANT`/`AUTH_FORBIDDEN`) for `staff`; eligible only in `trial`/`active` Business status; and — mirroring the existing reward-program catalogue test — a grant override is never honoured for it. | `evaluatePermission.test.ts` |
| Z | **`rewardProgram.manage` is unchanged: still `{owner: true, manager: false, staff: false}`.** A Manager must still be denied `createRewardProgram`, `updateRewardProgramDraft`, `createNextRewardProgramVersion`, and `publishRewardProgramVersion`. This is the regression gate against the tempting shortcut of widening the existing permission instead of adding a new one (§25 risk 4). | `evaluatePermission.test.ts` + `rewardProgramCommands.postgres.test.ts` |
| AA | The Platform Administrator boundary holds: no platform-admin identity can satisfy `qualifyingItem.manage` for a Business it holds no membership role in. Asserts the property `DEC-LOY-017` clause 3 requires, which is currently satisfied by construction (`Role` is the closed owner/manager/staff union; `evaluatePermission.ts` has no platform-admin path) and must stay true. | `evaluatePermission.test.ts` |
| AB | Staff can still read and select configured qualifying items via the membership-gated `listRewardPrograms` read, and can still record a purchase (`purchase.record` unchanged), while being denied every qualifying-item management callable. Proves `DEC-LOY-017` clause 2 end to end. | `index.test.ts` + `purchaseCommands.postgres.test.ts` |
| AC | The module-load-time cross-catalogue uniqueness invariant fires if `qualifyingItem.manage` were ever also claimed by the sensitive, ordinary, reward-program, or purchase catalogue — mirroring the existing invariant tests on the other catalogue modules. | permissions model suite |
...
> **CORRECTION 001 (2026-09-18, `PLATFORM-BASELINE-012-CORR-001`) — this section was rewritten.** An independent review of PR #259 raised one **P2**: the original seven-numbered-slice sequence below asserted that `main` stays coherent after every merge, but it did not. The original sequence is superseded in full by the lettered sequence that follows. See **§16A** for the root-cause analysis. The product architecture, `DEC-LOY-016`, `DEC-LOY-017`, and the final Phase 1 contract (`qualifyingItemId` as structural purchase identity) are **unchanged** — only the delivery sequence is corrected.

**Recommendation: three delivery packages (plus two deferrable), organised so that every merge boundary leaves `main` coherent — including for every already-shipped caller.**

The governing rule, which the superseded sequence violated:

> **No merged slice may make `qualifyingItemId` (or any other new field) mandatory on a shipped request path before every shipped caller of that path has been updated in the same merge.** A backend contract change and its consuming client/UI contract change are **one atomic merge boundary**. Internal commits within a slice may be smaller; `main` must never receive an intermediate incompatible contract.

| Package | Objective | Backend files / domains | Shipped callers updated in the same merge | Migration impact | Test gate | Depends on | Coherent `main` after merge? | Founder preview |
|---|---|---|---|---|---|---|---|---|
| **PB-013A.1** | Schema foundation, purely additive | `migrations/0016_*`, `0017_*` (+ `.down.sql`); `rewardProgramMigrations.postgres.test.ts` | **None needed** — no contract exists yet; nothing reads or writes the new tables | `0016`, `0017` (incl. backfill) | K; full migration up/down/idempotent-rerun suite | — | **Yes** — additive only; no existing caller can observe the change | No (invisible) |
| **PB-013A.2** | `QualifyingItem` domain: model, repository, validation, **permission**, authorization, new callables | new `repositories/qualifyingItemRepository.ts`; new `services/qualifyingItemCommands.ts`; new `permissions/models/qualifyingItemPermissionCatalogue.ts`; `permissions/evaluator/evaluatePermission.ts` (Step 5d branch); `index.ts` (**new** create/list/rename/retire callables + their parsers) | **None needed** — every callable added here is brand new; **no existing request contract is altered**, and `rewardProgram.manage` shows zero diff | none | A, B, C, F(app), G(app), U, V, **Y, Z, AA, AB, AC** | A.1 | **Yes** — strictly additive surface | No |
| **PB-013B** | Reward Program ↔ Qualifying Item configuration/binding — **atomic backend + its consuming UI** | `rewardProgram/models/rewardProgram.ts`; `repositories/rewardProgramRepository.ts`; the four `*Command.ts`; `services/rewardProgramKnowledgeValidation.ts`; `index.ts` reward-program parsers (`parseQualifyingNodes` → `parseQualifyingItems`) | **`RewardProgramManagementPage.tsx`; the new qualifying-items section; `api/rewardProgramMutations.ts` (`QualifyingNodeWire` → `QualifyingItemWire`); `hooks/*`; `queryKeys.ts`; EN/FR locales; the reward-program UI test files** — all in this same merge | none (reads/writes the `0017` tables) | D, E, H, I, J, U, V, W, and the reward-program half of X + full `rewardProgram` and `apps/web` suites | A.2 | **Yes** — the reward-program write contract and its only shipped callers move together; **the purchase path is not touched at all**, so every existing purchase caller keeps working unchanged | **Yes** — a Business can define "Black Coffee" and publish, end to end |
| **PB-013C** | **The purchase-contract transition — ONE atomic merge boundary** | `migrations/0018_*`; `purchase/models/purchase.ts`; `repositories/purchaseRecordRepository.ts`; `services/recordPurchaseCommand.ts` (validation + server-resolved programme/version eligibility + persistence); `services/purchaseRequestHash.ts` (**idempotency identity**); the `purchase.recorded` **Trust Event payload** (evidence identity); `index.ts::parseRecordPurchaseRequest` (transport contract) | **`PurchaseRecordsPage.tsx`; `api/purchaseMutations.ts` (`RecordPurchaseRequest`/`PurchaseRecordWire`); `hooks/purchaseMutations`; EN/FR locales; `PurchaseRecordsPage.test.tsx`; `purchaseMutations.test.ts`** — all in this same merge, plus any `tests/e2e` spec touching this path | `0018` (additive column + `NOT VALID`→`VALIDATE` FKs) | L, M, N, **N2**, O, P, Q, R, S, T, the purchase half of X, + the §15 zero-diff gate + full `purchase`, `index`, and `apps/web` suites | B | **Yes — by construction.** The transport contract and every shipped caller of it change in the same merge. There is no boundary at which the backend requires `qualifyingItemId` while the shipped UI still sends the old shape | **Yes** — highest-risk package |
| **PB-013D** | *(Optional / deferrable)* Re-scoped optional classification picker | — | `QualifyingNodeSelector.tsx` (multi→single select, attached per item) | none | U, V, W re-run; PB-008/PB-010B retain-row regressions (§14 rows 4–9) | C | **Yes** — UI-only, additive affordance | Optional |
| **PB-013E** | *(Deferred, environment-gated)* Legacy cleanup | `migrations/0019_*` | none | `DROP TABLE` + optional `SET NOT NULL` | verified row counts in the target environment (§13) | D, **plus environment verification** | **Yes** — by then nothing reads the dropped table | No |

**Dependency chain (linear, no parallel merges into `main`):**

```
PB-013A.1 ──▶ PB-013A.2 ──▶ PB-013B ──▶ PB-013C ──▶ PB-013D ──▶ PB-013E
 additive      additive      atomic:      ATOMIC:     UI-only     deferred,
 migrations    surface       RP backend   purchase    optional    env-gated
 only          only          + RP UI      backend
                                          + purchase
                                          UI + idempotency
                                          + Trust Event
       ▲ coherent   ▲ coherent   ▲ coherent   ▲ coherent   ▲ coherent   ▲ coherent
```

**Invariants preserved by the corrected sequence (none weakened to solve sequencing):**

- **`qualifyingItemId` remains the final Phase 1 structural purchase identity**, mandatory on the request path from the moment PB-013C merges. It is *not* softened to optional, and no permanent dual-authority state is created.
- **`itemLabel` remains exactly what §7 Q8/Q9 specified** — a server-derived, frozen, human-readable transaction-time display snapshot (column retained `NOT NULL`, no `ALTER`). It is removed from the transport whitelist in PB-013C and is **never again qualification authority**.
- **The server continues to resolve the applicable Reward Program and version** under lock from `program.currentVersionId` (§7 Q3). PB-013C adds no client-supplied version id and grants the client no programme/version-selection authority (CF-2, §17A).
- **Idempotency identity moves inside PB-013C**, not after it: `purchaseRequestHash`'s fingerprint gains `qualifyingItemId` and drops `itemLabel` in the same merge that makes `qualifyingItemId` authoritative, so there is no window in which the fingerprint is structurally blind to the qualifying item (CF-3, §17A; gated by test N2).
- **Trust Event / evidence identity is explicit in PB-013C**, in the same merge: the `purchase.recorded` payload gains the opaque `qualifyingItemId` reference, preserving the existing Trust Event architecture (unchanged `event_type`, `subject_type`, `subject_id`, causal linkage, in-transaction emission) and the platform's data-minimisation posture (CF-4, §17A; gated by test S).
- **Commerce Knowledge remains optional classification only** throughout — no package reintroduces it as a qualification gate.

**No transitional compatibility mode is introduced, and none is needed.** Requirement-4 test: an atomic PB-013C is practical in this repository, not merely preferable. It spans roughly eight backend files and five web files plus tests — smaller than `PLATFORM-BASELINE-008`, which shipped three new Commerce Knowledge read services, three new callables, three web adapters/hooks, a new component, and a page rewrite in a single PR (#255), and comparable to `PLATFORM-BASELINE-010B` (#257), which changed transport, domain validation, schema, and UI together. Both are direct precedents for a backend + consuming-client change landing as one coherent merge in this codebase. A dual-accept transitional mode would additionally require a temporary state in which the server accepts a purchase with **no** structural qualifying-item binding — reintroducing, for the duration, precisely the anti-fabrication gap §7/§10 exist to close. It is therefore rejected on trust grounds as well as on practicality.

**No new governance package is proposed.** `DEC-LOY-016` and `DEC-LOY-017` together remain sufficient authority for PB-013A–C; PB-013E needs an operational verification, not a decision. **This correction changes sequencing only — it alters no architectural conclusion, no entity model, no binding design, no schema, and no governance record.**

## 16A. Root cause of the superseded sequence's incoherence (CORR-001)

Recorded so the defect class is not reintroduced. Verified directly against the shipped code on `b2f1fda`, not inferred from the review note.

**The defect.** The superseded sequence placed every backend contract change in one slice and *all* web/UI changes together in a later slice 5. That grouping is organised by layer, not by contract. Any contract has callers; splitting a contract from its callers across a merge boundary breaks `main` at that boundary regardless of how correct each side is on its own.

**Instance 1 — the purchase path (the reported P2).** Superseded slice 4 changed `index.ts::parseRecordPurchaseRequest` to require `qualifyingItemId` and to drop `itemLabel` from the whitelist (§7 Q10, §10). Its only shipped caller chain — `PurchaseRecordsPage.tsx` → `hooks/purchaseMutations` → `api/purchaseMutations.ts`, which today sends `{rewardProgramId, artifact, quantity, itemLabel, purchaseDate, notes}` and no `qualifyingItemId` — was updated only in superseded slice 5. Between those two merges, `parseNonEmptyString(value.qualifyingItemId)` would throw on **every** purchase recorded from the shipped UI. Frontline purchase recording, the platform's core transaction, would be entirely broken on `main`. This directly contradicted this section's own former claim that "`main` is coherent and shippable after each."

**Instance 2 — the Reward Program configuration path (same root cause, found by this correction, not in the review note).** Superseded slice 3 cut the reward-program write contract over from `QualifyingNode` to qualifying-item references, including `index.ts`'s `parseQualifyingNodes`. Its shipped callers — `RewardProgramManagementPage.tsx`, `QualifyingNodeSelector.tsx`, `api/rewardProgramMutations.ts` (`QualifyingNodeWire`), `hooks/*`, `queryKeys.ts` — were likewise updated only in slice 5. Between slice 3 and slice 5, Reward Program create/edit/publish from the shipped UI would fail the same way. Disclosed here in full rather than silently folded in: the review identified one instance; the underlying grouping error produced two.

**The corrected rule** (stated once, applied to both instances above and binding on PB-013): *a request contract and every shipped caller of that contract cross a merge boundary together.* PB-013B satisfies it for the reward-program path; PB-013C satisfies it for the purchase path; PB-013A.1/A.2 satisfy it trivially by changing no existing contract at all.

**Why PB-013B cannot break existing purchase callers.** PB-013B touches no file under `functions/src/domains/purchase/**`, does not alter `parseRecordPurchaseRequest`, and does not run migration `0018` — `purchase_records` gains no column and `qualifying_item_id` does not yet exist. A purchase recorded between PB-013B and PB-013C follows exactly today's path, binding to the version as a whole with a free-text `itemLabel`, which is the pre-existing (and separately disclosed, §4/§7) state of affairs and is not worsened by this design.

> **Amendment 001 — PB-013A.2 permission scope (`DEC-LOY-017`).** FQ-1 is closed (§17). PB-013A.2 (superseded slice 2, renumbered by CORR-001) additionally delivers the permission-model amendment determined below. The **smallest coherent** amendment, derived from the existing catalogue's own conventions rather than guessed:
>
> - **A fifth, structurally separate catalogue module**, `functions/src/domains/permissions/models/qualifyingItemPermissionCatalogue.ts`, holding exactly one entry `qualifyingItem.manage` with `roleDefaults = {owner: true, manager: true, staff: false}` and `eligibleBusinessStatuses = ["trial", "active"]` (matching both existing domain catalogues), plus the same module-load-time cross-catalogue uniqueness invariant the other modules already enforce.
> - **A Step 5d branch in `evaluatePermission.ts`**, a structural copy of the existing Step 5b (reward-program) and Step 5c (purchase) branches — no new authorization algorithm, no override/inheritance path, mirroring `classifyPermission`'s existing shape.
> - **`rewardProgram.manage` shows zero diff.** It stays Owner-only. Widening it was rejected because that one permission also gates `createRewardProgram`, `updateRewardProgramDraft`, `createNextRewardProgramVersion`, and `publishRewardProgramVersion` through `authorizeRewardProgramManage` — widening it would hand Managers Reward Program creation and publication authority, which `DEC-LOY-017` does not grant and explicitly withholds, and would reopen the closed `PLATFORM-BASELINE-005-FOUNDER-DISPOSITION-001` FD-2 instrument for an unrelated purpose. Both existing catalogue module headers warn against exactly that move in their own words; creating a new disjoint module is how the third and fourth catalogues each handled the identical situation.
> - **Staff view/select needs no catalogue entry.** §7 Q2 established that the frontline picker reads the Business's configured items through the existing `listRewardPrograms` callable, gated by `authorizeRewardProgramRead` — membership-only by explicit existing design. Staff therefore already hold exactly the view-and-select capability `DEC-LOY-017` grants and nothing more. `purchase.record` (Staff/Manager/Owner) continues to gate the recording action itself, unchanged.
> - **The Platform Administrator boundary is already satisfied by construction.** `Role` is the closed union `"owner" | "manager" | "staff"` (`role.ts`) and `evaluatePermission.ts` contains no platform-administrator branch, bypass, or cross-Business path at all. Preserving `DEC-LOY-017` clause 3 requires **no change** — only a regression test asserting it stays true.
> - **No ad-hoc UI-only role check.** Control visibility in the Business dashboard remains convenience; `evaluatePermission` remains the sole authority, consistent with this codebase's existing posture throughout.
>
> Net effect: PB-013A.2 gains one new catalogue module, one evaluator branch, and their tests. No other package changes. Every callable it adds is new, so it alters no existing request contract and merges coherently on its own (§16). **Design recommendation only — not implemented by this report or its amendment.**

## 17. Founder/product questions discovered

Raised per the task spec's STOP instruction. **FQ-1 is now CLOSED (Amendment 001). FQ-2, FQ-3, and FQ-4 remain open; none blocks slices 1 or 3–5 outright.**

**FQ-1 — ✅ CLOSED 2026-09-18 by [`DEC-LOY-017`](../../00-governance/decisions/decision-register.md) / [`FD-QUALIFYING-ITEM-ROLE-AUTHORITY-001`](../../00-governance/decisions/evidence/FD-QUALIFYING-ITEM-ROLE-AUTHORITY-001-founder-decision-2026-09-18.md).**
**Founder answer:** Business Owner **and** Business Manager may create, rename/edit, archive, and optionally classify Qualifying Items. Business Staff may view and select configured items as necessary for authorised frontline transaction recording, but may not manage them. Platform Administrators receive no routine authority over a Business's Qualifying Items by virtue of platform-admin status. Management must not blindly inherit the Owner-only `rewardProgram.manage` permission where that would exclude Managers; the existing permission architecture must be used with the smallest coherent amendment determined from the catalogue's own conventions; and no ad-hoc UI-only role check may substitute for server-side permission evaluation.
**How this changes the permission design written below:** the original engineering default recorded here — reuse `authorizeRewardProgramManage`, accepting Owner-only — **is superseded and must not be implemented**. The determined replacement (a distinct `qualifyingItem.manage` permission in its own fifth catalogue module, with `rewardProgram.manage` left Owner-only and unchanged) is specified in §16 Amendment 001. Staff view/select turns out to need no new permission at all, and the Platform Administrator boundary is already satisfied by construction — both established by direct inspection, not assumed. **This remains a design recommendation; nothing was implemented.**
*Original question, retained verbatim as the historical record:* `rewardProgramPermissionCatalogue.ts` grants `rewardProgram.manage` as `{owner: true, manager: false, staff: false}` — **Owner-only**, with an explicit test asserting that no grant override can widen it. `purchase.record`, by contrast, is Staff/Manager/Owner (and its own catalogue comment calls out the deliberate contrast). Reusing `authorizeRewardProgramManage` — the safe, non-inventive default, and what PB-011 §L recommended — therefore makes **only the Owner** able to add "Cappuccino" to the menu. For a coffee shop or a restaurant whose offer changes weekly, that may be the wrong operational shape, and a Manager-capable item catalogue is a plausible product intent. This is **not derivable** from any recorded decision: `FD-REWARD-QUALIFYING-ITEM-001` §2.6 says only "the Business remains responsible for defining its commercial offer," which does not resolve which role inside the Business. *Engineering default if unanswered:* reuse `rewardProgram.manage` (Owner-only), which is non-regressive and can be widened later by a catalogue change with no schema impact. *Founder decision required to widen it*, because the permission catalogue is a governed artifact.
...
This design removes the `itemLabel` `TextField` and routes per-transaction detail to the existing optional `notes` field (§12 step 8). That is a genuine (if small) change to what an operator can record. It is safely derivable — `notes` already exists, is already optional, and is already unvalidated — but it is a visible workflow change worth confirming at the **PB-013C** Founder preview (§16) rather than discovering after merge.
...
## 17A. Carried-forward `PLATFORM-BASELINE-013` implementation requirements (Amendment 001)

*Added by `PLATFORM-BASELINE-012-FD-ROLE-CLOSE-001`.* Each item below was established by **direct inspection of the shipped code on `b2f1fda`**, is already reflected in the body of this report (§4, §7, §10, §14), and is restated here as an **explicit, unresolved, carried-forward requirement** so it cannot be lost between packages. **None is implemented, fixed, or resolved by this report or its amendment.** They bind the future, not-yet-authorized `PLATFORM-BASELINE-013`.

**CF-1 — `purchase_records.knowledge_node_id` must not be promoted to QualifyingItem authority merely because it already exists.**
*Finding (§4, §14 row 17):* the column is unreachable from every shipped UI — `PurchaseRecordsPage.tsx` never sends it — so it is settable only by direct callable invocation; when present it is validated standalone against Commerce Knowledge and is **never** cross-checked against the Reward Program version's own qualifying set. It is a Firestore-owned canonical classification reference, not a Business-owned item identity, and `DEC-LOY-016` makes that classification optional. *Requirement:* PB-013 must introduce a distinct `qualifying_item_id` as the structural authority and must **not** repurpose, alias, or overload `knowledge_node_id` to carry it. The existing column is retained untouched (no `ALTER`) while its transport parse, its Firestore validation call, and its insert parameter are removed from the write path. **Status: carried forward, not resolved.**

**CF-2 — The applicable Reward Program and version remain server-resolved; clients must gain no authority to select or override them.**
*Finding (§4, §7 Q3):* `recordPurchaseCommand.ts` resolves the applicable version itself, inside the transaction, from `program.currentVersionId` after `lockRewardProgramById`, then locks it and asserts `status = 'active'`. `parseRecordPurchaseRequest` is a deliberate mass-assignment whitelist that structurally cannot accept a version id, and `DEC-PROD-014` fixes a purchase permanently to its creation-time version. *Requirement:* PB-013 must not add a client-supplied `rewardProgramVersionId`, must not let a client's cached view of the qualifying set substitute for the in-transaction membership check against the **locked** version, and must not infer a qualifying item server-side from "the only one the client could have meant." A client holding a stale item list must fail the check, never bind to a stale version. **Status: carried forward, not resolved.**

**CF-3 — Idempotency identity must be reassessed when `qualifyingItemId` becomes transaction authority.**
*Finding (§7, §10):* `recordPurchaseCommand.ts` builds its request fingerprint — the input to `purchaseRequestHash`, and therefore the identity of an idempotent request — from a field list that currently includes `itemLabel` and `knowledgeNodeId`. *Requirement:* PB-013 must add `qualifyingItemId` to that fingerprint and remove `itemLabel` from it in the same change. If it does not, two genuinely different purchases differing **only** in qualifying item produce an identical `requestHash`, so a client reusing an idempotency key receives the first purchase's snapshot back as a "duplicate" — silently recording the wrong item — or draws an incorrect `conflict` verdict. This failure is **item-blind, silent, produces no compile error, and no existing test catches it**; test N2 (§15) exists specifically to gate it. **Status: carried forward, not resolved.**

**CF-4 — Transaction and trust evidence must preserve meaningful item identity.**
*Finding (§4, §10):* the `purchase.recorded` Trust Event payload carries `rewardProgramId`, `rewardProgramVersionId`, `quantity`, and `presentedArtifactType` — **no item identity of any kind** — while `purchase_records.dispute_reason` already offers `'wrong_item'` as a customer verdict with nothing structured to dispute against. *Design intent to carry forward:* `qualifyingItemId` is the **structural, canonical, Business-owned item reference**; `itemLabel` is the **human-readable transaction-time snapshot** (server-derived, frozen, surviving rename and retirement). *Requirement:* PB-013 must evaluate how `purchase.recorded` should carry qualifying-item identity while preserving the existing Trust Event architecture (unchanged `event_type`, `subject_type`, `subject_id`, causal linkage, and in-transaction emission — migration `0013` imposes no `CHECK` on payload contents, so an additive JSONB payload field is schema-free) **and** the platform's privacy/data-minimisation conventions — the recommendation in §10 is to carry the opaque `qualifyingItemId` reference rather than duplicating customer- or commercially-sensitive detail into the event payload. This is an evaluation requirement, not a settled answer. **Status: carried forward, not resolved.**

**None of CF-1 through CF-4 is touched, fixed, or implemented now.** They are requirements on a future package that is not authorized to begin.
...
**Amendment 001 (`PLATFORM-BASELINE-012-FD-ROLE-CLOSE-001`, second commit on the same branch — this repository's convention is always a new commit, never an amend) additionally changed exactly five files, all documentation:**

1. `docs/00-governance/decisions/evidence/FD-QUALIFYING-ITEM-ROLE-AUTHORITY-001-founder-decision-2026-09-18.md` — created, following `FD-REWARD-QUALIFYING-ITEM-001`'s structural convention exactly.
2. `docs/00-governance/decisions/decision-register.md` — new `DEC-LOY-017` row inserted after `DEC-LOY-016` in the Loyalty section; Register Summary updated (CONFIRMED 65→66, Total 117→118); header "Last controlled update"/"Prior update" chain extended. No other register entry modified.
3. `docs/00-governance/requirements-traceability-matrix.md` — `FR-RP-002` row only: `Related Decision IDs` gains `DEC-LOY-017`, `Notes` annotated. No other row touched; the RTM header was deliberately left unchanged, matching `PLATFORM-BASELINE-011`'s own treatment of this file.
4. This report — §16, §17, §17A (new), §15 (cases Y–AC), §25 risk 4, §19, and the Final Disposition.
5. `docs/00-governance/documentation-changes-log.md` — Entry 240 and the header chain.

Still zero files under `functions/`, `apps/`, `scripts/`, `tests/`, any migration directory, any dependency manifest, or any config file.

**Correction 001 (`PLATFORM-BASELINE-012-CORR-001`, third commit on the same branch — again a new commit, not an amend) changed exactly two files, both documentation:**

1. This report — header banner; **§16 rewritten**; **§16A added**; §11, §12, §15, §17 (FQ-3 preview reference), §19, §26 (rollback order) and the Final Disposition updated for the corrected package names. No other section altered; no architectural conclusion, entity model, binding design, schema specification, test case, or governance citation changed.
2. `docs/00-governance/documentation-changes-log.md` — Entry 241 and the header chain.

`DEC-LOY-016`, `DEC-LOY-017`, both founder-decision evidence files, the decision register, and the RTM were **not** touched by CORR-001. Still zero implementation, schema, migration, permission, or seed files across all three commits.
...
3. **The idempotency fingerprint change (§7) is easy to miss and silently wrong if missed** — it produces no compile error and no failing existing test. Mitigated by making test N2 a required gate in **PB-013C** — the same atomic merge that makes `qualifyingItemId` authoritative also re-keys the fingerprint, so no merge boundary exists at which the fingerprint is structurally blind to the qualifying item (§16, CF-3).
4. ~~**FQ-1's default silently makes item management Owner-only.**~~ **RETIRED by Amendment 001.** The Founder answered (`DEC-LOY-017`): Owner **and** Manager may manage Qualifying Items, delivered by a distinct `qualifyingItem.manage` permission in its own catalogue module rather than by widening `rewardProgram.manage` (§16 Amendment 001). The risk that a product shape would be chosen by engineering default no longer exists. *Residual, much smaller risk:* the implementation must not take the tempting shortcut of widening `rewardProgram.manage` instead, which would silently grant Managers Reward Program creation and publication authority that `DEC-LOY-017` withholds — gated by tests Y/Z/AA (§15; delivered in **PB-013A.2**, §16).
...
**For the specified implementation (guidance, not executed; package names per §16 as corrected by CORR-001):** roll back in strict reverse package order — **PB-013D** (revert the classification picker), **PB-013C** (revert as a single unit: the purchase backend contract, idempotency identity, Trust Event payload **and** the purchase UI together, then `0018.down` — drop the two FKs, then the column; reverting only one side would recreate exactly the incoherence CORR-001 removed), **PB-013B** (revert as a single unit: the reward-program backend cutover **and** its UI together; the `0017` tables become unread but harmless), **PB-013A.2** (revert the additive domain/callable/permission additions), **PB-013A.1** (`0017.down` **only if no real Business-created items exist** — otherwise it must refuse; then `0016.down`). The deferred `0019` (PB-013E) is intentionally last-in/never-early precisely so that no rollback path ever needs to recreate a dropped table from nothing. **Rollback obeys the same atomicity rule as delivery: a package that merged atomically reverts atomically.**
...
It is **B and not A** because two limitations remain real and disclosed rather than solved: **FQ-3** (removing the operator's free-text item field) is a visible workflow change best confirmed at a preview; and **FQ-4** (pre-existing purchase rows and the deferred `SET NOT NULL`) depends on deployed data no repository inspection can see. Neither blocks PB-013A.1, A.2, B, or C (§16), and each has a stated non-regressive default — so this is not **C**. Four requirements (**CF-1 – CF-4**, §17A) are carried forward to `PLATFORM-BASELINE-013` explicitly unresolved; they are requirements on a future package, not limitations of this design.

**FQ-1 is no longer among them.** Amendment 001 (2026-09-18) records the Founder's answer as `DEC-LOY-017` / `FD-QUALIFYING-ITEM-ROLE-AUTHORITY-001`: Owner **and** Manager may manage Qualifying Items; Staff may view and select but not manage; Platform Administrators receive no routine authority. The determined smallest coherent permission-model amendment — a distinct `qualifyingItem.manage` entry in its own fifth catalogue module, with `rewardProgram.manage` left Owner-only and untouched — is specified in §16 Amendment 001 as a **design recommendation only**, with tests Y–AC gating it. Nothing was implemented.

It is **not C** because no missing authority prevents implementation from starting: `DEC-LOY-016` and `DEC-LOY-017` together are sufficient for every slice, and the one question that touched a governed artifact (FQ-1, the permission catalogue) is now answered by recorded Founder decision. CORR-001 corrected the delivery sequence only; it altered no architectural conclusion. It is **not D** because the current architecture does not require reassessment — it turns out to be unusually well-suited to this correction: the versioned-snapshot discipline, the composite-FK scope-proof idiom, the `isEligibleForNewReference` / `isResolvableForExistingReference` lifecycle split, the whitelist transport parsers, and the membership-gated Reward Program read all compose to give this correction a home without inventing a single new architectural pattern.
...
**PLATFORM-BASELINE-012 — DESIGN CORRECTED / COHERENT-MAIN DELIVERY SEQUENCE ESTABLISHED / AWAITING NARROW RE-REVIEW.** (Correction 001, `PLATFORM-BASELINE-012-CORR-001`, 2026-09-18: §16 delivery sequence rewritten so every merge boundary leaves `main` coherent for every shipped caller; §16A records the root cause and a second, previously unnamed instance of it. Previously: Amendment 001, `PLATFORM-BASELINE-012-FD-ROLE-CLOSE-001`, 2026-09-18. `DEC-LOY-017` recorded; §17 FQ-1 closed; CF-1 – CF-4 carried forward to `PLATFORM-BASELINE-013`, which is **not** authorized to begin. No code, schema, migration, permission catalogue, evaluator, or seed change was made by this report or its amendment.)
