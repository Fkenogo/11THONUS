...
**F2 disposition:** **Fully resolved by AUTH-CORR-002** at the model/`-08` layer; AUTH-05's orchestration required **no F2 change** (it was already tuple-aware). The §16 incoherence (a same-uid second provider being unremovable because the projection was uid-keyed) no longer holds — proven by a dedicated emulator test (§17.5).

### 17.3 F1 correction (the one required code change)
...
**F1 RED→GREEN.** Four new unit tests asserting the access-state gate were authored first and run against the un-gated service → **RED** (link/unlink proceeded / TypeError on the missing gate). The gate was then implemented → **GREEN**.

- **Unit — `accountLinkingService.test.ts` (15 total; +5):** suspended acting identity → link `ACCOUNT_SUSPENDED` (no `-08` call); non-active (e.g. `locked`) → link `AUTH_FORBIDDEN`; suspended → unlink `ACCOUNT_SUSPENDED`; non-active → unlink `AUTH_FORBIDDEN`; active identity → link proceeds (loads identity, then mutates). The seven original tests were adapted to inject the `getIdentityById` active-identity seam.
- **Real Firebase Emulator — `accountLinkingService.emulator.test.ts` (10 total; +4):**
  ...
  - **same-uid unlink (F2-resolution proof):** the same-uid second provider is unlinked **without** tripping the last-reference invariant, the original reference remaining `linked` — the exact §16 scenario, now coherent.
...
1. Active identity links an additional provider — emulator "links a second provider…" + "same-uid multi-provider". 2. Same-uid two providers = distinct references — emulator same-uid test. 3. Both resolve via `-09` to one identity — emulator dual-resolution assertions. 4. Link never merges two identities — emulator cross-identity conflict (`VALIDATION_FAILED`, ownership unchanged, `…ConflictDetected`). 5. Reference owned by another identity rejected — same. 6/7. Access state enforced before link/unlink — unit + emulator F1 tests. 8. One of multiple references unlinked — emulator non-final + same-uid unlink. 9. Remaining reference still resolves — emulator assertions on the surviving `linked` ref. 10. Final-reference unlink rejected — emulator `INVALID_STATE_TRANSITION`. 11. Retry idempotency — derived credential-bound key consumed from `-08` (unchanged). 12. No credential material persisted — orchestration references no `rawToken`; secret grep clean; result asserted token-free. 13. No new error category — reused constructors only. 14. `CustomerAuthenticated` not emitted by AUTH-05 — AUTH-05 emits nothing. 15. No unrelated behavior changed — `git diff origin/main` touches only the six AUTH-05 paths + reconciled docs.
...
- `pnpm test` — **functions 511/511**, **web 300/300**.
- `pnpm emulators:validate` — full suite `201 passed, 2 failed`; **both failures are the inherited timeout flakes** (`commandDispatcher.emulator.test.ts` = `ENG-P1-002-CR1`; `crossPackageIdentityIntegration.emulator.test.ts` Scenario-7 race) — neither file is in the AUTH-05 diff, and both **pass 13/13 in isolation** with a normal timeout (evidence, not assertion). The **AUTH-05 emulator file passes 10/10**.
- e2e (Playwright) — deferred to CI (canonical gate); AUTH-05 is backend-only (functions + `index.ts`), no `apps/web` change.
...
### 17.9 Post-push PR evidence & review disposition

_Recorded after the reconstructed head is pushed and CI completes (final commit):_

- **Final PR #92 head SHA:** _(recorded post-push below)_
- **CI on final head:** _(recorded post-push below)_
- **Review findings (Codex + human) against the current head:** _(inspected post-push below)_
- **F1:** corrected (access-state gate, §17.3). **F2:** resolved by AUTH-CORR-002 (§17.2).

## Final Gate (resumption — 2026-08-09)

**AUTH-05 READY FOR FOUNDER REVIEW/MERGE.** The F2 architectural block is resolved by the Founder-approved AUTH-CORR-002 tuple model (merged to `main`); AUTH-05's orchestration was already tuple-aware and required no F2 change. F1 (acting-identity access-state enforcement) is corrected via TDD, reusing AUTH-03's existing gate and closed error taxonomy. All required invariants are proven by unit + real-emulator tests; local validation is green (the only emulator failures are inherited timeout flakes, proven to pass in isolation). PR #92 is **not** self-merged; AUTH-06+ is **not** started; the dirty primary worktree is untouched. Final head SHA / CI / review disposition recorded in §17.9 after push.
