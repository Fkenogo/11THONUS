...
| Payment terms — that a payment deadline exists (structural, no day count) | **B** (added this pass) | External Legal Opinion §20 Structural Subscription Provisions table ("Payment terms — Payment due within [X] days of invoice"), Reconciliation Matrix row 20, Class A — the concept is accepted structurally; the "[X]" value is not |
| Payment terms — the specific day count | **C** (added this pass) | Not covered by any `DEC-SUB-*` item (not listed under DEC-SUB-008's grace/billing values); genuinely untracked, not merely unresolved |
| Late payment — that reasonable late-payment charges/interest may apply (structural, no rate) | **B** (added this pass) | External Legal Opinion §20 table ("Late payment — Interest/reasonable charges for late payment"), Reconciliation Matrix row 20, Class A — same treatment as payment terms above |
| Late payment — the specific rate/charge mechanic | **C** (added this pass) | Not covered by any `DEC-SUB-*` item; genuinely untracked |
