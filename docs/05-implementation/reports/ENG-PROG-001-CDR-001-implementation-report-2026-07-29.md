...
## 15. Pre-Merge Correction — 4 Review Findings on PR #25

Following Founder approval and merge authorization for PR #25, a final merge-conditions check found 4 unresolved automated-review threads (2 P1, 2 P2, from Codex) that had not been present at the amendment's own last CI/review check. All four were investigated against the live Engineering Implementation Programme before any change was made, per this task's established discipline of verifying before fixing.

1. **P1 — Capability 4 missing Phase 4 as a precondition.** The Programme's Phase Summary Table states Phase 5 is `Blocked (depends on P4)`; the document had only noted `ENG-P4-001`/`ENG-P4-002` as a Capability 5 precondition, meaning Capability 4 could appear ready once Capability 3 finished, without a Reward Program yet existing to record a purchase against. **Fixed:** `ENG-P4-001`/`ENG-P4-002` moved into Capability 4's own major work packages and §8 mapping-table row, with the sequencing rationale stated explicitly; the stale aside note removed.
2. **P1 — Capability 8/9 circular dependency.** Capability 8 originally grouped Phase 12 (`Blocked (depends on P2, P4, P11)`) with Phase 14 (`Blocked (depends on P0–P13 substantially complete)` — i.e., including Phase 13, which is Capability 9) while stating Capability 8 depended only on Capability 1; Capability 9 in turn was stated to depend on Capability 8 — a genuine cycle (Cap 8 → needs Phase 13/Cap 9 via Phase 14; Cap 9 → needs Cap 8). **Fixed:** Phase 14 regrouped from Capability 8 into Capability 9 (alongside Phase 13, both being the final pre-pilot hardening phases); Capability 8's dependency corrected to Capability 7 (Phase 12's real Phase 11 dependency); the §6 Capability Timeline narrative — which had incorrectly described Capabilities 7/8/9 as independently parallel — corrected to state the true linear chain (7→8→9); §8's table rows updated to match.
3. **P2 — 5 of the 47 work packages (`ENG-P15-xxx`/`ENG-P16-xxx`) unmapped to any capability despite §1's "all 47" framing.** **Fixed:** §1 corrected to state 42 of 47 are capability-mapped, with the remaining 5 explicitly identified as milestone-level; §7's two affected future-milestone bullets (Pilot Readiness, Production Readiness) now list the exact work-package IDs each corresponds to, and Production Readiness's dependency corrected from "Capability 8" to "the Pilot Readiness milestone / Phase 15" (Phase 16 is `Blocked (depends on P15)`, not on Phase 12 directly) — a further accuracy correction surfaced while fixing this finding.
4. **P2 — `OBS-OPS-001` included in Capability 9's narrative but absent from its §8 table row.** Since §10 (Definition of Capability Completion) checks only work packages "listed for the capability in §8," Capability 9 could have been marked complete while `OBS-OPS-001` — which §5's own text already said was "included" — remained unimplemented. **Fixed:** added to Capability 9's §8 table row.

**A fifth, self-caught defect** (not from the reviewer, found while re-reading the document to plan these fixes): §10 item 1 still read "listed for the capability in §7" — a leftover miss from the `ENG-PROG-001A` renumbering's own multi-round audit, pointing at Milestone Structure instead of the Engineering Work Package Mapping section. Corrected to §8.

**Validation performed:** re-read the full document end-to-end after all edits to check for further inconsistencies introduced by the corrections themselves (e.g., confirmed Capability 8's "Milestone contribution" no longer claims "Production Readiness" now that Phase 14 moved out of it); Prettier clean; section headers still sequential 1–12; all 28 links still resolve; a table-scoped script confirmed the §8 mapping table contains exactly 42 unique `ENG-Pn-xxx` IDs plus `OBS-OPS-001` (with `ENG-P2-004` legitimately appearing in both Capability 2 and 3's rows, as already disclosed) — manually cross-checked against a by-hand capability-by-capability count to confirm 42 is correct (2+3+2+5+8+3+2+9+2+6, deduplicating the shared `ENG-P2-004`).

**Files modified:** `docs/05-implementation/roadmap/CDR-001-capability-delivery-roadmap.md`; this report; `docs/changes/IMPLEMENTATION_CHANGES.md`.

**Commands executed:** `gh api graphql` (review-thread inspection); direct reads of the live Engineering Implementation Programme's Phase Summary Table (`B.1`) to verify every dependency claim before correcting; `pnpm exec prettier --check`; the link-resolution Python script; a table-scoped Python script counting unique work-package IDs; `git diff --stat`.
