...
The tip of `feat/eng-p2-003c-staff-membership-lifecycle` at merge-review time — see PR #135's own head SHA for the exact value (a docs-only trailing commit or two recording this report's own PR/CI-status finalization is expected on top of `a240ae342b52aa5d44c51352aa134a0621e02dbe`, the last commit with functional-code changes; `git diff` against that SHA shows zero functional-code diff for any such trailing commit — only this report file itself). All functional-code test/validation evidence in this report (§33/§34) was captured directly against `a240ae3`'s code.
