...
`AUTH-MFA-003A1 TRUSTED PLATFORM-ADMINISTRATOR DISCOVERY IMPLEMENTED — SERVER-VERIFIED IDENTITY (CUSTOMER IDENTITY ID, NOT FIREBASE UID — AUTHORITATIVE TRUST CHAIN CORRECTING §8A.3'S CONCEPTUAL SKETCH) — MINIMAL { isPlatformAdministrator: boolean } CONTRACT — ACTIVE-ONLY LIFECYCLE — FAIL-CLOSED (MALFORMED RECORD → PERMISSION-DENIED; INELIGIBLE IDENTITY → UNAUTHENTICATED) — NO AUDIT MUTATION AND ZERO OUTBOX WRITES END TO END (SEE §15) — NO LIVE CONFIG CHANGE — MASS-ASSIGNMENT REGRESSION-TESTED — 1651 UNIT + 10 EMULATOR TESTS GREEN (756/758 FULL SUITE) — READY FOR FOUNDER TECHNICAL REVIEW`

## 15. Automated-review P2 correction (`-CORR-001`): discovery resolution is now genuinely non-mutating

The automated reviewer (Codex) raised one inline **P2** finding on this PR: although the package declared discovery read-only/no-audit, the callable's identity resolution ran through `resolveAuthenticatedIdentityActor`, whose authentication-purpose merged lookup **always persists an `IdentityLookupAttempted` outbox event** (`identityLookupRepository.runLookup` → `emitAuditEvent`, `AUDIT_ALWAYS_PURPOSES` includes `authentication`). Repeated client routing probes therefore created unbounded, non-deduplicated writes, an audit-transaction failure could make this "read-only" callable fail even when its target records were readable, and the emulator tests missed it because they asserted only the `platformAdministrationAuditRecords` collection — not the shared outbox. The reviewer asked for a non-mutating trusted identity-resolution path, otherwise to stop calling the endpoint read-only.

**Fix (`-CORR-001`, same PR):** a non-mutating twin in the identity domain — `resolveAuthenticatedIdentityActorReadOnly` (`functions/src/domains/identity/repositories/authenticatedIdentityActor.ts`). It is byte-for-byte the same authority chain (verified token → `-09` authentication-reference lookup → `getCustomerIdentityById` → the **shared** `active`/`dormant` eligibility gate), but the reference step runs through the merged lookup with `purpose: "internal_service"` — the codebase's deliberately non-audited purpose (documented to exist precisely "to avoid emitting on every routine scan"; `trustSignalIngestionService` already uses it). The declared purpose is **hardcoded** in the resolver, so the twin structurally cannot route a governed `authentication` purpose, and it exposes no `resolveCredential`/`resolutionDeps` ports a caller could point at an auditing lookup. Not-found still fails closed to `identityActorNotEligibleError` (`AUTH_REQUIRED`), mirroring the audited twin. The `discoverPlatformAdministrator` callable now uses the twin.

**Proof (new tests, `-CORR-001`):**
- `authenticatedIdentityActor.emulator.test.ts` (new, identity domain, 4 tests) — under the real Firestore emulator with a linked authentication reference:
  - the **audited** twin persists **exactly one** `identity_lookup_attempted` outbox event on resolution (contrast control);
  - the **read-only** twin resolves the same identity while the `outboxEntries` count is **unchanged**;
  - read-only twin, unknown reference → `AUTH_REQUIRED`, outbox unchanged;
  - read-only twin, suspended identity → `AUTH_REQUIRED`, outbox unchanged.
- `discoverPlatformAdministrator.emulator.test.ts` — the read-only test now snapshots **both** the platform-administration audit collection **and** the shared outbox, and asserts neither changes across repeated discovery (directly closing the reviewer's "checks only platformAdministrationAuditRecords, not the shared outbox" gap).
- Unit: 6 new `resolveAuthenticatedIdentityActorReadOnly` tests (non-auditing purpose pass-through, no audit ports, not-found/suspended fail-closed, dormant accepted, infrastructure errors propagated).

**Gate impact:** unit **1651** passed / 153 files; typecheck, lint, build, format clean; emulator full suite **59 files / 756 passed / 2 skipped** (fresh green run). The P2's audit-transaction-failure mode is now structurally absent from the discovery path (no transaction exists in the read-only resolution at all). The two resolution paths share the single eligibility gate, so no divergence can slip in silently. This section is the `-CORR-001` record; it is also recorded in `IMPLEMENTATION_CHANGES.md` and `documentation-changes-log.md` Entry 170.

**Next task (Founder/Technical Review):** review this PR (`feat/auth-mfa-003a1-trusted-administrator-discovery`) at its `-CORR-001` head; on approval, the subsequent client MFA packages (`AUTH-MFA-003B` enrollment UI, `AUTH-MFA-003C` challenge UI) consume this callable for routing. Each remains separately authorizable per `DEC-SEC-004`.