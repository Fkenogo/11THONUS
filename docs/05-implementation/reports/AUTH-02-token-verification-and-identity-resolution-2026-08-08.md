...
> **Version:** 1.1 · **Status:** Implemented (TDD) + Founder-directed pre-merge security correction (P1/P2) applied — pending Founder-authorized review/merge · **Classification:** Working (implementation report)
...
> **Last controlled update:** 2026-08-08 (`AUTH-02` v1.1 — Founder-directed pre-merge correction: provider-provenance binding (P1) + Firebase transport-error mapping (P2); §13 added)
...
- **Unit tests:** 30 AUTH-02 unit tests (21 verifier — incl. 10 v1.1 provenance/transport regression tests — + 9 resolver); full `functions` suite **477/477** (was 447 at AUTH-01, 467 at AUTH-02 v1.0). Verifier proven at its Firebase seam with a test double (no live Firebase; `DEC-AUTH-001` D-A4).
...
## 13. Pre-Merge Correction (v1.1) — Founder-directed, both automated-review findings accepted

Applied before merge, TDD (RED→GREEN), confined to `firebaseTokenVerifier.ts` (+ its test); no unrelated files; **§12 deliberately NOT addressed** (it stays the separately-governed pre-AUTH-03 reconciliation).

- **P1 — provider-provenance binding (security).** The verifier no longer trusts `RawProviderCredential.referenceType` as authoritative. The governed reference type is **derived from the verified `decoded.firebase.sign_in_provider`** via a closed MVP mapping (`phone`→`phone_otp`, `google.com`→`google_sign_in`; `DEC-AUTH-001` D-A2). If the verified provider is unsupported, or the caller-declared type does not match the verified provider, verification **fails closed** with `authenticationForbiddenError()` (`AUTH_FORBIDDEN`, existing closed-taxonomy factory — **no new category**). The credential's `referenceType` and `providerSignals.signInProvider` now both come from the verified token. Kept at the AUTH-02 verification boundary (not deferred to AUTH-04): AUTH-04 owns provider *flows*, while verified provider *provenance* is a property of verification itself.
- **P2 — transient transport mapping.** Firebase Admin `app/network-error` and `app/network-timeout` (plus `auth/network-timeout`) now map to `TEMPORARY_UNAVAILABLE`; raw socket codes retained as a fallback; unknown errors keep their existing fail-closed `INTEGRATION_FAILED` treatment.
- **Regression tests added (all green):** google-verified token cannot be `phone_otp`; phone-verified token cannot be `google_sign_in`; supported providers map to the correct governed type; unsupported/mismatched provenance fails closed (`password`/`apple.com`/`custom`/`anonymous`); `app/network-error` and `app/network-timeout` → `TEMPORARY_UNAVAILABLE`; all pre-existing AUTH-02 verification/resolution tests remain green (one revocation test's declared type was aligned to its token's verified provider, its assertion — `verifyIdToken(token, true)` — unchanged).

## Final Gate
- **Provider provenance bound to the verified token (P1)** — declared type validated against `sign_in_provider`; unsupported/mismatched fails closed on the closed taxonomy. ✅
