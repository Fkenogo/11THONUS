...
> **PR #216 review correction (same commit range, in place):** Codex's P2 finding on the original §24/§25 text is confirmed genuine — row 2's narrow Burundi language sub-element was incorrectly cited as an example of "Established optional/appropriate overlay." Per the approved readiness-assessment boundary (`DEC-LEGAL-002-BT-PART-VIII-READINESS-001-assessment-report-2026-09-02.md` §34/§35), that point is **governed/default treatment requiring no additional overlay content** — the existing, already-drafted §25.6 text already satisfies it, with no separate overlay content adopted. §24/§25 below are corrected in place: category 2 currently has no example in either jurisdiction; row 2's narrow point is correctly placed in category 3. No other section of this report is affected — the drafting-boundary matrix (§10), category counts (§14–16), and overall gate (§31) are unchanged by this correction.
...
1. **Established mandatory overlay** — a specific mandatory local-law requirement has been verified and stated as Burundi/Rwanda-specific overlay *content*, beyond what the portable core already provides (currently: row 11 Burundi only — the mandatory pre-acceptance disclosure categories under *Loi n° 1/11* Arts. 6–8 — the sole item in this category across both jurisdictions).
2. **Established optional/appropriate overlay** — a non-mandatory but appropriate local provision has been affirmatively adopted as additional overlay *content*, distinct from the portable core's own existing text. **No current example exists in either jurisdiction.** **Correction (Codex P2, PR #216 review):** row 2's narrow Burundi language sub-element does **not** belong here — the readiness assessment's own approved boundary (`DEC-LEGAL-002-BT-PART-VIII-READINESS-001-assessment-report-2026-09-02.md` §34/§35) classifies that point as **governed/default treatment requiring no additional overlay content**, because the existing, already-drafted §25.6 English/French text already satisfies the recommendation without any separate overlay text being adopted. It belongs to category 3 below, not this one. Category 2 exists in the label scheme for a future case where a jurisdiction genuinely adopts non-mandatory local content the portable core does not already supply — none currently does.
3. **Verified — no additional overlay required for the tested question (governed/default treatment)** — the specific, narrow question was directly verified and the portable core's existing text already fully addresses it, with no separate overlay content needed (currently: rows 3, 6 (×2), 8 Rwanda, 9 Rwanda, all C-classified per the merged CORR-001 report; rows 1 Rwanda, 5, 7, 10 per the broader 13-topic matrix — governing law, suspension/exit, general liability principle, privacy/data, all C because Rwanda is the default jurisdiction or the point is affirmatively embodied; and row 2's narrow Burundi language sub-element, per the correction above).
...
Exactly the five labels enumerated at §24: *Established mandatory overlay*; *Established optional/appropriate overlay*; *Verified — no overlay required (governed/default treatment)*; *Unresolved/reserved*; *Future-triggered*. No other label (e.g., a bare "N/A," a silent omission, or an unqualified "no overlay required" applied to any of the 11 D rows) is permitted, per the merged report's own express instruction that silence must never be presented as an affirmative C-equivalent finding. A row may only be labeled category 1 or 2 where actual overlay-specific content was verified and adopted beyond the portable core's existing text — never merely because the core's existing text happens to already satisfy a jurisdiction's recommendation (that case is category 3), consistent with the approved readiness-assessment boundary at its §34/§35.
