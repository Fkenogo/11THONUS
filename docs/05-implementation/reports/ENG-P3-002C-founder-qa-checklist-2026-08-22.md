...
10. **Staff invitation.** Attempt to invite someone. *Expected (currently):* a visible "you don't have permission to do that" message — confirm it is visible, not silent (this is the fix this task made; confirm it, don't be surprised by the underlying denial itself — that's a known, separately-tracked gap).
