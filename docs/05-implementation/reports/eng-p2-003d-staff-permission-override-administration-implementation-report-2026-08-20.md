...
Founder-authorized "ENG-P2-003D — Independent Final Review, Merge & Closure" task, executed in an isolated worktree (primary checkout `/Users/theo/11THONUS` untouched throughout). By the time this task began, the branch had already been pushed and PR #136 opened (resolving the blocker recorded above); this addendum covers the subsequent independent review and merge.

**Entry verification:** PR #136 confirmed `OPEN`/`draft`/`MERGEABLE`/`mergeStateStatus: CLEAN` at entry, head `f1165f84ddd5dc7e13394524c0cebc79168fc11a` (unchanged since the implementation report — no commits landed after it). Hosted CI already `success` on that exact head — run `32344934503` ("Build, Lint, Test, Emulator Validation"), 4m4s. `ENG-P2-003E` confirmed still not started (no matching branch, PR, or file).

**Reconstructed authority:** re-read `ENG-P2-003-DESIGN-001` §29 directly (`FD-003D-1` — `permissions[]` is current-configuration-only, at most one override per `(membership, permissionId)`, atomic replace-not-append, same-direction idempotent; `FD-003D-2` — the four-status matrix: `active` grant+revoke, `suspended` revoke-only, `removed`/`invited` neither), `permissionOverride.ts`, `sensitivePermissionCatalogue.ts`, `evaluatePermission.ts`, `authorizeAndExecute.ts`, and the `staff.assignPermissions` catalogue entry, independently, not from this report.

**Independent final review, re-derived directly from source (not this report):**
- **Core boundary:** `staffPermissionOverrideCommand.ts` only administers configuration — `createPermissionOverride` (unmodified `ENG-P2-004A`) remains the sole eligibility/precedence authority. `evaluatePermission.ts`, `permissionOverride.ts`, and `sensitivePermissionCatalogue.ts` confirmed **byte-identical** to `origin/main` (`git diff` empty) — zero evaluator/catalogue regression.
- **Current-configuration invariant / replacement semantics (FD-003D-1):** `prepare()` reads the target's raw `permissions[]` in one transactional read (`permissionOverrideAdminRepository.ts`), filters to at-most-one matching record (fails closed via `overrideAdminMalformedExistingOverrideStateError` if more than one is found — pre-existing malformed state is never silently repaired), and `apply()` writes the complete replaced array — never an append. Confirmed by the `REPLACEMENT SEMANTICS` test group (grant→revoke replace, revoke→eligible-grant replace, revoke→ineligible-grant fails with the existing revoke unchanged, same-direction replay produces one record and no new outbox event, untouched permissions for other permissionIds survive, malformed duplicate pre-existing state fails closed).
- **Status matrix (FD-003D-2):** `removed`/`invited` reject both directions; `suspended` rejects `grant` only, before any persisted mutation; `active` permits both, subject to the unmodified `createPermissionOverride` contract — read live inside the same transaction, never trusted from request data. Matches the design table exactly, verified in code (`staffPermissionOverrideCommand.ts` lines 161–168) and by dedicated tests for all eight cells.
- **Suspended security proof (Phase G, mandatory):** the `"suspended revoke -> reactivate: evaluator still DENIES that permission"` test independently re-run — active Manager granted, suspended, revoked-while-suspended, reactivated, then the **real** `evaluatePermission` (not mocked) confirmed `allowed: false`. Passed.
- **Authorization / target isolation / role eligibility:** every mutation runs through `authorizeAndExecute` gated on `staff.assignPermissions` exclusively (no `staff.manage`/`staff.assignRole` substitution); target membership is read fresh by ID and rejected on cross-business mismatch and on `role === "owner"` (delegated to the unmodified `createPermissionOverride` constructor); target role is always the just-read live value, never the actor's or the request's — confirmed by the demote-then-stale-grant-unusable test.
- **Ordinary/unknown permissions (CORR-001 boundary):** `getSensitivePermissionEntry(params.permissionId)` gates before any transaction I/O, rejecting both ordinary and unknown permission IDs (`unrecognisedSensitivePermissionError`, `VALIDATION_FAILED`) — confirmed by dedicated tests, preserving CORR-001's explicit deferral.
- **Concurrency / TOCTOU:** authorization, target role/status read, raw-overrides read, and the replacement write all occur inside one `authorizeAndExecute` Firestore transaction (single read of the target document, per `permissionOverrideAdminRepository.ts`'s own design-intent comment) — re-run fresh on every retry. Five real-Firestore concurrency scenarios (double-grant, grant-vs-revoke, role-change-vs-grant, removal-vs-grant, two-different-permissions) independently re-run via the full emulator suite — all passed, each producing a single, non-contradictory current record.
- **Idempotency / error taxonomy / outbox privacy:** client-retry-same-key and same-key-different-payload (`IDEMPOTENCY_CONFLICT`) both verified; only two new error constructors added, both mapped to pre-existing categories (`INVALID_STATE_TRANSITION`, `AUTH_FORBIDDEN`) — no new taxonomy category; `StaffPermissionOverrideChangedPayload` confirmed to carry only identifiers/categorical values, no credentials or protected identity data, fired only on an actual change (never on a no-op replay).
- **Non-regression:** `git diff origin/main...HEAD --stat` confirms the diff is scoped to exactly the ten files this package should touch (command, repository, doc-model additive parser, write-repository additive function, error additions, event builder, docs) plus the emulator test file — no `firestore.rules`, `firebase.json`, `package.json`, or lockfile change anywhere in the diff.

No material finding across any reviewed phase.

**Full validation (re-run fresh, this task):** `pnpm install --frozen-lockfile` clean; `pnpm typecheck` clean (both workspaces); `pnpm lint` clean; `pnpm format:check` clean; `pnpm build` clean; `functions` unit **1240/1240**; `apps/web` unit **397/397**; `pnpm run emulators:validate` (real Firebase Emulator Suite) **519/519** across 38 test files, including the full `staffPermissionOverrideCommand.emulator.test.ts` suite (authorization, target/cross-business/Owner, grant, revoke, replacement semantics, target-status matrix, role interaction, idempotency/integrity, concurrency); manual secret-pattern scan of the PR's file-level diff (`git diff` against merge-base) clean.

**Merge:** PR #136 marked ready for review (undrafted) and merged. **Merge commit: `6ed08020dddc3fbfedabcffad0bda35e5ffb8cda`.** Confirmed an ancestor of `origin/main` (`git merge-base --is-ancestor` = true) after `git fetch origin main` (`7608a64..6ed0802`).

**Post-merge CI:** run `32346680946` on `main`, headSha `6ed08020dddc3fbfedabcffad0bda35e5ffb8cda` — `success`, 4m6s, all steps (Build/Lint/Format/Typecheck/Unit/Playwright e2e/Emulator Suite) passed.

**Closure sync:** this addendum, plus a dated-supersession note appended to `CDR-001` §5 Capability 3 and the Engineering Implementation Programme header, record: `ENG-P2-003D` = **Complete/merged**. `ENG-P2-003E` = **Not started**. **Capability 3 = `Open — partially implemented; not closed`** (unchanged — not marked Complete). No `ENG-P2-003E` work begun; no new scope entered beyond this task's authorization.

**FINAL GATE: `ENG-P2-003D MERGED AND CLOSED — ENG-P2-003E AWAITS FRESH FOUNDER AUTHORIZATION`**
