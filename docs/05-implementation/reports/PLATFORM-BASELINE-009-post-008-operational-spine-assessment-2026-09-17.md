...
**Date:** 2026-09-17 (original); **`CORR-001` appended 2026-09-17.**
**Type:** Read-only technical/operational assessment. No implementation performed. No migrations created. No governance decisions modified. Nothing merged. **`CORR-001` (§3A):** documentation-only correction to the Business Terms section's terminology and authority framing, following an independent dedicated trace (`PLATFORM-BASELINE-009-BUSINESS-TERMS-ACTIVATION-TRACE-001`, review-only). No implementation, schema, migration, config, or decision-register change; original §3 findings not overturned, only clarified. See §3A.
...
**Terminology note, corrected in full by `CORR-001` below:** "Business Terms" in this section refers to the Business's platform-participation agreement with 11thONUS, not to any individual Reward Program's own configuration or content. See §3A.

---

## 3A. `PLATFORM-BASELINE-009-CORR-001` — Business Participation Terms vs. Reward Program Terms (terminology and authority correction)

**Basis:** an independent, dedicated investigation dispatched after this report was first drafted (`PLATFORM-BASELINE-009-BUSINESS-TERMS-ACTIVATION-TRACE-001`, review-only, no implementation/governance change, PR #256 never touched) traced the full authority chain from Founder/product intent through the decision register, PRD/TRD, schema, and code for "Business Terms," "Reward Program terms," and the Business activation gate. Its findings are adopted here as a correction to §3's wording; §3's substantive findings (real-environment activation blocked; engineering gate correct; classification D) are **not** overturned — they are clarified and given their correct authority citations.

**1. What "Business Terms" means in the activation path.** Every occurrence of "Business Terms" in §3, in `assertCurrentBusinessTermsAccepted`, and in `platformConfig/businessTerms`/`businessTermsAcceptances` refers to a single, platform-wide **Business Participation Agreement** — the Business's contractual relationship with 11thONUS as platform operator (parties/acceptance, participation obligations, platform action/exit, commercial terms, risk allocation, legal mechanics; drafted as Parts I–VII of `DEC-LEGAL-002-BT-DRAFT-001` et seq.). It does **not** mean, and has never meant, the terms, rules, or configuration of any individual Business's Reward Program. Direct code confirmation: `createRewardProgramCommand.ts` and `createNextRewardProgramVersionCommand.ts` contain no reference to `assertCurrentBusinessTermsAccepted`, `businessTermsAcceptance`, or `termsVersion` anywhere; Reward Program authorization (`rewardProgramAuthorization.ts` → `evaluatePermission`) gates solely on `business.status ∈ {trial, active}`, never on Terms re-acceptance.

**2. Real-environment activation block — preserved, unchanged.** §3's finding stands exactly as stated: no Business can leave `draft` in any non-emulator environment today, because `platformConfig/businessTerms.currentVersion` is `NOT CONFIGURED` in every real/production/staging environment, and `DEC-LEGAL-002` (the decision governing the Terms *content*) remains `OPEN_LEGAL`.

**3. Why the engineering gate is correct (not merely asserted).** `assertCurrentBusinessTermsAccepted` is correct because Business Participation Agreement acceptance is legitimately **both Business-scoped and activation-scoped**: it is a fact about *this Business's own relationship with the platform* (not about any Reward Program the Business may separately operate), and it is exactly the kind of platform-eligibility precondition that belongs at the activation boundary (`draft → pending_verification`, rechecked at `pending_verification → trial`) rather than at any narrower, per-programme boundary. Gating activation on it is therefore the correct altitude for this check — gating individual Reward Program actions on it would be the wrong altitude (and the codebase does not do that; see Finding 4).

**4. Reward Program independence — explicit finding.** Confirmed by direct code trace: a Business, once `trial`/`active`, may create multiple Reward Programs (`reward_programs.business_id` carries only a non-unique index — no one-programme-per-Business constraint exists); each Program is independently, immutably versioned (`reward_program_versions`, `DA-004`'s "Historical Reward Program terms shall remain versioned and reproducible" — a commercial-mechanics versioning guarantee, distinct from any legal-acceptance artifact); Programs may carry different qualifying nodes, thresholds, and reward definitions; and creating or revising a Reward Program never calls, checks, or requires re-satisfying `assertCurrentBusinessTermsAccepted`, and never touches Business lifecycle status. No Business reactivation, and no Business Terms re-acceptance, is triggered by Reward Program creation, versioning, or publication.

**5. Correcting "one-time" framing of acceptance.** Business Terms acceptance should not be characterized as simply "one-time." It is more precisely a **Business-scoped and versioned platform-participation acceptance**: the platform maintains a single current Terms version (`platformConfig/businessTerms.currentVersion`), each Business's acceptance of a specific version is recorded as an immutable, non-overwritten record (`businessTermsAcceptances`, keyed by Business + version + accepting owner identity), and the *currently-required* version is re-checked at each activation-relevant transition (`draft → pending_verification` and, again, `pending_verification → trial`) rather than trusted from a single earlier check. (§3's two uses of "one-time" describe an unrelated fact — the ops action of *configuring* the platform-wide current-version document — and are accurate as written; they are not about the acceptance act itself and required no change.)

**6. Correcting the authority chain — DEC-LEGAL-002 does not itself establish the activation gate.** §3, read carelessly, could be misread as implying `DEC-LEGAL-002` is the authority for gating activation on Terms acceptance. It is not. The corrected chain is:
`DEC-BUS-ACT-001` (Product, **CONFIRMED**, 2026-09-12 — *"Activation requires the Business to exist in exactly `pending_verification` and the current owner to have accepted the currently-required Business Terms version (rechecked at activation time, including across Terms-version changes)"*) → the Business onboarding/activation engineering design (`ENG-P3-002-DESIGN-001` §37, Terms-of-Service Acceptance Architecture) → `assertCurrentBusinessTermsAccepted` (`businessLifecycleCommand.ts`) → its two call sites (`submitBusinessForVerificationCommand`, `activateBusinessAfterVerificationCommand`) → the emulator test suite's "Terms preconditions" coverage.
`DEC-LEGAL-002` sits in a separate lane: it governs the **content/legal drafting** of the Business Participation Agreement (what the Terms say), owned by Founder + legal adviser, and remains `OPEN_LEGAL` for that reason. It has never itself stated that Terms acceptance shall gate activation — that authorization comes from `DEC-BUS-ACT-001` alone.

**7. Terminology-clarity finding (documentation/governance observation only — DEC-LEGAL-002 not amended by this correction or by this task).** `DEC-LEGAL-002`'s own Question field reads: *"Reward Program terms, business obligation to honour rewards, dispute language, platform liability, subscription terms."* This wording loosely bundles "Reward Program terms" together with platform-liability and subscription-terms language under a single legal-decision label, even though the instrument actually drafted under that decision is a single Business Participation Agreement (which addresses, but does not separately legally version, Reward Program *operation* obligations in its Part III). This wording is the most plausible source of the activation/Reward-Program-terms conflation this correction resolves. **Recommendation for a future, separate governance task (not performed here):** rename or annotate the instrument's governance label — e.g., "Business Terms — Platform Participation Agreement" or "11thONUS Business Participation Terms" — while retaining `DEC-LEGAL-002` and all existing `BT-DRAFT-*`/`BT-PART-*` historical identifiers unchanged, so that future readers do not need to re-derive this distinction from source code.

**8. Traceability gap (recorded, not closed).** `DEC-BUS-ACT-001` explicitly establishes the Terms-acceptance/re-check requirement, and the implementation (`assertCurrentBusinessTermsAccepted` and its two call sites) faithfully follows it. No explicit FR-/NFR-/BR- row in `requirements-traceability-matrix.md` or the PRD/TRD states, as a numbered requirement, that "Terms acceptance gates Business activation." This is a documentation-traceability gap between a Founder-confirmed decision and the requirements matrix, not a code or governance defect. Not closed by this correction, per task instruction.

**9. Other PB-009 findings — unaffected, kept separate.** This correction changes nothing about, and should not be read as bearing on: the Commerce Knowledge Reward Category content blocker (§2 — a product-taxonomy/content-authority gap, unrelated to Business Participation Terms); the redemption-mechanics Founder-decision gap (§4/§11 — no governing decision exists yet for how a Business confirms/fulfils a Reward; not designed by this correction); the latent, non-blocking `unitValueMinor: 0` transport/domain-layer mismatch (§6); or the `DEC-CKS-001`/`DEC-CKS-002` decision-register traceability discrepancy (§14). Each remains exactly as originally classified.

**10. Recommended-next-package re-evaluation.** §11's recommendation (the bounded, decision-free, additive Business-facing Reward/Loyalty-Cycle visibility read surface) is **unchanged** by this correction — nothing in the terminology clarification alters its ranking, scope, or dependencies. Restated with the corrected framing, the assessment's blockers now separate cleanly into four independent categories, none of which is resolved or reordered by another:
   1. **Legal/content blocker:** Business Participation Terms finalisation and platform-wide configuration (`DEC-LEGAL-002` `OPEN_LEGAL`; owned by Founder + legal adviser).
   2. **Product taxonomy/content blocker:** Commerce Knowledge `reward_program_category`/`standard_product`/`standard_service` seed content and mapping authority (owned by Founder/Knowledge-Studio).
   3. **Founder-decision blocker:** redemption mechanics (no governing decision exists yet in the decision register).
   4. **Engineering/operability work that can proceed independently of 1–3:** the recommended Business-facing Reward/Loyalty-Cycle visibility read surface (§11) — it depends on none of the above and requires no new Founder/legal/content decision.

**Disposition of this correction:** `PLATFORM-BASELINE-009` — **ASSESSMENT CORRECTED / BUSINESS-PARTICIPATION VS REWARD-PROGRAM TERMS CLARIFIED / AWAITING FOUNDER REVIEW.** This does not change the report's overall disposition (§13, unchanged: **B — ENGINE ASSEMBLED / ENTRY OR OPERABILITY GAPS REMAIN**). No implementation, schema, migration, config, or decision-register file was changed by this correction. No Founder decision was created. PR #256 remains open and unmerged. The recommended next package (§11) was not started.
...
**`CORR-001` constraints confirmation (added, not superseding the above):** the correction (§3A) is documentation-only, applied on the existing, still-unmerged `docs/platform-baseline-009-post-008-assessment-001` branch (PR #256) in the same pre-existing isolated worktree. No implementation, schema, migration, dependency, configuration, or decision-register file was changed. No new Founder decision was created or implied. The primary worktree was not entered. PR #256 was not merged. The recommended next package (§11) was not started.
