...
No CI run was required for this stage's own local validation work (validation happens against the local worktree). CI ran when the Stage 4 PR itself was opened, and recorded a **fourth** occurrence of the same pre-existing class of flakiness (§47): PR #22's evidence-recording follow-up commit failed once on `idempotencyService.emulator.test.ts`'s "reports 'duplicate' with the stored response after completeIdempotencyKey" — the identical test and failure signature already seen once during Stage 3's PR #21 review — then passed cleanly on rerun. `functions/` remains untouched by this PR (confirmed by an empty diff both before and after), and every deterministic CI step passed consistently in every instance across all four occurrences. This is recorded accurately here, not re-investigated or fixed, per this task's explicit instruction not to modify unrelated backend tests as part of `ENG-P1-003`.

## 38. ENG-CI-001 Recommendation

**Recommended, not implemented in this task:** `ENG-CI-001 — Firebase Emulator CI Flakiness Investigation and Stabilisation`. Rationale: **four** occurrences now recorded across `ENG-P1-003`'s three PRs (once in Stage 2's PR #20, twice in Stage 3's PR #21, once in this Stage 4's own PR #22), affecting three different specific tests within the same real-Firestore-emulator concurrency suite (`functions/src/shared/commands/commandDispatcher.emulator.test.ts`, `functions/src/shared/idempotency/idempotencyService.emulator.test.ts` — the latter recurring identically twice), with every deterministic CI step (build/lint/typecheck/unit/e2e) passing consistently in every instance — strong, now-repeated evidence of emulator-timing-specific flakiness warranting dedicated investigation, independent of and not blocking any `ENG-P1-003` stage's own merge readiness.
...
GitHub Actions run `30279504301` — `Build, Lint, Test, Emulator Validation`: **pass**, first attempt, on head commit `cef7cd64d74734b2c51049c7e00e2f5ef4e9e539`.

**Disclosed CI evidence on the current head commit** (`919f6aec4bd350f363e5be93d73dfd8dd70e8068`, a docs-only follow-up recording the PR/CI evidence above — `functions/` untouched, confirmed by an empty `git diff origin/main --stat -- functions/` both before and after): run `30279687960` failed on its first attempt and passed on rerun. Every deterministic step (Build, Lint, Format check, Typecheck, Unit/component tests, Playwright e2e) passed; only "Firebase Emulator Suite validation" failed, on `idempotencyService.emulator.test.ts`'s "reports 'duplicate' with the stored response after completeIdempotencyKey" (`expected 'acquired' to be 'duplicate'`) — the identical test and failure signature already seen once during Stage 3's PR #21 review. This is the **fourth** documented occurrence of this class of pre-existing, unrelated `functions/` real-emulator-concurrency flakiness across `ENG-P1-003`'s three PRs (once in Stage 2's PR #20, twice in Stage 3's PR #21, once here in Stage 4's PR #22), strengthening rather than weakening the `ENG-CI-001` recommendation (§38). The rerun passed cleanly: `Build, Lint, Test, Emulator Validation`: **pass**, on the exact current head commit `919f6aec4bd350f363e5be93d73dfd8dd70e8068`. `mergeable: "MERGEABLE"`, `state: "OPEN"`.
