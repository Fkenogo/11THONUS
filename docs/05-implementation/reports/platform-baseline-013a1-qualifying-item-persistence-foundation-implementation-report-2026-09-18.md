...
>
> **Correction 002 (2026-09-18, lightweight pre-merge record hygiene, `ITR-002`):** independent narrow re-review `ITR-002` verified Correction 001 as **A — approvable for merge**, with no new correctness/security finding, and identified exactly two residual documentation-arithmetic inconsistencies: the final file count was still stated as "10 files" in §16/§19/the PR description (the correct final count is **9** — 7 under `functions/`, 2 under `docs/`; `vitest.postgres.config.ts` carries no net change and is not part of the final diff), and the targeted-timeout narrative still stated "7 tests" (the correct count is **10** — 3 pre-existing top-level tests, 1 `PLATFORM-BASELINE-010B` test, 6 `PLATFORM-BASELINE-013A.1` tests). Both corrected in place; **no migration, test, or config file was touched.** See the new **"Correction 002"** section (after "Correction 001") for the full record.
...
**F3 measurement (per §"Correction 001" for the full timeout-strategy analysis):** run twice against vitest's plain, unmodified defaults (5000ms test / 10000ms hook, no config override of any kind), capturing every individual test's own reported duration. Both runs: **30/30 pass** (pre-F8; 35/35 after F8 added the 5 new tests, confirmed separately). Worst individual test duration across both clean runs: 3805ms (`rolls back the full migration set and re-applies cleanly`); every other test measured under 2950ms; most single-`migrateUp` constraint tests measured 1000-2200ms. A single non-reproduced outlier of 7816ms on the same test was observed once, earlier in this task, under a period of unusually heavy multi-session host contention (37 combined seconds of test-suite time that run vs. the usual ~35-50s). See §"Correction 001" F3 (as amended by §"Correction 002") for the resulting targeted-timeout decision (15000ms, applied to 10 tests that perform a full `migrateUp`/`migrateDown`/multi-step-scratch-directory sequence over the real migrations directory).
...
Confirmed. No file under `functions/src/index.ts`, any `*Command.ts`, or any parser was touched. `git diff --stat` against `origin/main` (see §6 for the full, corrected inventory — **9 files** [corrected by Correction 002; see §"Correction 002"]: 7 under `functions/src/infrastructure/postgres/`, none under `functions/src/domains/` or `functions/src/index.ts`, plus 2 under `docs/`) confirms no request-contract file is among them.
...
Confirmed. `git diff --stat` against `origin/main` shows exactly the **9 files** listed in §6 (corrected by CORR-001 F1, file count itself further corrected by Correction 002) — 7 under `functions/src/infrastructure/postgres/`, 2 under `docs/`, and `vitest.postgres.config.ts` net-unchanged (confirmed **not** a file in the final diff). No file under `functions/src/domains/`, `functions/src/index.ts`, or `apps/` — no unrelated file touched.
...
**Determination:** the vitest default is adequate for every test except a specific, identifiable group that performs multiple full `migrateUp`/`migrateDown` calls or a multi-step scratch-directory sequence over the real (17-migration) directory — exactly the tests whose measured durations above already sit closest to 5000ms, and the same tests implicated in every timeout failure observed across this task. **Final strategy:** the file-wide `testTimeout: 20000` is **removed** (config reverted to byte-identical to `origin/main`); a **targeted, per-test `it(name, fn, 15000)` timeout** is applied to **10 tests** in this category (corrected by **Correction 002** — this section originally undercounted the group as "7"; see §"Correction 002" for the corrected breakdown: 3 top-level tests, the `PLATFORM-BASELINE-010B` pre-existing-rows test, and 6 PB-013A.1-specific backfill/rollback tests). 15000ms was chosen as the smallest round value clearing the single worst measured spike (7816ms) with comfortable (~2x) margin, without being large enough to mask a genuine hang. Every other test in the file — 25 of the file's 35 — keeps vitest's plain, unmodified defaults.
...
## Correction 002 (2026-09-18, lightweight pre-merge record hygiene, `ITR-002`)

**Basis:** independent narrow re-review `ITR-002` of PR #260 at the CORR-001 head `514cd03e56f17a1e81e72a811425244e607bb887`, disposition **A — CORRECTION INDEPENDENTLY VERIFIED / APPROVABLE FOR MERGE**. `ITR-002` found the underlying `0016`/`0017` persistence architecture, `0017.down`'s rollback precondition, the deterministic `ORDER BY`, and all F8 test coverage independently sound, with **no regression and no new correctness/security finding**. It identified exactly two residual **documentation-arithmetic** inconsistencies in this report, both of the same class as F1 (a stale historical count left in a final-state sentence). **This is pre-merge record hygiene, not a further engineering correction** — no migration, test, or config file is touched by this section; only this report and the changes-log entry are corrected.

**Correction A — final file count.** §6's own itemized list was always correct (9 numbered files: 7 under `functions/`, 2 under `docs/`), and `vitest.postgres.config.ts` was already correctly described there as reverted/net-unchanged. But §16, §19 (now corrected above), and the GitHub PR description separately stated "the 10 files listed in §6" / "touches 10 files: 7 under `functions/`, 2 under `docs/`" — an internal arithmetic error (7+2=9, not 10) that conflated the **historical pre-correction diff** (genuinely 10 files, before `vitest.postgres.config.ts`'s change was reverted) with the **final, current diff** (9 files, since that file carries no net change and is not part of the diff at all). Corrected in §16, §19, §20, and §28 above; the PR description corrected in the same commit. Statements that explicitly describe the pre-correction state (§6's opening sentence, §"Correction 001" F1's own root-cause narrative) are left untouched, since they accurately describe history, not the final PR.

**Correction B — targeted timeout count.** §"Correction 001" F3 and the corresponding sentences in §6/§15 stated the `15000ms` per-test override was applied to "exactly 7" tests. Direct inspection of the final test file shows **10 tests** carry the override:

- **3 pre-existing/top-level heavy migration tests:** `bootstraps a clean empty database and applies all migrations in order`; `re-running migrateUp against an already-migrated database is a safe no-op`; `rolls back the full migration set and re-applies cleanly`.
- **1 `PLATFORM-BASELINE-010B` migration test:** `applying 0001-0014 then 0015 against a table with existing non-null-category rows preserves every existing value and allows a new NULL row afterward`.
- **6 `PLATFORM-BASELINE-013A.1` migration tests:** the `0017` backfill-synthesis test; the backfill-idempotency test; `0017.down removes only the migration's own synthesized items...` (compatibility-only rollback); `0017.down refuses to run when a genuine...binding...` (F4 rollback-refusal); the two-Businesses isolation test; the `NULL business_display_name` fallback test.

Total: 3 + 1 + 6 = **10**. All 10 are structurally consistent with the original measurement rationale (each performs a full `migrateUp`, a `migrateUp`+`migrateDown` pair, or a scratch-directory-then-real `migrateUp` sequence) — `ITR-002` independently confirmed none is an unjustified broad timeout weakening, and confirmed `vitest.postgres.config.ts` remains byte-identical to `origin/main` with no shared/global timeout of any kind. **The timeout implementation itself is unchanged by this correction** — only the narrative describing it, in §6/§15/§"Correction 001" above, is corrected to state 10 (broken down as above) rather than 7.

**Scope discipline:** no migration, test, config, domain, permission, UI, schema, or dependency file was touched by this correction — confirmed by `git diff --stat` below. `DEC-LOY-016`, `DEC-LOY-017`, Commerce Knowledge content, and seed content are untouched. `PLATFORM-BASELINE-013A.2` was not started.

**Review threads:** none exist on PR #260 beyond the one pre-existing `chatgpt-codex-connector` usage-limit notice (not a review) already recorded in §"Correction 001".

---

## FINAL DISPOSITION

**PLATFORM-BASELINE-013A.1 — APPROVED / MERGED / CLOSED.**

The additive PostgreSQL persistence foundation for the Business-owned Qualifying Item model (`DEC-LOY-016`) is implemented exactly to the approved `PLATFORM-BASELINE-012` design's §5/§6/§8/§13 specification. `ITR-001`'s findings were corrected without redesigning that architecture (F1–F8, `Correction 001`), and `ITR-002` independently verified that correction and found it approvable for merge, subject only to the two documentation-arithmetic fixes recorded in `Correction 002` above (final file count 9, not 10; targeted timeout count 10, not 7 — both narrative-only, no implementation change). Nothing outside `functions/` and this package's own two `docs/` files was ever touched; no request contract, Reward Program behaviour, purchase behaviour, or UI changed; no runtime path depends on the new schema; `vitest.postgres.config.ts` carries no net change. All available validation (58 postgres-infra tests, 1765 unit tests, typecheck, lint, build, `git diff --check`) passes; the two Firestore-emulator-dependent suites remain disclosed as not run in this environment rather than assumed passing. `PLATFORM-BASELINE-013A.2` and later packages remain not started.
