...
- **Explicit JDK 21 setup and verification before the emulator step**, per prompt §4.4. The initial run ([`29608691029`](https://github.com/Fkenogo/11THONUS/actions/runs/29608691029)) exposed that GitHub's `ubuntu-latest` image's default `java` resolves to Temurin 17, and `firebase-tools` now requires 21+ — see §14 for the full evidence. The finalized workflow installs `actions/setup-java@v4` (`distribution: temurin`, `java-version: "21"`) immediately before the `Verify Java runtime` step, so that step now confirms the corrected version rather than the runner's stale default before `Firebase Emulator Suite validation` runs.
...
- The Firestore Emulator's CI Java requirement (prompt §4.4) is now controlled, not merely investigated: the workflow explicitly installs Temurin JDK 21 before the emulator step, and this is verified by two independent passing CI runs ([`29609566311`](https://github.com/Fkenogo/11THONUS/actions/runs/29609566311), [`29609818932`](https://github.com/Fkenogo/11THONUS/actions/runs/29609818932)) — see §14.
...
- Informational, non-blocking: GitHub's own CI logs note several actions used in this workflow target Node.js 20 internally but are being run on Node.js 24 by the runner — unrelated to this repository's own Node 20 application target; a future action-version-bump item, not a defect.

## 12. Unresolved Issues

None for ENG-P0-002. The initial run ([`29608691029`](https://github.com/Fkenogo/11THONUS/actions/runs/29608691029)) failed at the `Firebase Emulator Suite validation` step because the runner's default Java was version 17; the corrected run ([`29609566311`](https://github.com/Fkenogo/11THONUS/actions/runs/29609566311)) passed with JDK 21 explicitly installed; the subsequent documentation-only commit's run ([`29609818932`](https://github.com/Fkenogo/11THONUS/actions/runs/29609818932)) also passed. There is no remaining open question about Firebase Emulator Suite reliability in CI for this work package.
