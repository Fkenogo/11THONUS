/**
 * Managed reCAPTCHA verifier lifecycle for the optional Phone flow, used by
 * the production `SignInPage` (`PRODUCT-ALIGN-002`).
 *
 * Deliberately duplicated from `dev/signInPreview/recaptchaLifecycle.ts`
 * rather than cross-imported from a `dev/` module into the production sign-in
 * path — mirrors this codebase's own "disclosed duplication" convention
 * (see `identity/api/identityCallableClient.ts`) rather than having
 * production code depend on a dev/preview-owned module. Behaviour is
 * identical: a `RecaptchaVerifier` owns injected iframe/widget resources, so
 * constructing a new one per send without clearing the previous one would
 * leak widgets and listeners for the page's lifetime. This helper retains the
 * current verifier + its container node and tears both down before creating
 * the next and on unmount. Kept pure and dependency-injected so the lifecycle
 * is unit-testable without Firebase/DOM.
 */
